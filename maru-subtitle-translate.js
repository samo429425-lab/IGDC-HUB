...
function isInstructionLeak(text) {
  ...
  return /(?:system\s*(?:prompt|message|policy)|developer\s*message|internal\s*(?:policy|instruction)|return\s+(?:json|only)|target\s+language\s*[:=]|source\s+(?:file|language)|translate\s+only\s+subtitle|preserve\s+every\s+cue|audible\s+(?:spoken|dialogue|lyric)|clearly\s+sung|musical\s+(?:note|marker|image)|instrumental[-\s]*only|vocal\s+onset|(?:자막|가사|음성)\s*(?:생성|번역|출력)\s*(?:규칙|지침|정책|안내)?|(?:명확하게|실제)\s*(?:노래|가사|음성)|(?:짧은|하나의)\s*(?:음표|음악)|(?:가사가|음성이)\s*명확하게\s*(?:불려|들려)|음악적(?:이거나|인)|(?:내부|시스템)\s*(?:지침|정책|규정|안내))/iu.test(value);
}
