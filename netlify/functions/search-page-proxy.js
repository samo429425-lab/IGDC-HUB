...
function metaContent(markup, name){
  ...
  let found = '';
  String(markup || '').replace(/<meta\b[^>]*>/ig, function(tag){
    if(found) return tag;
    const key = (attrValue(tag, 'name') || attrValue(tag, 'property')).toLowerCase();
    if(key === wanted) found = attrValue(tag, 'content');
    return tag;
  });
  return found;
}
...
function snapshotDocument(htmlText, finalUrl){
  ...
  const title = firstMatch(raw, /<title[^>]*>([\s\S]*?)<\/title>/i) || metaContent(raw, 'og:title') || finalUrl;
  const desc = metaContent(raw, 'description') || metaContent(raw, 'og:description') || stripTags(raw).slice(0, 900);
  const image = absoluteUrl(metaContent(raw, 'og:image') || metaContent(raw, 'twitter:image'), finalUrl);
  const host = (() => { try { return new URL(finalUrl).hostname.replace(/^www\./,''); } catch(e){ return ''; } })();
  let links = [];
  raw.replace(/<a\b[^>]*href\s*=\s*(["'])(.*?)\1[^>]*>([\s\S]*?)<\/a>/ig, function(_, q, href, label){
    if(links.length >= 12) return '';
    const abs = absoluteUrl(href, finalUrl);
    const text = stripTags(label).slice(0, 90);
    if(/^https?:\/\//i.test(abs) && text) links.push({ href: abs, text });
    return '';
  });
  const linkHtml = links.map(l => '<a href="' + escapeHtml(proxyUrl(l.href, finalUrl, { mode:'static' })) + '">' + escapeHtml(l.text) + '</a>').join('');
  return '<!doctype html><html lang="ko"><head><meta charset="utf-8"><base href="' + escapeHtml(finalUrl) + '"><style>html,body{margin:0;background:#fff;color:#111827;font-family:system-ui,-apple-system,Segoe UI,sans-serif}.wrap{max-width:1060px;margin:0 auto;padding:24px 26px 38px}.host{font-size:13px;font-weight:800;color:#2563eb;margin-bottom:8px}.title{font-size:26px;line-height:1.25;font-weight:900;letter-spacing:-.02em;margin:0 0 10px}.desc{font-size:15px;line-height:1.7;color:#334155;white-space:pre-line}.hero{max-width:100%;max-height:420px;object-fit:contain;border-radius:14px;border:1px solid #e5e7eb;background:#f8fafc;margin:14px 0}.links{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:8px;margin-top:18px}.links a{display:block;padding:10px 12px;border:1px solid #e5e7eb;border-radius:10px;color:#1d4ed8;text-decoration:none;font-weight:700;background:#f8fafc}.url{margin-top:14px;color:#64748b;font-size:12px;word-break:break-all}</style>' + lightweightBridge(finalUrl, { proxyId:'' }) + '</head><body><div class="wrap"><div class="host">' + escapeHtml(host) + '</div><h1 class="title">' + escapeHtml(stripTags(title)) + '</h1>' + (image && /^https?:\/\//i.test(image) ? '<img class="hero" src="' + escapeHtml(image) + '" alt="">' : '') + '<div class="desc">' + escapeHtml(desc || '원문 페이지의 주요 내용을 IGDC 검색 화면 안에서 표시합니다.') + '</div>' + (linkHtml ? '<div class="links">' + linkHtml + '</div>' : '') + '<div class="url">' + escapeHtml(finalUrl) + '</div></div></body></html>';
}
