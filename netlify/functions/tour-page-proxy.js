...
function rewriteAttributes(markup, baseUrl, opts){
  ...
  out = out.replace(/<script\b([^>]*?)\ssrc\s*=\s*(["'])(.*?)\2/ig, function(m, before, quote, src){
    const u = live ? assetProxyUrl(src, baseUrl, 'script') : absoluteUrl(src, baseUrl);
    return '<script' + before + ' src=' + quote + escapeHtml(u) + quote;
  });

  out = out.replace(/<link\b([^>]*?)\shref\s*=\s*(["'])(.*?)\2/ig, function(m, before, quote, href){
    const isCss = /\brel\s*=\s*(["'])?stylesheet\1?/i.test(before) || /\.css(?:[?#]|$)/i.test(href);
    const u = (live && isCss) ? assetProxyUrl(href, baseUrl, 'style') : absoluteUrl(href, baseUrl);
    return '<link' + before + ' href=' + quote + escapeHtml(u) + quote;
  });
...
function rewriteCssForRelay(cssText, finalUrl){
  let out = String(cssText || '');
  out = out.replace(/url\((['"]?)([^'"\)]+)\1\)/ig, function(m, q, raw){
    raw = String(raw || '').trim();
    if(!raw || /^data:|^blob:|^javascript:/i.test(raw)) return m;
    return 'url("' + assetProxyUrl(raw, finalUrl, 'asset').replace(/"/g, '%22') + '")';
  });
  out = out.replace(/@import\s+(?:url\()?\s*(["'])(.*?)\1\s*\)?/ig, function(m, q, raw){
    return '@import url("' + assetProxyUrl(raw, finalUrl, 'style').replace(/"/g, '%22') + '")';
  });
  return out;
}
...
async function frameCheck(event, target){
  ...
  try{
    ...
    return json(200, { ok:true, directAllowed:true, reason:'probe-unavailable-direct-preferred:' + String(e && e.message || e || 'frame-check-failed'), finalUrl: target.href });
  }finally{
    clearTimeout(timer);
  }
}

function targetLanguage(target, fallback){
  const h = String((target && target.hostname) || '').toLowerCase();
  if(/\.co\.kr$|\.kr$/.test(h)) return 'ko-KR,ko;q=0.9,en-US;q=0.7,en;q=0.6';
  if(/\.co\.jp$|\.jp$/.test(h)) return 'ja-JP,ja;q=0.9,en-US;q=0.7,en;q=0.6';
  if(/\.cn$|taobao\.com$|tmall\.com$|jd\.com$|pinduoduo\.com$/.test(h)) return 'zh-CN,zh;q=0.9,en-US;q=0.6,en;q=0.5';
  if(/\.in$/.test(h)) return 'en-IN,en;q=0.9,hi-IN;q=0.7';
  if(/\.com\.au$|\.au$/.test(h)) return 'en-AU,en;q=0.9';
  if(/\.de$/.test(h)) return 'de-DE,de;q=0.9,en;q=0.7';
  if(/\.ru$/.test(h)) return 'ru-RU,ru;q=0.9,en;q=0.6';
  if(/\.kz$/.test(h)) return 'ru-KZ,ru;q=0.8,kk;q=0.7,en;q=0.5';
  if(/\.sa$/.test(h)) return 'ar-SA,ar;q=0.9,en;q=0.7';
  if(/\.ae$/.test(h)) return 'en-AE,en;q=0.9,ar;q=0.7';
  return fallback || 'en-US,en;q=0.9';
}
...
function copyRequestHeaders(event, target, profile){
  ...
  const headers = {
    ...
    'Accept-Language': targetLanguage(target, inHeaders['accept-language'] || inHeaders['Accept-Language']),
...
function upstreamProblemScore(text, status){
  const t = String(text || '').toLowerCase();
  let score = 0;
  const s = Number(status || 0);
  if(s >= 400) score += 80;
  const bad = [
    'verify you are human','captcha','access denied','automated access','unusual traffic',
    'robot check','bot detection','봇(bot)','간단한 확인 절차','사람이 직접 조작하지 않고',
    "we couldn't find that page",'web address you entered is not a functioning page',
    'sorry, we couldn\'t find that page','request blocked','temporarily unavailable'
  ];
  for(const needle of bad){ if(t.includes(needle)) score += 55; }
  const plain = stripForText(text);
  if(plain.length < 120) score += 20;
  return score;
}

async function fetchProfileOnce(event, target, fetchOpts, profile){
  const ctrl = new AbortController();
  const timer = setTimeout(() => { try{ ctrl.abort(); }catch(e){} }, 4200);
  try{
    const opts = Object.assign({}, fetchOpts || {}, { signal:ctrl.signal });
    opts.headers = copyRequestHeaders(event, target, profile);
    return await fetch(target.href, opts);
  } finally { clearTimeout(timer); }
}

async function improveProblemHtml(event, target, fetchOpts, firstPack){
  let best = firstPack;
  let bestScore = upstreamProblemScore(best.text, best.status);
  if(bestScore < 45) return best;
  const tries = await Promise.allSettled([
    fetchProfileOnce(event, target, fetchOpts, 'no-referer'),
    fetchProfileOnce(event, target, fetchOpts, 'mobile')
  ]);
  for(const row of tries){
    ...
    const pack = { status:res.status || 200, text, finalUrl:res.url || target.href, contentType:ct };
    const sc = upstreamProblemScore(text, pack.status);
    if(sc < bestScore || (sc === bestScore && stripForText(text).length > stripForText(best.text).length)){
      best = pack; bestScore = sc;
    }
  }
  return best;
}
...
exports.handler = async function(event){
  ...
  try{
    ...
    let contentType = upstream.headers.get('content-type') || 'text/html; charset=utf-8';
    let finalUrl = upstream.url || target.href;

    if(!/text\/html|application\/xhtml\+xml/i.test(contentType)){
      const buf = Buffer.from(await upstream.arrayBuffer());
      if(String(qp.asset || '') === '1' && /text\/css/i.test(contentType)){
        const css = rewriteCssForRelay(buf.toString('utf8'), finalUrl);
        return response(upstream.status || 200, css, { 'Content-Type': contentType, 'Cache-Control':'public, max-age=300' }, false);
      }
      return response(upstream.status || 200, buf.toString('base64'), { 'Content-Type': contentType, 'Cache-Control': String(qp.asset || '') === '1' ? 'public, max-age=300' : 'no-store' }, true);
    }

    let text = await upstream.text();
    let pack = { status:upstream.status || 200, text, finalUrl, contentType };
    if(upstreamProblemScore(text, pack.status) >= 45){
      try{ pack = await improveProblemHtml(event, target, fetchOpts, pack); }catch(e){}
      text = pack.text; finalUrl = pack.finalUrl; contentType = pack.contentType || contentType;
    }
