...
function paymentUrl(identity, deviceId) {
  ...
  try {
    ...
    if (identity) url.searchParams.set('license_identity', identity);
...
exports.handler = async (event) => {
  ...
  const paid = paidSet();
  if (key && owners.has(key)) return json(200, { ok: true, active: true, allowed: true, status: 'active', plan: 'OWNER_ADMIN', role: 'owner', source: 'owner-allowlist', paymentUrl: paymentUrl(identity, deviceId) });
  if (key && paid.has(key)) return json(200, { ok: true, active: true, allowed: true, status: 'active', plan: 'AI_PRO', role: 'paid_user', source: 'paid-allowlist', paymentUrl: paymentUrl(identity, deviceId) });
  return json(200, { ok: true, active: false, allowed: false, status: 'payment_required', plan: 'FREE', role: 'guest_or_unpaid', message: identity ? 'AI Pro payment is required.' : 'Login or member identity is required.', paymentUrl: paymentUrl(identity, deviceId) });
};
