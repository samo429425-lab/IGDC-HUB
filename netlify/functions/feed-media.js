...
export async function buildMediaSnapshotFromBank() {
  ...
  // candidate channel keys (bank마다 다를 수 있음)
  const CHANNEL_CANDIDATES = ["media","video","ott","movie","drama","shorts","music","animation","documentary"];

  const channelIds = [];
  for(const ck of CHANNEL_CANDIDATES){
    const ids = byChannel?.[ck];
    if(Array.isArray(ids)) channelIds.push(...ids);
  }
  ...
  // 2) 섹션이 전부 비었으면 channelIds로 라운드로빈 분배
  const hasAny = FRONT_SECTION_KEYS.some((k) => buckets[k].length > 0);
  if (!hasAny && channelIds.length > 0) {
    let cursor = 0;
    channelIds.forEach((id) => {
      ...
      buckets[key].push(normalizeFromBankItem(it));
      cursor++;
    });
  }

  // 3) 그래도 비었으면 items 자체에서 media 후보 추출 후 분배
  const hasAny2 = FRONT_SECTION_KEYS.some((k) => buckets[k].length > 0);
  if (!hasAny2 && itemList.length > 0) {
    const mediaCandidates = itemList.filter(looksLikeMedia);
    // 최신 우선(가능하면)
...
export const handler = async (event = {}) => {
  try {
    ...
    const limit = Math.max(1, Math.min(500, Number.isFinite(limitRaw) && limitRaw > 0 ? limitRaw : 0));
