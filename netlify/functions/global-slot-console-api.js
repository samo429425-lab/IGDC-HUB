...
async function publish(actor, body) {
  requireCapability(actor, 'approve');
  const requestedScope = clean(body.scopeCountry, 3).toUpperCase() || 'GLOBAL';
  if (requestedScope !== 'GLOBAL') {
    const e = new Error('현재 Netlify 정적 Snapshot 발행은 GLOBAL 배치만 반영합니다. 국가별 배치는 관리 원장에 보존되며, 국가 선택 Front 소비 계약이 연결된 뒤 별도 발행됩니다.');
    e.statusCode = 409;
    throw e;
  }
  const assignments = await sb.select('gslot_slot_assignments', 'select=id,candidate_id,hub_key,country_code,region_code,slot_key,priority,state,publication_status,manual_pinned,candidate:gslot_candidates(id,title,status)&state=in.(approved,pinned)&publication_status=eq.ready');
  const globalAssignments = (assignments || []).filter(row => row.country_code === 'GLOBAL');
  const deferredCountryAssignments = (assignments || []).filter(row => row.country_code && row.country_code !== 'GLOBAL');
  const release = {
    id: sb.id('release'), scope_country: 'GLOBAL', requested_by: actor.sub, status: 'requested', assignment_count: globalAssignments.length,
    detail: {
      assignmentIds: globalAssignments.map(row => row.id),
      deferredCountryAssignmentCount: deferredCountryAssignments.length,
      publicationMode: 'global_static_snapshot_only'
    },
    created_at: iso()
  };
  await sb.insert('gslot_publication_releases', release, 'return=minimal');
  const hook = clean(process.env.NETLIFY_GSLOT_BUILD_HOOK_URL);
  let hookState = 'queued';
  if (hook) {
    try {
      const response = await fetch(hook, { method: 'POST' });
      if (!response.ok) throw new Error('Build hook HTTP ' + response.status);
      hookState = 'build_requested';
    } catch (error) {
      hookState = 'build_request_failed';
      release.detail.hookError = String(error && error.message || error).slice(0, 400);
    }
  }
  await sb.update('gslot_publication_releases', 'id=eq.' + encodeURIComponent(release.id), { status: hookState, updated_at: iso() }, 'return=minimal');
  await audit(actor, 'request_publication', 'release', release.id, { scopeCountry: 'GLOBAL', assignmentCount: globalAssignments.length, deferredCountryAssignmentCount: deferredCountryAssignments.length, hookState });
  const next = hook
    ? 'GLOBAL 승인 배치의 Netlify 재배포를 요청했습니다.'
    : 'GLOBAL 승인 배치를 기록했습니다. 현재 GitHub push로 배포하면 오버레이가 생성됩니다. NETLIFY_GSLOT_BUILD_HOOK_URL을 추가하면 이후 승인 발행 때 자동 재배포됩니다.';
  return { ok: true, release: Object.assign({}, release, { status: hookState }), next };
}
...
exports.handler = async function handler(event) {
  try {
    ...
    else if (action === 'publish' && method !== 'GET') out = await publish(actor, body);
