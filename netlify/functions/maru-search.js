...
const VERSION = 'A1.5.2-operational-wide-internal';
...
// ===== SEARCH BANK INTERNAL CALL (keeps search volume, removes URL roundtrip) =====
...
function getSearchBankEngine(){
  ...
  try { SearchBankEngine = require("./search-bank-engine"); }
  catch(e) { SearchBankEngine = null; }
  return SearchBankEngine;
}

function unwrapSearchBankResult(data){
  if(!data) return null;
  if(Array.isArray(data.items)) return { source:'search-bank', results:data.items, total: typeof data.total === 'number' ? data.total : null };
  if(Array.isArray(data.results)) return { source:'search-bank', results:data.results, total: typeof data.total === 'number' ? data.total : null };
  if(data.data && Array.isArray(data.data.items)) return { source:'search-bank', results:data.data.items, total: typeof data.total === 'number' ? data.total : null };
  if(data.baseResult && Array.isArray(data.baseResult.items)) return { source:'search-bank', results:data.baseResult.items, total: typeof data.total === 'number' ? data.total : null };
  if(data.baseResult && data.baseResult.data && Array.isArray(data.baseResult.data.items)) return { source:'search-bank', results:data.baseResult.data.items, total: typeof data.total === 'number' ? data.total : null };
  return null;
}
...
async function callSearchBankEngineInternal(event, params){
  try{
    ...
    // Keep the 345 search path open. Do not force external/off/live/off/collector/off here.
    // Only mark the caller so search-bank will not recurse back into maru-search.
    const safeParams = {
      ...(params || {}),
      from: 'maru-search',
      noAnalytics: '1',
    ...
    if(typeof engine.runEngine === 'function'){
      return unwrapSearchBankResult(await engine.runEngine(event || {}, safeParams));
    }
    ...
    if(typeof engine.handler === 'function'){
      ...
      if(typeof res === 'object' && Object.prototype.hasOwnProperty.call(res, 'body')){
        try { return unwrapSearchBankResult(JSON.parse(res.body || '{}')); }
        catch(e) { return null; }
      }
      return unwrapSearchBankResult(res);
    }

    if(typeof engine === 'function'){
      return unwrapSearchBankResult(await engine(event || {}, safeParams));
    }
...
const Containers = {
  ...
  search_bank: {
    ...
    async fetch(q, limit, offset, event){
      return await callSearchBankEngineInternal(event, {
        q: String(q || ''),
        query: String(q || ''),
        limit,
        offset: offset || 0
...
async function orchestrateSearch({ event, q, limit, start, lang, external, noExternal, disableExternal, deep, includeMedia }) {
  ...
  const SEARCH_SOFT_TIMEOUT_MS = deepMode ? 9500 : 8500;
  const TARGET_MIN_RESULTS = Math.min(MAX_LIMIT, Math.max(limit || 0, deepMode ? 800 : 500));
  const mediaIntent = /(image|photo|picture|video|watch|media|이미지|사진|포토|영상|동영상|미디어)/i.test(String(q || ''));

  const budgets = {
    searchBankPages: deepMode ? 10 : 10,      // internal call; keep enough search-bank volume
    naverPages: externalSuppressed ? 0 : (deepMode ? 6 : 4),
    googlePages: externalSuppressed ? 0 : (deepMode ? 5 : 3), // each page performs web + news CSE calls
    bingPages: externalSuppressed ? 0 : (deepMode ? 5 : 3),
