...
const VERSION = 'A1.5-runtime-routing+sanmaru-v2-hook';
...
// ===== SANMARU ENGINE v2 HOOK =====
function sanmaruSearchKey(it){
  if(!it) return "";
  return String(
    it.id ||
    it.url ||
...
function sanmaruDecorateInputItem(it){
  it = (it && typeof it === "object") ? it : {};
  const title = String(it.title || "").toLowerCase();
  const summary = String(it.summary || it.description || "").toLowerCase();
  const source = String(it.source || "").toLowerCase();
  const mediaType = String(it.mediaType || it.type || "").toLowerCase();

  let trustScore = Number(it.trustScore || it.sourceTrust || 0);
  if(!trustScore){
    if(source.includes("search-bank")) trustScore = 0.76;
    else if(source.includes("google") || source.includes("naver") || source.includes("bing")) trustScore = 0.72;
    else if(source.includes("youtube")) trustScore = 0.66;
    else trustScore = 0.55;
  }

  let impact = Number(it.impact || 0);
  if(mediaType === "video" || mediaType === "image") impact += 0.08;
  if(title || summary) impact += 0.05;

  return {
    ...it,
    trustScore,
    sourceTrust: Number(it.sourceTrust || trustScore),
    impact,
    freshnessSeconds: Number(it.freshnessSeconds || 0),
    entities: Array.isArray(it.entities) ? it.entities : [],
    tags: Array.isArray(it.tags) ? it.tags : []
  };
}

async function applySanmaruV2Hook(query, items, opts){
  const baseItems = Array.isArray(items) ? items : [];

  if(String(process.env.MARU_SANMARU_SEARCH || "").toLowerCase() === "off"){
    return { items: baseItems, meta: { used:false, reason:"env_off" } };
  }

  if(!SanmaruEngine || typeof SanmaruEngine.runSanmaru !== "function"){
    return { items: baseItems, meta: { used:false, reason:"not_loaded" } };
  }

  if(!baseItems.length){
    return { items: baseItems, meta: { used:false, reason:"empty_items" } };
  }
  ...
  try{
    const maxResults = Math.max(
      1,
      Math.min(
        Number((opts && opts.limit) || baseItems.length || 200),
        1000
      )
    );

    const localResultPoolAdapter = {
      name: "maru-search-result-pool",
      match: function(){ return true; },
      access: async function(){
        return baseItems.map(sanmaruDecorateInputItem);
      }
    };

    const result = await SanmaruEngine.runSanmaru(String(query || ""), {
      adapters: [localResultPoolAdapter],
      additionalEngineNames: [],
      maxResults,
      promotionThreshold: 0.88,
      bufferTtlMs: 1000 * 60 * 10
    });

    const ranked = Array.isArray(result && result.items) ? result.items : [];
    if(!ranked.length){
      return { items: baseItems, meta: { used:true, count:0, promoted:0, mode:"no_ranked_items" } };
    }

    const rankMap = new Map();
    ranked.forEach((it, idx) => {
      const key = sanmaruSearchKey(it);
      if(key && !rankMap.has(key)){
        rankMap.set(key, {
          idx,
          finalScore: Number(it.finalScore || it.score || 0),
          logosWeight: Number(it.logosWeight || 0),
          trustScore: Number(it.trustScore || 0)
        });
      }
    });

    const originalRanked = baseItems
      .map((it, idx) => {
        const key = sanmaruSearchKey(it);
        const hit = rankMap.get(key) || null;
        return {
          ...it,
          _sanmaruRank: hit ? hit.idx : 999999 + idx,
          _sanmaruScore: hit ? hit.finalScore : Number(it._finalScore || it.score || 0),
          _sanmaruLogosWeight: hit ? hit.logosWeight : undefined,
          _sanmaruTrustScore: hit ? hit.trustScore : undefined
        };
      })
      .sort((a, b) => {
        if((a._sanmaruRank || 0) !== (b._sanmaruRank || 0)) return (a._sanmaruRank || 0) - (b._sanmaruRank || 0);
        return Number(b._sanmaruScore || 0) - Number(a._sanmaruScore || 0);
      });

    return {
      items: originalRanked,
      meta: {
        used: true,
        version: SanmaruEngine.VERSION || "SANMARU-v2",
        count: ranked.length,
        promoted: Array.isArray(result.promoted) ? result.promoted.length : 0,
        recalled: Array.isArray(result.recalled) ? result.recalled.length : 0,
        intent: result.meta && result.meta.intent ? result.meta.intent : null,
        mode: "rerank_existing_search_pool"
      }
    };
  }catch(e){
    return {
...
exports.handler = async function (event) {
  try {
    ...
    const sanmaru = await applySanmaruV2Hook(q, base.items, { event, limit, start, lang });
    const finalItems = sanmaru.items || base.items || [];

    // 🔧 비동기 처리 (응답 속도 확보)
    syncSearchAnalytics(event, q, finalItems);
    distributeRevenue(event, finalItems);
...
exports.runEngine = async function(event = {}, params = {}){
  ...
  const sanmaru = await applySanmaruV2Hook(q, base.items, {
    event,
    limit: params.limit,
    start: params.start,
    lang: params.lang || null
