...
function packForLocale(locale){
  const base=LOCAL_QUERY_PACKS[locale]||LOCAL_QUERY_PACKS[baseLocale(locale)]||null;
  if(!base)return null;
  const categories=base.categories.map((value,index)=>{
    const key=CATEGORY_KEYS[index],suffix=focusedCategorySuffix(locale,key);
    return suffix?(value+" "+suffix).replace(/\s+/g," ").trim():value;
  });
  return{commerce:base.commerce,categories};
}
...
function queryCategories(geo){
  ...
  const chosen=[];
  for(const index of positive.concat(defaultFocus,neutral,ranked)){
    if(!chosen.includes(index))chosen.push(index);
    if(chosen.length>=5)break;
  }
  while(chosen.length<5&&chosen.length<size){const index=(offset+chosen.length)%size;if(!chosen.includes(index))chosen.push(index);else break;}
  return chosen;
}
...
function stagedGeo(params){
  ...
  const categoryWeights={};for(const key of CATEGORY_KEYS)categoryWeights[key]=Math.max(-20,Math.min(20,Number(requested.categoryWeights&&requested.categoryWeights[key])||0));
  return {country,region:Core.normalizeRegion(requested.region||requested.targetRegion,country),city:"",countryName:Core.COUNTRY_NAMES[country]||country,categoryWeights,signalPlanVersion:text(requested.signalPlanVersion),policyHints:policyHints(requested.policyHints),scopeAuthority:text(requested.scopeAuthority)||"explicit-request",ignoreRequestGeo:requested.ignoreRequestGeo===true};
}
...
async function stagedSanmaru(event,row,geo,limit,timeoutMs,externalFallback){
  ...
  const allowExternal=externalFallback===true;
  try{const wait=boundedResearchTimeout(timeoutMs,7500,5000,10000),result=await withTimeout(Sanmaru.runEngine(event||{},{q:row.query,query:row.query,country:geo.country,targetCountry:geo.country,queryCountry:geo.country,region:geo.region||undefined,limit:Math.min(20,limit||18),candidatePool:allowExternal?72:48,language:row.locale,locale:row.locale,type:"site",channel:"commerce",entity:"supplier",external:allowExternal?"force":"off",directExternal:allowExternal?"1":"0",noExternal:allowExternal?"0":"1",noMedia:"1",deep:allowExternal?"1":"0",timeoutMs:wait,from:"regional-brokerage-autoselector-staged",source:"regional-brokerage-autoselector-staged",regionalBrokerageSupply:"1",scopeAuthority:geo.scopeAuthority||"explicit-request",ignoreRequestGeo:geo.ignoreRequestGeo===true,noAnalytics:"1",noRevenue:"1",readOnly:"1",noWrite:"1",noSync:"1",writeMode:"readonly"}),wait+800);const items=researchFirst(extractItems(result));return{provider:"sanmaru",status:items.length?"ok":"empty",detail:allowExternal?"external_fallback_enabled":null,items};}catch(error){return{provider:"sanmaru",status:providerErrorCode(error),detail:providerErrorDetail(error),items:[]};}
}
