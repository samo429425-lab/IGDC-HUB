...
const VERSION = "commerce-policy-discussion-v1.4.0-admin-execution-proposals";
...
const MANAGEMENT_OPERATIONS = new Set(["queue_register","latest_delete","queue_unmatch","ai_place","hold","reject","restore","remove_from_list","purge"]);
const MANAGEMENT_FILTERS = new Set(["selected","missing_thumbnail","missing_url","invalid_url","document_link","duplicate_url","title_contains","url_contains","supplier_contains","all"]);
...
function fallbackProposal(scope,error,language){return{summary:fallbackSummary(language),objectives:[],categoryWeights:categoryWeights({}),priorityDirections:[],avoidDirections:[],manualPriorityTargets:[],manualBlockedTargets:[],executionPlan:[],managementActions:[],risks:[clean(error,240)||"AI_POLICY_DISCUSSION_FAILED"],evidenceNeeded:[],validityDays:scope.scopeType==="global"?30:scope.scopeType==="regional"?21:30,confidence:0,safety:{trustGateChanged:false,supplierApproval:false,productImport:false,publicPublication:false,administratorDecisionRequired:true}};}
async function aiProposal(scope,workspace,instruction,context,requestedLanguage,managementContext){
...
async function appendExecutionLog(actorId,input){
  const raw=plain(input),scope=normalizeScope(raw.scope||raw),current=await getWorkspace(scope),workspace=current.workspace,summary=clean(raw.summary||raw.resultSummary,2400);if(!summary){const error=new Error("실행 결과 기록 내용이 없습니다.");error.statusCode=400;throw error;}const language=responseLanguage(summary,raw.language||raw.responseLanguage),messages=array(workspace.messages).slice(-79);messages.push({id:messageId(),role:"system",content:summary,createdAt:iso(),language:language.code});const next=Object.assign({},workspace,{messages,updatedAt:iso(),updatedBy:text(actorId)||"administrator"}),saved=await persistWorkspace(actorId,next);return{ok:true,version:VERSION,workspace:saved,logged:true};
}
...
module.exports={VERSION,CATEGORY_KEYS,CATEGORY_LABELS,normalizeScope,getWorkspace,discuss,saveDecision,deleteMessages,clearDecision,deleteWorkspace,promoteToPolicy,appendExecutionLog,effectivePolicy,mergeWithAutomaticWeights};
