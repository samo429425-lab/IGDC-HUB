...
const SUPPLIER_INSPECTION_LIMIT = 300;
const SUPPLIER_REVIEW_LIMIT = 300;
...
function researchScope(input) {
  ...
  return { country, region, regionGroup: countryInfo.regionGroup, countryName: countryInfo.nameKo || countryInfo.nameEn || country, scopeAuthority: lower(raw.scopeAuthority) === "administrator-selected" ? "administrator-selected" : "explicit-request", requestGeoIgnored: raw.ignoreRequestGeo === true || lower(raw.scopeAuthority) === "administrator-selected" };
}
...
function supplierSurfaceDisposition(item, blockedKeysInput) {
  ...
  const documentTitle=/(?:시행지침|지침서|규정|정책|공고|입찰|제도|보고서|연구자료|연구보고서|백서|연차보고서|지속가능(?:경영)?보고서|ESG(?:보고서)?|투자자\s*자료|IR\s*자료|목\s*차|매뉴얼|서식|양식|회의록|보도자료|사업계획서|논문|학술자료|자료실|다운로드|white\s*paper|annual\s*report|sustainability\s*report|research\s*report|investor\s*relations|press\s*release|publication)/i.test(title);
  const documentPath=/(?:\/(?:report|reports|research|white-?paper|press|news|media|publication|publications|investor|investors|ir|esg|sustainability|document|documents|archive|reference|download|downloads)(?:\/|$)|[?&](?:download|file|attachment)=)/i.test(parsed.pathname+parsed.search);
  const commerceCue=/(?:shop|store|mall|product|products|goods|item|catalog|category|cart|buy|order|판매|상품|제품|쇼핑|스토어|몰|구매|주문)/i.test((title||"")+" "+parsed.pathname);
  ...
  if(documentExtension||downloadSurface||documentTitle||publicDocumentHost||(documentPath&&!commerceCue)) return {state:"holding",reason:documentExtension?"document_file_link":(downloadSurface?"download_handler_link":((documentTitle||documentPath)?"document_or_report_page":"public_document_reference")),originalUrl,normalizedUrl:rootUrl,host,urlKey,hostKey};
...
function supplierResearchRoundCount(input){
...
async function beginResearchJob(actorId, input, event) {
  const raw=plain(input),scope=researchScope(raw),state=await configState(),effective=effectiveSetting(state,scope.country,scope.region==="NATIONWIDE"?"":scope.region),existing=await researchJobRule(scope),context=await researchContext(scope),researchRounds=supplierResearchRoundCount(raw);
  ...
  const researchSignature=sha256(JSON.stringify({
    ...
    effective:{mode:effective.mode,enabled:effective.enabled===true,intervalDays:Number(effective.intervalDays||0),maxCandidates:Number(effective.maxCandidates||DEFAULT_MAX_CANDIDATES),expandSubdivisions:effective.expandSubdivisions===true,researchRounds},
  ...
  const perRunCandidateLimit=Math.max(1,Math.min(50,Number(effective.maxCandidates||DEFAULT_MAX_CANDIDATES)));
  ...
  try {
    if (job.status === "searching") {
      ...
      if (!task) { job.inspectionPool = RegionalSelector.prepareSupplierInspectionPool(job.rawCandidates, Object.assign({}, selectorInput, { limit: Math.min(SUPPLIER_INSPECTION_LIMIT, Math.max(80, Number(job.effective && job.effective.maxCandidates || DEFAULT_MAX_CANDIDATES) * 4)) })); job.status = "inspecting"; job.inspectCursor = 0; }
        ...
        const result = await RegionalSelector.searchSupplierResearchStep(event || {}, Object.assign({}, selectorInput, { task, externalFallback, limit: Number(job.effective && job.effective.maxCandidates || DEFAULT_MAX_CANDIDATES), timeoutMs: 7000 }));
...
function prepareProductStagingPhase(job) {
  // Inspection completion now flows directly into the private candidate ledger.
  // The existing bounded staging engine is reused in small batches, so the
  // operator no longer has to manage a separate intermediary queue screen.
  ...
  job.productInspectionRetryPending = 0;
  job.trace = array(job.trace).concat([{ at: iso(), source: "product-research", status: "research_complete_candidate_sync_started", eligible: Number(plain(job.stageSummary).eligible || 0), automaticPrivateQueueWrite: true }]).slice(-240);
}
...
function productPolicyAffinity(rowInput,contextInput){
  const row=plain(rowInput),context=plain(contextInput),priority=policySearchTerms(array(context.manualPriorityTargets).concat(array(context.priorityDirections)).concat(text(context.finalDecision)?[context.finalDecision]:[])),avoid=policySearchTerms(array(context.avoidDirections)),blocked=policySearchTerms(array(context.manualBlockedTargets)),hay=lower([row.productName,row.title,row.sourceTitle,row.productUrl,row.url,row.supplierName,row.supplierOfficialUrl].map(text).join(" "));
  if(blocked.some((term)=>hay.includes(term)))return{blocked:true,score:-1000};
  let score=0;for(const term of priority)if(hay.includes(term))score+=12;for(const term of avoid)if(hay.includes(term))score-=6;
  return{blocked:false,score:Math.max(-120,Math.min(120,score))};
}
function orderProductDiscoveryByPolicy(rowsInput,contextInput){
  return array(rowsInput).map((row,index)=>({row,index,affinity:productPolicyAffinity(row,contextInput)})).filter((entry)=>!entry.affinity.blocked).sort((a,b)=>Number(b.affinity.score||0)-Number(a.affinity.score||0)||a.index-b.index).map((entry)=>Object.assign({},entry.row,{researchPolicyAffinity:Number(entry.affinity.score||0)}));
}
...
function productSupplierResearchCounts(job) {
  ...
  const explicitExisting=Number(job&&job.existingSupplierSourceCount),explicitGap=Number(job&&job.gapSupplierSourceCount),explicitNew=Number(job&&job.newSupplierSourceCount);
  const grouped=sources.some((row)=>text(row&&row.researchCycleGroup));
  const existingTotal=Number.isFinite(explicitExisting)?Math.max(0,explicitExisting):(grouped?sources.filter((row)=>["existing","gap"].includes(text(row&&row.researchCycleGroup))).length:Number(job&&job.priorResearchedSupplierCount||0));
  const gapTotal=Number.isFinite(explicitGap)?Math.max(0,explicitGap):(grouped?sources.filter((row)=>text(row&&row.researchCycleGroup)==="gap").length:0);
  ...
  return {
    existingTotal,gapTotal,newTotal,
    ...
    gapChecked:grouped?checked.filter((row)=>text(row&&row.researchCycleGroup)==="gap").length:0,
...
function publicProductJob(job) {
  ...
  return {
    ...
    researchCycle:{mode:"delta_gap_fill",preserveExisting:true,restartFromFirstSupplierEveryRun:false,supplierGroups:"known_preserved_gap_retry_new",newProductsOnlyInspection:true,duplicatePolicy:"skip known supplier rescan; retry zero-result/deferred suppliers; inspect newly discovered products; refresh same product by supplier id/canonical url/exact identity; append only truly new products",administratorDecisionPrecedence:true},
    ...
    summary: {
      ...
      gapSupplierSources: supplierResearch.gapTotal,
      gapSuppliersChecked: supplierResearch.gapChecked,
      skippedKnownSupplierSources: Number(job.skippedKnownSupplierSourceCount||0),
...
function productGapSupplierKeys(existing){
  const keys=new Set();
  if(!existing||existing.schema!==PRODUCT_JOB_SCHEMA)return keys;
  array(existing.temporarilyDeferredSupplierKeys).map(text).filter(Boolean).forEach((key)=>keys.add(key));
  array(existing.supplierRetryQueue).map(text).filter(Boolean).forEach((key)=>keys.add(key));
  array(existing.zeroProductSupplierKeys).map(text).filter(Boolean).forEach((key)=>keys.add(key));
  for(const row of array(existing.trace)){
    const supplierKey=text(row&&row.supplierKey);if(!supplierKey||text(row&&row.source)!=="supplier-product-discovery")continue;
    const count=Number(row&&row.count||row&&row.returned||row&&row.fallbackCount||0),status=lower(row&&row.status);
    if(status==="ok"&&count>0)keys.delete(supplierKey);
    else if((status==="ok"||status==="supplier_url_missing"||row&&row.retryable===true)&&count<=0)keys.add(supplierKey);
  }
  return keys;
}
...
async function beginProductResearchJob(actorId, input) {
  ...
  const priorResearchedKeys=researchedProductSupplierKeys(existing),priorGapKeys=productGapSupplierKeys(existing),supplierSourceFingerprint=productSupplierSourceFingerprint(allSupplierSources);
  const classifiedSupplierSources=allSupplierSources.map((source)=>{
    const key=productSupplierSourceKey(source),group=!priorResearchedKeys.has(key)?"new":(priorGapKeys.has(key)?"gap":"existing");
    return Object.assign({},source,{researchCycleGroup:group});
  });
  const existingSupplierSourceCount=classifiedSupplierSources.filter((source)=>source.researchCycleGroup!=="new").length;
  const gapSupplierSourceCount=classifiedSupplierSources.filter((source)=>source.researchCycleGroup==="gap").length;
  const newSupplierSourceCount=classifiedSupplierSources.filter((source)=>source.researchCycleGroup==="new").length;
  const skippedKnownSupplierSourceCount=Math.max(0,existingSupplierSourceCount-gapSupplierSourceCount);
  // Default product research is cumulative/delta based. Suppliers already
  // researched successfully are preserved without another full-site crawl.
  // Only newly added suppliers plus zero-result/deferred gap suppliers are
  // searched. A diagnostic full rescan is available only with fullRescan=true.
  const forceSourceRefresh=raw.fullRescan===true||raw.forceFullRescan===true;
  ...
  const supplierSources=forceSourceRefresh?classifiedSupplierSources:classifiedSupplierSources.filter((source)=>source.researchCycleGroup==="new"||source.researchCycleGroup==="gap");
  if(existing&&existing.schema===PRODUCT_JOB_SCHEMA&&!forceSourceRefresh&&existing.status==="complete"&&!supplierSources.length){
    const view=Object.assign({},existing,{version:VERSION,rankingVersion:ProductRanking.VERSION,supplierResearchJobId:supplierLedgerSourceId,currentSupplierSourceCount:allSupplierSources.length,existingSupplierSourceCount,gapSupplierSourceCount,skippedKnownSupplierSourceCount,newSupplierSourceCount,priorResearchedSupplierCount:existingSupplierSourceCount,supplierSourceFingerprint,researchedSupplierKeys:Array.from(priorResearchedKeys)});
...
async function advanceProductResearchJob(actorId, input) {
  ...
  if(job.status==="staging"){
    const stageResult=await stageCurrentProductResearchQueue(actorId,{countryCode:scope.country,subdivisionCode:scope.region,jobId:job.jobId,source:"research_auto",operation:"stage"}),part=plain(stageResult&&stageResult.partialQueue);
    job.stageSummary=Object.assign({},plain(job.stageSummary),{eligible:Number(part.eligible||plain(job.stageSummary).eligible||0),created:Number(part.created||0),updated:Number(part.updated||0),preserved:Number(part.preserved||0),skipped:Number(part.skipped||0),failed:Number(part.failed||0),remaining:Number(part.remaining||0)});
    if(part.complete===true||Number(part.remaining||0)===0){job.status="complete";job.finishedAt=iso();job.lastError=null;job.trace=array(job.trace).concat([{at:iso(),source:"product-research",status:"candidate_ledger_sync_complete",eligible:Number(part.eligible||0),automaticPrivateQueueWrite:true}]).slice(-240);await saveProductJob(job,actorId);}
    else{job.trace=array(job.trace).concat([{at:iso(),source:"product-research",status:"candidate_ledger_sync_progress",remaining:Number(part.remaining||0),attempted:Number(part.attempted||0),automaticPrivateQueueWrite:true}]).slice(-240);await saveProductJob(job,actorId);}
    const syncRuntime=await refreshPausedProductRuntime(scope,actorId,job,requestedJobId),response=productResearchStepResponse(job,input);response.candidateSync=part;
    return attachProductRuntime(response,syncRuntime);
  }
  ...
  try{
    if(job.status==="discovering"){
      ...
      if(!source){prepareProductInspectionPhase(job);job.status="inspecting";}
        ...
        if(retryable&&!discoveredItems.length&&researchedKey){const attempt=Number(retryCounts[researchedKey]||0)+1;retryCounts[researchedKey]=attempt;if(attempt<3){const queue=array(job.supplierRetryQueue);if(!queue.includes(researchedKey))queue.push(researchedKey);job.supplierRetryQueue=queue;}else{const deferred=array(job.temporarilyDeferredSupplierKeys);if(!deferred.includes(researchedKey))deferred.push(researchedKey);job.temporarilyDeferredSupplierKeys=deferred;}if(!array(job.zeroProductSupplierKeys).includes(researchedKey))job.zeroProductSupplierKeys=array(job.zeroProductSupplierKeys).concat([researchedKey]);}
        else if(researchedKey){if(!array(job.researchedSupplierKeys).includes(researchedKey))job.researchedSupplierKeys=array(job.researchedSupplierKeys).concat([researchedKey]);job.temporarilyDeferredSupplierKeys=array(job.temporarilyDeferredSupplierKeys).filter((key)=>key!==researchedKey);if(discoveredItems.length){job.zeroProductSupplierKeys=array(job.zeroProductSupplierKeys).filter((key)=>key!==researchedKey);}else if(!array(job.zeroProductSupplierKeys).includes(researchedKey)){job.zeroProductSupplierKeys=array(job.zeroProductSupplierKeys).concat([researchedKey]);}}
...
function productQueueIdentity(row){
  return ProductRanking.productIdentity(row)||text(row&&row.productIdentity)||text(row&&row.id)||productUrl(row);
}
...
async function registeredProductCandidateByUrl(product){
  const url=productUrl(product);if(!url)return null;
  try{
    const rows=await SlotStore.select("gslot_candidates","select=id,official_url,status,source_ref,source_payload&source_ref=eq."+encodeURIComponent(PRODUCT_SOURCE_REF)+"&official_url=eq."+encodeURIComponent(url)+"&limit=1");
    const row=array(rows)[0];return row&&text(row.source_ref)===PRODUCT_SOURCE_REF?row:null;
  }catch(_error){return null;}
}
async function durableProductCandidate(scope,product,candidateIdInput){
  const candidateId=text(candidateIdInput)||productCandidateId(scope,product);
  if(candidateId){
    try{
      const row=array(await SlotStore.select("gslot_candidates","select=id,official_url,status,source_ref,source_payload&id=eq."+encodeURIComponent(candidateId)+"&limit=1"))[0];
      if(row&&text(row.source_ref)===PRODUCT_SOURCE_REF)return row;
    }catch(_error){}
  }
...
function latestProductValidationDisposition(productInput){
  const product=plain(productInput),readiness=ProductPipeline.researchReadiness(product),risk=plain(product.riskAssessment),status=lower(product.researchStatus),blockers=array(readiness.blockers).map(lower),reviewGaps=array(readiness.reviewGaps).map(lower);
  const structuralBlockers=new Set(["specific_product_detail_url_missing","template_or_placeholder_url","supplier_identity_missing","supplier_product_domain_mismatch"]);
  const structuralInvalid=!productUrl(product)||risk.specificProductUrl===false||product.sameSupplierSite===false||blockers.some((code)=>structuralBlockers.has(code))||["http_404","non_html"].includes(status);
  const decision=structuralInvalid?"reject":"hold";
  const reason=structuralInvalid?"invalid_product_reference":((product.productPageLive===false||risk.explicitUnavailable===true||blockers.includes("product_page_unavailable"))?"product_page_temporarily_unavailable":"validation_incomplete");
  return{decision,reason,blockers,reviewGaps,status:status||null};
}
async function persistLatestProductValidationDisposition(actorId,scope,productInput,dispositionInput){
  const product=plain(productInput),disposition=plain(dispositionInput),decision=disposition.decision==="reject"?"reject":"hold",candidateId=productCandidateId(scope,product),now=iso();
  if(!candidateId)return{status:"failed",decision,error:"missing_product_identity"};
  let existing=array(await SlotStore.select("gslot_candidates","select=id,status,source_ref,source_payload,official_url,thumbnail_url,title&id=eq."+encodeURIComponent(candidateId)+"&limit=1"))[0];
  if(!existing){const currentUrl=productUrl(product);if(currentUrl)existing=await registeredProductCandidateByUrl(product);}
  if(existing&&text(existing.source_ref)!==PRODUCT_SOURCE_REF)return{status:"protected",decision,candidateId:text(existing.id)||candidateId,reason:"existing_non_product_candidate_preserved"};
  const prior=plain(existing&&existing.source_payload),priorDecision=lower(prior.slotDecision),priorControl=plain(prior.managementControl),priorQueue=plain(prior.queueControl),front=plain(prior.frontPublication),frontStatus=lower(front.status);
  const protectedState=priorQueue.permanentExcluded===true||priorControl.administratorLocked===true||lower(priorControl.source)==="administrator"||priorDecision==="slot_candidate"||["queued","publish_requested","matched","published","active","unpublish_requested"].includes(frontStatus);
  if(protectedState)return{status:"protected",decision:priorDecision||decision,candidateId:text(existing&&existing.id)||candidateId,reason:"existing_downstream_or_administrator_state_preserved"};
  const base=productCandidatePayload(actorId,scope,product,"research_pending"),payload=Object.assign({},prior,base),finalId=text(existing&&existing.id)||candidateId;
  payload.id=finalId;payload.slotDecision=decision;payload.approvedPlacement=null;payload.selectedPlacement=null;payload.placement=null;payload.publicPublication=false;payload.automaticImport=false;
  payload.queueControl=Object.assign({},priorQueue,{schema:"igdc-private-product-queue-control.v1",action:decision,hiddenFromCountryQueue:true,permanentExcluded:false,rediscoveryAllowed:true,automaticValidationDisposition:true,validationReason:text(disposition.reason),validationBlockers:array(disposition.blockers),validationReviewGaps:array(disposition.reviewGaps),decidedAt:now,decidedBy:text(actorId)||"administrator"});
  payload.review=Object.assign({},plain(prior.review),{state:decision==="reject"?"validation_rejected":"validation_hold",decidedAt:now,decidedBy:text(actorId)||"administrator",reason:text(disposition.reason)});
  payload.managementControl=Object.assign({},priorControl,{schema:"igdc-product-management-control.v1",source:"research_validation",administratorLocked:false,aiReclassificationAllowed:true,updatedAt:now,updatedBy:text(actorId)||"administrator"});
  payload.pipeline=Object.assign({},plain(payload.pipeline),{stage:decision==="reject"?"rejected":"held",nextGate:decision==="reject"?"new_valid_product_reference":"administrator_reconsideration",validationDisposition:true,updatedAt:now});
  delete payload.page;delete payload.channel;delete payload.section;delete payload.psom_key;delete payload.slot;
  try{await SlotStore.remove("gslot_slot_assignments","candidate_id=eq."+encodeURIComponent(finalId));}catch(_assignmentError){}
  const row={id:finalId,kind:"product",title:text(payload.title)||text(product.productName||product.title)||"상품 검증 대기",official_url:safeUrl(payload.externalProductUrl)||null,status:decision==="reject"?"suppressed":"hold",source_ref:PRODUCT_SOURCE_REF,thumbnail_url:safeUrl(payload.image)||null,description:"Latest product research validation disposition.",owner_note:decision==="reject"?"최신 상품 검증에서 실제 상품 상세 URL 조건을 통과하지 못해 제외 목록으로 이동했습니다. 다음 리서치에서 유효 상품 URL이 다시 발견되면 재검토할 수 있습니다.":"최신 상품 검증이 일시적으로 완료되지 않아 보류 목록으로 이동했습니다. 다음 리서치 또는 관리자 재검토에서 다시 확인할 수 있습니다.",source_payload:storageSafeJsonValue(payload,0),updated_at:now};
  if(existing)await SlotStore.update("gslot_candidates","id=eq."+encodeURIComponent(finalId),row);else{row.created_at=now;row.created_by=text(actorId)||"administrator";await SlotStore.insert("gslot_candidates",row,"resolution=merge-duplicates,return=minimal");}
...
async function stageCurrentProductResearchQueueChunked(actorId,input,job,runtime,scope,operation){
  ...
  if(operation==="delete"){
    ...
    // Deleting from the latest-research view is a list-management action. Always
  ...
  // background inspection reached it. Re-inspect only the selected <=4 rows in this
  // request, then keep the normal private-queue safety gate. Invalid/list/document
  // URLs still remain blocked and can never be promoted by this path.
  if(hasSelection&&manualReviewQueue&&plain(input).reinspectSelected===true&&selectedRows.length){
    const pendingIndexes=[],pendingRows=[];
    selectedRows.forEach((row,index)=>{if(!queueAccepts(row)){pendingIndexes.push(index);pendingRows.push(row);}});
    if(pendingRows.length){
      const checked=await RegionalSelector.inspectProductResearchStep(pendingRows.slice(0,4),{country:scope.country,region:scope.region,timeoutMs:6000}),checkedRows=array(checked&&checked.items);
      for(let i=0;i<checkedRows.length&&i<pendingIndexes.length;i++){
        const at=pendingIndexes[i],prior=selectedRows[at],inspected=checkedRows[i];
        selectedRows[at]=compactProductWorkingRow(Object.assign({},prior,inspected,{researchResultIndex:Number(prior&&prior.researchResultIndex),researchCycleJobId:text(prior&&prior.researchCycleJobId)||text(job.jobId)}));
      }
    }
  }
  ...
  const selectedValidationBlockedRows=hasSelection?selectedRows.filter((row)=>!queueAccepts(row)):[],validationRoute=hasSelection&&manualReviewQueue&&selectedValidationBlockedRows.length?await routeBlockedLatestProducts(actorId,scope,selectedValidationBlockedRows):{attempted:0,routedHold:0,routedReject:0,protectedPreserved:0,failed:0,routed:0,unresolved:selectedValidationBlockedRows.length,details:[]};
  /* A completed one-way cursor can become stale when inspection/ranking makes
     ...
     says there should still be unstaged eligible rows.  Explicit latest-list
     registration does not use this cursor at all. */
  if(!hasSelection&&scanCursor>=total&&total>0)scanCursor=0;
  if(hasSelection){
    for(const product of selectedRows){const identity=productQueueIdentity(product);if(!identity||handled.has(identity))continue;if(queueAccepts(product))batch.push({identity,product,absoluteIndex:Number(product.researchResultIndex)});if(batch.length>=PRODUCT_PARTIAL_STAGE_BATCH)break;}
  }else{
...
async function stageCurrentProductResearchQueue(actorId,input){
  ...
  const durablePaused=runtime.pauseRequested===true&&(!text(runtime.jobId)||text(runtime.jobId)===text(job.jobId)),explicitSelection=array(input&&input.productIds).map(text).filter(Boolean).length>0||array(input&&input.productRefs).length>0;
  if(operation==="stage"&&job.status!=="complete"&&!durablePaused&&!explicitSelection){const error=new Error("상품 리서치가 진행 중입니다. 전체 조사분 일괄 등록은 리서치 완료 또는 서버 일시정지 후 실행하세요. 화면에서 선택한 최신 상품은 진행 중에도 개별 대기열 등록할 수 있습니다.");error.statusCode=409;error.code="PRODUCT_RESEARCH_QUEUE_PHASE_NOT_READY";throw error;}
...
async function stageCurrentProductResearchQueueLegacy(actorId, input) {
  ...
  let requestedRows = requestedIds.size ? portfolioRows.filter((row) => productSelectionMatches(row, requestedIds)) : portfolioRows;
  ...
  if (operation === "unmatch") {
    ...
    const partialQueue = {
      ...
      researchCursorPreserved: true, latestResearchRowsPreserved: true, automaticCandidateLedgerSync: lower(input&&input.source)==="research_auto",
  ...
  if(requestedIds.size&&manualReviewQueue&&plain(input).reinspectSelected===true&&requestedRows.length){
    const checked=await RegionalSelector.inspectProductResearchStep(requestedRows.slice(0,4),{country:scope.country,region:scope.region,timeoutMs:6000}),checkedRows=array(checked&&checked.items);
    requestedRows=requestedRows.map((prior,index)=>index<checkedRows.length?Object.assign({},prior,checkedRows[index]):prior);
  }
  ...
  const blockedLegacyRows=requestedIds.size&&manualReviewQueue?requestedRows.filter((row)=>!eligibleAll.includes(row)):[],legacyValidationRoute=blockedLegacyRows.length?await routeBlockedLatestProducts(actorId,scope,blockedLegacyRows):{attempted:0,routedHold:0,routedReject:0,protectedPreserved:0,failed:0,routed:0,unresolved:blockedLegacyRows.length,details:[]};
  selectionReconcile=await reconcileExplicitPrivateQueueSelection(scope,eligibleAll,handled,job.rankingContext);
  const eligibleRows=array(selectionReconcile.products).filter((row)=>row&&row.inspectionComplete===true&&(ProductPipeline.researchReadiness(row).queueEligible===true||(manualReviewQueue&&administratorReviewQueueEligible(row))));
  const eligibleByIdentity = new Map();
  for (const row of eligibleRows) { const identity = productQueueIdentity(row); if (identity && !eligibleByIdentity.has(identity)) eligibleByIdentity.set(identity, row); }

  // Historical job completion/stage cursors are not proof of a durable private
  // candidate row. Only reconcileExplicitPrivateQueueSelection() above may mark
  ...
  if (batch.length) {
    ...
    const evaluatedByIdentity=new Map(array(evaluatedBatch).map((row)=>[productQueueIdentity(row),row]));
    ...
    for (let offset = 0; offset < rankedBatch.length; offset += PRODUCT_PARTIAL_STAGE_CONCURRENCY) {
      ...
      const settled = await Promise.allSettled(group.map(([, product]) => syncProductResearchPreview(actorId, scope, product, {allowAdministratorReviewQueue:manualReviewQueue})));
  ...
  const partialQueue = {
    ...
    validationBlocked:blockedLegacyRows.length,
    routedHold:Number(legacyValidationRoute.routedHold||0),
    routedReject:Number(legacyValidationRoute.routedReject||0),
    protectedPreserved:Number(legacyValidationRoute.protectedPreserved||0),
    blocked,
...
function productCandidateId(scope, product) {
  ...
  const identity=productQueueIdentity(product);
  return identity?"country_product_"+sha256(scope.country+"|"+scope.region+"|"+identity).slice(0,24):"";
}
...
function administratorReviewQueueEligible(productInput) {
  const product=plain(productInput),risk=plain(product.riskAssessment),url=productUrl(product);
  if(product.inspectionComplete!==true||!url)return false;
  if(product.productPageLive===false||risk.explicitUnavailable===true||risk.specificProductUrl===false)return false;
  return true;
}
async function syncProductResearchPreview(actorId, scope, productInput, optionsInput) {
  const options=plain(optionsInput),readiness = ProductPipeline.researchReadiness(productInput),manualReviewQueue=options.allowAdministratorReviewQueue===true;
  if (!readiness.queueEligible && !(manualReviewQueue&&administratorReviewQueueEligible(productInput))) return { status: "research_preview_skipped", candidateId: null, registered:false, blockers: readiness.blockers };
  ...
  if (existing && text(existing.source_ref) !== PRODUCT_SOURCE_REF) return { status: "existing_non_auto_candidate_preserved", candidateId, registered:false, currentStatus: text(existing.status) };
  ...
  if(existing&&(["slot_candidate","hold","reject","purge"].includes(operatorDecision)||!["approval_pending"].includes(lower(existing.status)))){
    ...
    const durable=await durableProductCandidate(scope,product,candidateId);if(!durable)throw new Error("PRIVATE_PRODUCT_QUEUE_DURABLE_VERIFY_FAILED");candidateId=text(durable.id)||candidateId;
    return { status: "operator_state_preserved_content_updated", candidateId, registered:true, currentStatus: text(existing.status), decision: operatorDecision||null, approvedPlacement: plain(existingPayload.approvedPlacement) };
  }
  ...
  if (existing) { await SlotStore.update("gslot_candidates", "id=eq." + encodeURIComponent(candidateId), row); const durable=await durableProductCandidate(scope,product,candidateId);if(!durable)throw new Error("PRIVATE_PRODUCT_QUEUE_DURABLE_VERIFY_FAILED");candidateId=text(durable.id)||candidateId;return { status: "research_preview_updated", candidateId, registered:true, decision: "undecided", approvedPlacement: null }; }
  row.created_at = iso(); row.created_by = text(actorId) || "product-research-orchestrator"; await SlotStore.insert("gslot_candidates", row, "return=minimal"); const durable=await durableProductCandidate(scope,product,candidateId);if(!durable)throw new Error("PRIVATE_PRODUCT_QUEUE_DURABLE_VERIFY_FAILED");candidateId=text(durable.id)||candidateId;return { status: "research_preview_created", candidateId, registered:true, decision: "undecided", approvedPlacement: null };
}
...
async function productCandidateAction(actorId, input) {
  ...
  const id = text(input && input.productId), decision = lower(input && input.decision); if (!id || !["slot_candidate","hold","reject","purge","undecided","ai_reclassify","remove_from_list","dismiss","affiliate_settlement"].includes(decision)) { const error = new Error("상품 후보 ID와 관리자 판정을 확인하세요."); error.statusCode = 400; throw error; }
  ...
  if (decision === "dismiss") {
    const current=plain(job.products[index]),candidateId=productCandidateId(scope,current),now=iso(),relationTables=["gslot_slot_assignments","gslot_candidate_availability","gslot_candidate_revenue","gslot_candidate_evidence"],relationCleanup=[];
    for(const table of relationTables){try{const removed=await SlotStore.remove(table,"candidate_id=eq."+encodeURIComponent(candidateId));relationCleanup.push({table,ok:true,count:array(removed).length});}catch(error){relationCleanup.push({table,ok:false,error:text(error&&error.message)||"delete_failed"});}}
    const relationFailures=relationCleanup.filter((item)=>item&&item.ok!==true);
    if(relationFailures.length){const cleanupError=new Error("상품 후보 관계 원장 삭제를 완료하지 못했습니다. 후보 삭제를 중단했습니다.");cleanupError.statusCode=409;cleanupError.code="candidate_relation_cleanup_failed";cleanupError.relationCleanup=relationCleanup;throw cleanupError;}
    try{await SlotStore.remove("gslot_candidates","id=eq."+encodeURIComponent(candidateId));}catch(error){const deleteError=new Error("상품 후보 원장 삭제를 완료하지 못했습니다. 다시 시도해 주세요.");deleteError.statusCode=409;deleteError.code="candidate_delete_failed";deleteError.cause=text(error&&error.message)||"delete_failed";throw deleteError;}
    const currentIdentity=ProductRanking.productIdentity(current);
    job.products=array(job.products).filter((row,position)=>position!==index&&ProductRanking.productIdentity(row)!==currentIdentity);
    job.rawProducts=array(job.rawProducts).filter((row)=>text(row&&row.id)!==id&&ProductRanking.productIdentity(row)!==currentIdentity);
    job.trace=array(job.trace).concat([{at:now,source:"product-management-control",status:"dismiss",productId:id,candidateId,rediscoveryAllowed:true,publicPublication:false,actor:text(actorId)||"administrator"}]).slice(-240);
    await saveProductJob(job,actorId);
    const result=publicProductJob(job);result.actionResult={productId:id,candidateId,decision:"dismiss",effectiveDecision:"deleted",deleted:true,rediscoveryAllowed:true,relationCleanup,publicPublication:false,paymentExecution:false};return result;
  }
...
async function productCandidateLedgerAction(actorId, input) {
  ...
  if (decision === "slot_candidate") {
    ...
    payload.slotDecision = "removed"; status = "removed"; releaseAssignment = true; effectiveDecision = "removed";
...
async function productCandidateLedgerBulkAction(actorId, input) {
  ...
  if (!ids.length) { const error = new Error("관리할 상품 후보를 선택하세요."); error.statusCode = 400; throw error; }
  const results = [], failures = [];
  // Persist each exact administrator decision without reloading the whole product
  // board between items. The browser refreshes the candidate ledger only once at
  // the end, so multi-select manual placement/hold/reject cannot lose its remaining
  // selection after the first successful item.
  for (let offset = 0; offset < ids.length; offset += 6) {
    const batch = ids.slice(offset, offset + 6);
    const settled = await Promise.allSettled(batch.map((candidateId) => productCandidateLedgerAction(actorId, { countryCode:scope.country, subdivisionCode:scope.region, candidateId, decision, placementKey })));
    settled.forEach((entry, index) => {
      const candidateId = batch[index];
      if (entry.status === "fulfilled") results.push(entry.value.actionResult || { candidateId, decision, ok:true });
      else failures.push({ candidateId, error:text(entry.reason && entry.reason.message || entry.reason) });
    });
  }
  return { ok:true, complete:failures.length===0, candidateLedger:true, requested:ids.length, processed:results.length, failed:failures.length, decision, placementKey:placementKey||null, results, failures, publicPublication:false, paymentExecution:false };
}
...
async function productCandidateAiRecover(actorId, input) {
  ...
  const result = await revalidateCandidateLedgerRows(actorId, input, ids, {
    ...
    reuseFreshValidation:true, freshValidationMinutes:720,
...
async function runScope(options) {
  ...
  try {
    ...
    const selection = await RegionalSelector.runSelection(opts.event || {}, {
      country, region: region === "NATIONWIDE" ? undefined : region, scopeAuthority: "administrator-selected", ignoreRequestGeo: true, privateCollection: true, privateLimit: maxCandidates, maxCandidates,
...
async function persistSupplierResearchDecision(scope,targetRow,action,actorId,options){
  ...
  let rows=[];if(Array.isArray(options.rows))rows=options.rows;else try{rows=array(await supplierScopedCandidateRows(scope,"id,status,source_ref,official_url,source_payload","",1000));}catch(error){return{updated:0,error:text(error&&error.message)||"supplier_ledger_read_failed"};}
  let updated=0;const now=iso(),actor=text(actorId)||"administrator";
  for(const entry of rows){
    if(entry&&entry.__deleted===true)continue;
    ...
    if(matchHost?(!!targetHost&&rowHost===targetHost):(!!exactRoot&&rowRoot===exactRoot)){
      if(action==="dismiss"){await SlotStore.remove("gslot_candidates","id=eq."+encodeURIComponent(text(entry.id)));entry.__deleted=true;updated+=1;continue;}
      let status=lower(entry&&entry.status)||"approval_pending";
      if(action==="hold"||action==="unblock")status="hold";
      else if(action==="restore"||action==="keep")status="approval_pending";
      else if(action==="purge"||action==="block")status="suppressed";
      else if(action==="remove_from_list")status="removed";
      payload.aiAutomation=Object.assign({},automation,{operatorDecision:action,operatorDecisionAt:now,operatorDecisionBy:actor,publicPublication:false,productImport:false,hiddenFromCountryQueue:action==="remove_from_list",rediscoveryAllowed:action==="remove_from_list"});
...
async function researchCandidateAction(actorId,input){
  ...
  for(const targetUrl of requested){
    ...
    let durable={updated:0},durableError=durableReadError;
    if(!durableError){try{durable=await persistSupplierResearchDecision(scope,target,action,actorId,{matchHost:action==="block",rows:durableRows});if(durable&&durable.error)durableError=text(durable.error);}catch(error){durableError=text(error&&error.message)||"supplier_ledger_write_failed";}}
    results.push({url:targetUrl,host,status:durableError?"durable_failed":action,requestedAction:action,durableUpdated:Number(durable&&durable.updated||0),durableError:durableError||null});
  }
  ...
  const result=publicResearchJob(job);result.candidateAction={action,requested:requested.length,processed:results.filter((row)=>row.status!=="not_found"&&row.status!=="durable_failed").length,notFound:results.filter((row)=>row.status==="not_found").length,durableFailed:results.filter((row)=>row.status==="durable_failed").length,results,blockedKeyCount:job.blockedSupplierKeys.length,manualRegistryUpdated:manualChanged,publicPublication:false,productImport:false};return result;
}
