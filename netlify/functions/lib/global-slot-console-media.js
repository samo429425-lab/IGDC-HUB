...
async function queueJob(actor, body) {
  ...
  const row = { id: sb.id('media_job'), candidate_id: candidateId, asset_id: assetId, job_type: jobType, status: 'queued', request: body.request && typeof body.request === 'object' ? body.request : {}, result: {}, requested_by: actor.sub, created_at: now(), updated_at: now() };
  await sb.insert('gslot_media_jobs', row, 'return=minimal');
  const hook = clean(process.env.GSLOT_MEDIA_WORKER_HOOK_URL);
  if (hook) {
    try {
      const res = await fetch(hook, { method: 'POST', headers: { 'Content-Type':'application/json' }, body: JSON.stringify({ jobId: row.id, candidateId, jobType }) });
      const status = res.ok ? 'queued' : 'failed';
      await sb.update('gslot_media_jobs', 'id=eq.' + encodeURIComponent(row.id), { status, updated_at: now(), result: { workerHook: res.status } }, 'return=minimal');
      row.status = status;
    } catch (error) {
      await sb.update('gslot_media_jobs', 'id=eq.' + encodeURIComponent(row.id), { status:'failed', updated_at:now(), result:{ workerHookError: String(error.message || error).slice(0, 500) } }, 'return=minimal');
      row.status = 'failed';
    }
  }
  await audit(actor, 'queue_media_job', 'media_job', row.id, { candidateId, jobType, workerHookConfigured: !!hook });
  return { ok: true, row, note: hook ? '외부 처리 워커에 작업을 전달했습니다.' : '처리 작업을 대기열에 기록했습니다. 대형 변환·AI 처리는 추후 전용 워커를 연결해 처리합니다.' };
}
