...
(function (global) {
  ...
  function hostInList(host, list) {
    for (var i=0; i<list.length; i++) if (hostMatches(host, list[i])) return true;
    return false;
  }
  ...
  function framePolicy(target) {
    ...
    policyInflight[key] = new Promise(function (resolve) {
      ...
      var timer = global.setTimeout(function () {
        ...
        if (!settled) {
          settled = true;
          resolve({ ok:false, directAllowed:true, reason:'check-timeout-direct-kept' });
        }
      }, POLICY_TIMEOUT_MS);
      ...
      fetch(frameCheckUrl(target), {
        ...
        return r.json().catch(function () { return null; }).then(function (json) {
          ...
          if (!r.ok || !json || json.ok === false) {
            resolve({ ok:false, directAllowed:(json && json.directAllowed !== false), reason:(json && json.reason) || 'check-failed' });
            return;
          }
          var policy = {
            ok:true,
        ...
        settled = true;
        clearTimeout(timer);
        resolve({ ok:false, directAllowed:true, reason:'check-error-direct-kept' });
      });
  ...
  function loadTarget(target, options) {
    ...
    var compat = compatibilityMode(target, state.kind);
    if (options.forceProxy || preferProxy(target)) {
      setFrameProxy(target, options.proxyMode || 'live');
      return;
    }
    if (compat === 'live') {
      setFrameProxy(target, 'live');
      return;
    }
    if (compat === 'direct') {
    ...
    if (cached) {
      ...
      else setFrameProxy(target, 'live');
      return;
    }
    ...
    framePolicy(target).then(function (policy) {
      ...
      showLoading('사이트 호환 모드로 전환하는 중입니다…');
      setFrameProxy(target, 'live');
    });
  }
