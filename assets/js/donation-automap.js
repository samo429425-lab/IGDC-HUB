...
function groupBySection(items){
  ...
  function seedOrdinal(it){
    // Builder seed UIDs may end in a hash, so never use UID before the visible seed title.
    // The front contract is the visible placeholder number: 1,2,3...100.
    const visible = [it?.title, it?.org?.name];
    ...
    // Static seed snapshot UIDs use a zero-padded terminal ordinal.
