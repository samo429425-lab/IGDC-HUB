...
(function(){
  ...
  async function runLatestProductQueueAction(operation,scopeMode,options){
    ...
    latestQueueActionActive=true;syncLatestProductQueueControls();var totalHandled=0,totalFailed=0,totalBlocked=0,totalEligible=0,deletedUiKeys={},state=$('productQueueStageState'),requestedScope=scopeSnapshot(),requestedKey=scopeKey(requestedScope);
    try{
      if(unmatch||deleting){
        for(var off=0;off<selectedRows.length;off+=100){
          ...
          totalHandled+=Number(part.handled||0);totalFailed+=Number(part.failed||0);totalBlocked+=Number(part.blocked||0);totalEligible+=Number(part.eligible||0);if(deleting&&result&&result.ok!==false)chunkIds.forEach(function(id){deletedUiKeys[id]=true;});
