/* IGDC Global/Region/Country Commerce Control v3.11.2-human-checkbox-hit-targets
...
(function(){
  ...
  async function applyProductExclusionBulk(kind,decision){
    ...
    var labels={reject:'제외 관리로 이동',purge:'영구 제외',dismiss:'관리목록 삭제·재수집 허용',remove_from_list:'현재 관리 목록에서만 제거',undecided:'후보 목록으로 복원'};
    ...
    try{var selectedRows=ids.map(productById).filter(Boolean);await unmatchRowsBeforeManagementChange(selectedRows);var result=await managementCandidateBulk(ids,decision,'');var summary='요청 '+ids.length+'건 · 성공 '+Number(result.processed||0)+'건 · 실패 '+Number(result.failed||0)+'건';var suffix=decision==='purge'?' · 영구 제외·재수집 차단을 저장했습니다.':decision==='dismiss'?' · 현재 관리 목록에서 제거하고 공유 원장·증빙은 보존했습니다. 이후 리서치에서 재발견될 수 있습니다.':decision==='remove_from_list'?' · 원장을 보존하고 현재 관리 목록에서 숨겼습니다.':' · 관리자 후보 상태를 변경했습니다. 프론트 공개는 실행하지 않았습니다.';show(summary+suffix,result.failed?'warn':'ok');renderExcludedLedger();}
  ...
  async function managementCandidateBulk(ids,decision,placementKey){
    ...
    /* Candidate-ledger management has one write authority.  The previous split
       (restore -> ADMIN_AUTHORITY, delete -> QUEUE_CONTROL, hold/reject/purge ->
       CONTROL) let the same row be interpreted by three different state layers;
       a refresh could therefore make purge/delete look as though they never ran. */
    for(var off=0;off<ledger.length;off+=100){var chunk=ledger.slice(off,off+100).map(function(row){return text(row.candidateId||row.id);}).filter(Boolean);try{var source=decision==='undecided'?'administrator_restore':decision==='dismiss'?'administrator_delete_rediscovery':'administrator';var result=await authorityCandidateAction(decision,chunk,placementKey||'',{source:source});processed+=Number(result&&result.processed||0);failed+=Number(result&&result.failed||result&&result.failures&&result.failures.length||0);(result&&result.failures||[]).slice(0,20).forEach(function(f){failures.push(f);});}catch(error){failed+=chunk.length;failures.push({error:text(error&&error.message)||'bulk_failed'});}}
    var last=null;for(var i=0;i<legacy.length;i++){try{last=await api(CONTROL,'product_candidate_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',productId:text(legacy[i].id),decision:decision,placementKey:placementKey||''},45000);processed+=1;}catch(error){failed+=1;failures.push({id:text(legacy[i].id),error:text(error&&error.message)||'item_failed'});}}
