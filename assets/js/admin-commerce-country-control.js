...
(function(){
  ...
  function pendingProductManagementChanges(mode,sectionKey,productId,selection){
    ...
    Object.keys(productPlacementDirty||{}).forEach(function(id){
      if(productPlacementDirty[id]!==true)return;
      var row=productById(id),value=text(productPlacementSelections[id]);
      if(!row||!value||value.indexOf('affiliate:')===0)return;
      ...
      changes.push({id:text(id),candidateId:text(row&&row.candidateId||row&&row.id),row:row,decision:decision,placementKey:decision==='slot_candidate'?desiredKey:'',currentKey:currentKey,desiredKey:desiredKey});
    });
    return changes;
  }
  async function persistPendingProductManagementChanges(changes){
    changes=Array.isArray(changes)?changes:[];
    if(!changes.length)return{changedCount:0,changedCandidateIds:[],placementProductIds:[],placementCandidateIds:[]};
    for(var checkIndex=0;checkIndex<changes.length;checkIndex++){
      var check=changes[checkIndex];
      if(check.decision==='purge'&&check.row&&frontPublicationActive(check.row)){var blockedError=new Error('현재 프론트에 매칭된 상품은 먼저 매칭 해제한 뒤 영구 제외할 수 있습니다.');blockedError.status=409;throw blockedError;}
    }
    var groups={},changedCandidateIds=[],placementProductIds=[],placementCandidateIds=[],successfulDirtyIds=[];
    changes.forEach(function(change){var key=change.decision+'|'+(change.placementKey||'');if(!groups[key])groups[key]={decision:change.decision,placementKey:change.placementKey||'',changes:[]};groups[key].changes.push(change);});
    var groupKeys=Object.keys(groups);
    for(var g=0;g<groupKeys.length;g++){
      var group=groups[groupKeys[g]],ledger=group.changes.filter(function(change){return change.row&&change.row.ledgerSource==='candidate';}),legacy=group.changes.filter(function(change){return !change.row||change.row.ledgerSource!=='candidate';});
      for(var off=0;off<ledger.length;off+=100){
        var ledgerChunk=ledger.slice(off,off+100),candidateIds=ledgerChunk.map(function(change){return text(change.candidateId);}).filter(Boolean);
        ...
        if(Number(result&&result.failed||0)>0){var bulkError=new Error('관리자 수정사항 저장 중 '+Number(result.failed||0)+'건이 실패했습니다. 프론트 실행을 중단하고 저장되지 않은 수정사항을 유지합니다.');bulkError.status=409;bulkError.failures=result&&result.failures||[];throw bulkError;}
        ledgerChunk.forEach(function(change){successfulDirtyIds.push(change.id);if(change.candidateId)changedCandidateIds.push(change.candidateId);if(change.decision==='slot_candidate'){placementProductIds.push(change.id);if(change.candidateId)placementCandidateIds.push(change.candidateId);}});
      }
      for(var l=0;l<legacy.length;l++){
        ...
        successfulDirtyIds.push(change.id);if(change.candidateId)changedCandidateIds.push(change.candidateId);if(change.decision==='slot_candidate'){placementProductIds.push(change.id);if(change.candidateId)placementCandidateIds.push(change.candidateId);}
      }
    }
    successfulDirtyIds.forEach(function(id){delete productPlacementDirty[id];});
    saveReviewSnapshot();
    await refreshCandidateLedgerProducts({preserveResearchReport:true});
    return{changedCount:successfulDirtyIds.length,changedCandidateIds:Array.from(new Set(changedCandidateIds)),placementProductIds:Array.from(new Set(placementProductIds)),placementCandidateIds:Array.from(new Set(placementCandidateIds))};
  }
  async function runProductFrontSync(operation,mode,sectionKey,productId,selection){
    ...
    if(!targetIds.length&&!pendingChanges.length){show(unmatch?'선택 범위에 매칭 해제할 상품이 없습니다.':'선택 범위에 최종 재검증·프론트 적용할 상품이 없습니다.','warn');return;}
    var explanation=unmatch?label+'의 기존 프론트 매칭 해제를 요청합니다.':label+'의 현재 배치 상품을 프론트 공개 원장에 적용합니다.'+(pendingChanges.length?' 화면에서 수정한 '+pendingChanges.length+'건을 먼저 관리자 원장에 확정 저장한 뒤 최신 원장 기준으로 실행합니다.':'')+' 최근 검증이 살아 있는 상품은 저장된 검증을 재사용하고, 오래됐거나 실제 공개 중인 상품만 다시 확인합니다. 삭제·잘못된 접근·판매처 홈 리다이렉트 상품은 제거하고 대체 후보가 없으면 기존 샘플 fallback을 유지합니다.';
    if(!window.confirm(explanation+'\n\n'+(unmatch?'프론트에서 해당 매칭을 해제하시겠습니까?\n변경은 소량 묶음으로 저장한 뒤 마지막에 스냅샷 빌드를 1회만 실행합니다.':'프론트 실상품 매칭 절차를 실행하시겠습니까?\n화면 수정사항을 먼저 원장에 확정하고 최신 원장을 다시 읽은 뒤, 마지막 빌드 1회로 프론트에 반영합니다.')))return;
    productFrontSyncActive=true;
    var state=$('productFrontSyncState'),batchSize=12,maxConcurrent=3,requestTimeout=75000,aggregate={requested:0,queued:0,persisted:0,pendingBuild:0,blocked:0,revalidated:0,remoteChecked:0,freshReused:0,invalid:0,inconclusive:0,withdrawn:0,changedSection:0,items:[],persistedCandidateIds:[],changedCandidateIds:[],preparation:{requested:0,prepared:0,blocked:0},batchErrors:[]},publishPersistedMap={},changedMap={},completed=0,pendingCommit={changedCount:0,changedCandidateIds:[],placementProductIds:[],placementCandidateIds:[]};
    ...
    try{
      if(pendingChanges.length){
        if(state){state.className='front-sync-state running';state.textContent=label+' · 화면 수정 '+pendingChanges.length+'건을 관리자 원장에 먼저 저장하는 중';}
        pendingCommit=await persistPendingProductManagementChanges(pendingChanges);
      ...
      if(needsFinalize){
        try{
          ...
          var finalizeOperation=unmatch?'unmatch':(aggregate.persistedCandidateIds.length?'match':'refresh'),finalizeIds=aggregate.persistedCandidateIds.length?aggregate.persistedCandidateIds:Array.from(new Set(aggregate.changedCandidateIds.concat(pendingCommit.changedCandidateIds||[])));
