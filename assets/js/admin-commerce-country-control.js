...
(function(){
  ...
  async function applyProductExclusionBulk(kind,decision){
    ...
    var labels={reject:'제외 관리로 이동',purge:'영구 제외',dismiss:'삭제·재수집 허용',remove_from_list:'현재 관리 목록에서만 제거',undecided:'전체 상품 후보·배치 관리 목록으로 복원'};
    if(['reject','purge','dismiss','remove_from_list'].indexOf(decision)>=0&&!window.confirm('선택 '+ids.length+'건을 '+(labels[decision]||decision)+' 처리하시겠습니까?'+(decision==='dismiss'?'\n\n공유 후보 원장은 보존하며 현재 관리 목록에서 제거하고 다음 공식 리서치의 재발견을 허용합니다.':decision==='purge'?'\n\n동일 상품이 다시 자동 후보로 올라오지 않도록 영구 제외 기록을 유지합니다.':'')))return;
    try{
      var selectedRows=ids.map(productById).filter(Boolean),ledgerRows=selectedRows.filter(function(row){return row&&row.ledgerSource==='candidate';}),legacyRows=selectedRows.filter(function(row){return !row||row.ledgerSource!=='candidate';}),processed=0,failed=0,failures=[];
      /* HOLD/REJECT management is intentionally isolated from the normal
         candidate/18-section/front handlers.  Ledger rows always go through the
         guarded queue-control endpoint with the visible bucket as an invariant. */
      for(var off=0;off<ledgerRows.length;off+=100){
        var chunk=ledgerRows.slice(off,off+100).map(function(row){return text(row.candidateId||row.id);}).filter(Boolean);
        if(!chunk.length)continue;
        try{var result=await api(QUEUE_CONTROL,'bulk_action','POST',{}, {candidateIds:chunk,decision:decision==='undecided'?'restore':decision,expectedBucket:kind,countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE'},45000);processed+=Number(result&&result.processed||0);failed+=Number(result&&result.failures&&result.failures.length||0);(result&&result.failures||[]).slice(0,20).forEach(function(f){failures.push(f);});}
        catch(error){failed+=chunk.length;failures.push({error:text(error&&error.message)||'isolated_bulk_failed'});}
      }
      for(var i=0;i<legacyRows.length;i++){var legacy=legacyRows[i];if(!legacy)continue;try{await api(CONTROL,'product_candidate_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',productId:text(legacy.id),decision:decision,placementKey:''},45000);processed+=1;}catch(error){failed+=1;failures.push({id:text(legacy.id),error:text(error&&error.message)||'legacy_item_failed'});}}
      await refreshCandidateLedgerProducts({preserveResearchReport:true});saveReviewSnapshot();renderExcludedLedger();
      var suffix=decision==='undecided'?' · 선택 상품을 전체 상품 후보·배치 관리 목록의 미배정 후보로 복원했습니다. 18개 섹션 배치나 프론트 공개는 실행하지 않았습니다.':decision==='purge'?' · 영구 제외·재수집 차단을 저장했습니다.':decision==='dismiss'?' · 공유 후보 원장은 보존하고 현재 목록에서 제거했으며 재수집을 허용했습니다.':decision==='remove_from_list'?' · 원장을 보존하고 현재 관리 목록에서만 숨겼습니다.':' · 선택 상품 상태만 변경했습니다. 프론트 공개는 실행하지 않았습니다.';
      show('요청 '+ids.length+'건 · 성공 '+processed+'건 · 실패 '+failed+'건'+suffix,failed?'warn':'ok');
  ...
  async function runLatestProductQueueAction(operation,scopeMode,options){
    ...
    try{
      ...
      var finalText=(purging?'최신 상품 영구 제외':(deleting?'최신 상품 목록 삭제':(unmatch?'최신 상품 후보 등록 해제':'최신 상품 후보 관리목록 등록')))+' 완료 · '+(allMode?'현재 리서치 전체':'선택 '+ids.length+'건')+' · 반영 '+totalHandled+'건'+(totalBlocked?' · 보호/차단 '+totalBlocked+'건':'')+(totalFailed?' · 실패 '+totalFailed+'건':'');if(state)state.textContent=finalText;if(manualState)manualState.textContent=finalText;
      show((purging?'선택 상품을 영구 제외했습니다. 같은 상품은 이후 자동 리서치에서 다시 후보로 승격하지 않습니다.':(deleting?'선택 상품을 현재 최신 리서치 목록에서 삭제했습니다.':(unmatch?'선택 상품의 후보 등록을 해제했습니다.':'최신 상품을 전체 상품 후보·배치 관리 목록에 등록했습니다.')))+(totalBlocked?(deleting?' · 후속 배치·공개·관리자 고정 '+totalBlocked+'건은 그대로 보존했습니다.':' · 보호 상태 '+totalBlocked+'건은 변경하지 않았습니다.'):'')+(!unmatch&&!deleting&&!purging&&totalHandled?' · 전체 상품 후보·배치 관리 목록에 '+totalHandled+'건 반영했습니다.':''),totalFailed?'warn':'ok');return{ok:totalFailed===0,handled:totalHandled,failed:totalFailed,blocked:totalBlocked,eligible:totalEligible,candidateIds:typeof stagedCandidateIds==='object'?Object.keys(stagedCandidateIds):[]};
  ...
  async function managementCandidateBulk(ids,decision,placementKey){
    ...
    for(var off=0;off<ledger.length;off+=100){var chunk=ledger.slice(off,off+100).map(function(row){return text(row.candidateId||row.id);}).filter(Boolean);try{var result=decision==='dismiss'?await api(QUEUE_CONTROL,'bulk_action','POST',{}, {candidateIds:chunk,decision:'dismiss',countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE'},45000):await api(CONTROL,'product_candidate_ledger_bulk_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',candidateIds:chunk,decision:decision,placementKey:placementKey||''},45000);processed+=Number(result&&result.processed||0);failed+=Number(result&&result.failed||result&&result.failures&&result.failures.length||0);(result&&result.failures||[]).slice(0,20).forEach(function(f){failures.push(f);});}catch(error){failed+=chunk.length;failures.push({error:text(error&&error.message)||'bulk_failed'});}}
