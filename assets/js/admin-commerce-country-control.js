...
(function(){
  ...
  async function runProductFrontSync(operation,mode,sectionKey,productId,selection){
    ...
    try{
      ...
      var result=aggregate,requested=Math.max(Number(result.requested||0),Number(pendingCommit.changedCount||0)),prepared=Number(result.preparation&&result.preparation.prepared||0),queued=Number(result.queued||0),persisted=Number(result.persisted||0),pendingBuild=Number(result.pendingBuild||0),blocked=Number(result.blocked||0),reasonCounts={};
