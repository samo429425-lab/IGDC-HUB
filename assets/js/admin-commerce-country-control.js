/* IGDC Global/Region/Country Commerce Control v2.5.0
...
(function(){
  ...
  function rememberReport(kind,data){if(!data)return;presentJson(data);if(kind==='global'){lastGlobalJson=data;setButtonEnabled('globalSignalDownloadBtn',true);}else if(kind==='regional'){lastRegionalJson=data;setButtonEnabled('regionalSignalDownloadBtn',true);}else if(kind==='country'){lastCountryJson=data;setButtonEnabled('countryRunDownloadBtn',true);}else if(kind==='diagnostic'){lastDiagnosticJson=data;setButtonEnabled('diagnosticDownloadBtn',true);}if(kind==='global'||kind==='regional')setButtonEnabled('downloadSignalJsonBtn',true);}
  function downloadRemembered(kind){var data=kind==='global'?lastGlobalJson:kind==='regional'?lastRegionalJson:kind==='country'?lastCountryJson:kind==='diagnostic'?lastDiagnosticJson:lastJson;if(!data){show('해당 범위에서 다운로드할 JSON이 없습니다. 먼저 점검 또는 실행을 완료해 주세요.','warn');return;}presentJson(data);download();}

  function invalidateJson(){researchStopRequested=true;lastJson=null;lastCountryJson=null;lastCountryPreview=null;setDownloadEnabled(false);setButtonEnabled('countryRunDownloadBtn',false);setButtonEnabled('runNowBtn',false);$('jsonOutput').textContent='';$('jsonPanel').classList.add('hidden');if($('countryResearchProgress'))$('countryResearchProgress').textContent='선택 범위가 바뀌었습니다. 새 범위의 저장 작업 상태를 읽어 주세요.';}
  function savedScope(){
  ...
  async function loadScope(){if(!selectedCountry)return;hide();$('refreshScopeBtn').disabled=true;var requested=scopeParams(),requestedKey=scopeKey(requested);try{var all=await Promise.all([api(REVIEW,'summary','GET',requested),api(REVIEW,'candidates','GET',requested),api(CONTROL,'scope','GET',requested),api(CONTROL,'research_status','GET',requested).catch(function(){return{ok:true,status:'not_started',candidates:[]};})]);if(scopeKey()!==requestedKey)return;renderSummary(all[0].summary||{},all[2],'저장 원장 기준');renderQueue(all[1].candidates||[]);renderAi(all[2].candidates||[]);renderActiveSignals(all[2].marketSignals||{});renderResearchProgress(all[3]);if((all[3].status==='complete'||all[3].status==='committed')&&Array.isArray(all[3].candidates)&&all[3].candidates.length){lastCountryPreview=all[3];rememberReport('country',all[3]);renderSummary(all[3],{candidates:all[3].candidates},'완료된 단계별 검색 결과');renderAi(all[3].candidates);}show(scopeName()+' 범위를 읽었습니다.','ok');}catch(e){if(scopeKey()===requestedKey)show(text(e.message)||'범위를 읽지 못했습니다.','fail');}finally{$('refreshScopeBtn').disabled=false;}}
  ...
  function download(){
    ...
    if(type==='igdc-global-market-signal-check')name='IGDC_GLOBAL_MARKET_SIGNAL_'+stamp+'.json';
