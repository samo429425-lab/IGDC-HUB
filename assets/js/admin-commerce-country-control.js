...
(function(){
  ...
  function productManagementOptions(row,mode,groupKey,slotAllowed){var counts=productSectionCounts(),current=mode==='section'?groupKey:'',recommended=sectionKeyOf(row&&row.primaryPlacement)||candidatePlacementKey(row),selected=productPlacementSelections[row.id]||((current||recommended)&&slotAllowed?'section:'+(current||recommended):'');if(selected.indexOf('section:')===0){var selectedKey=selected.slice(8),selectedFull=Number(counts[selectedKey]||0)>=100&&current!==selectedKey;if(!slotAllowed||selectedFull){selected='';delete productPlacementSelections[row.id];}}if(selected)productPlacementSelections[row.id]=selected;var pages={home:'홈',distribution:'유통',network:'네트워크',social:'소셜',tour:'투어'},groups=[];Object.keys(pages).forEach(function(page){var options=PRODUCT_SECTIONS.filter(function(section){return section.page===page;}).map(function(section){var full=Number(counts[section.key]||0)>=100&&current!==section.key,disabled=!slotAllowed||full,label=section.label+(full?' · 정원 100개':'');return'<option value="section:'+esc(section.key)+'" '+(selected==='section:'+section.key?'selected ':'')+(disabled?'disabled ':'')+'>'+esc(label)+'</option>';}).join('');groups.push('<optgroup label="'+esc(pages[page])+'">'+options+'</optgroup>');});var locked=productManagementLocked(row),stage=affiliateStage(row);return'<div class="product-placement-box"><div class="product-placement-title">관리 작업 선택</div><div class="product-management-row"><select class="product-management-select" data-product-management-select="'+esc(row.id)+'"><option value="">작업을 선택하세요</option>'+groups.join('')+'<optgroup label="상태 관리"><option value="hold" '+(selected==='hold'?'selected ':'')+'>보류 목록으로 이동</option><option value="reject" '+(selected==='reject'?'selected ':'')+'>제외 목록으로 이동</option><option value="purge" '+(selected==='purge'?'selected ':'')+'>영구 제외</option><option value="dismiss" '+(selected==='dismiss'?'selected ':'')+'>목록 삭제·재수집 허용</option><option value="remove_from_list" '+(selected==='remove_from_list'?'selected ':'')+'>목록에서만 제외</option><option value="undecided" '+(selected==='undecided'?'selected ':'')+'>미배정 후보로 되돌리기</option><option value="ai_reclassify" '+(selected==='ai_reclassify'?'selected ':'')+'>관리자 고정 해제·AI 재분류 허용</option></optgroup><optgroup label="제휴·수익 정산"><option value="affiliate:connection_required">제휴 연결 필요로 설정</option><option value="affiliate:referral_verified">리퍼럴 수익 확인</option><option value="affiliate:online_affiliate_active">온라인 제휴 활성</option><option value="affiliate:formal_partner">정식 파트너</option></optgroup></select><button type="button" class="product-management-apply" data-product-management-apply="'+esc(row.id)+'" '+(selected?'':'disabled')+'>적용</button></div><div class="product-ai-recommendation">AI 추천: '+esc(recommended&&recommended!=='unassigned'?sectionLabel(recommended):'추가 검증 필요')+' · '+(locked?'관리자 결정 고정':'AI 재분류 가능')+' · 현재 '+esc(affiliateStageLabel(stage))+'</div></div>';}
  ...
  function candidateLedgerProductRow(row){
    ...
    var ledgerDecision=text(row.slotDecision||row.queueControl&&row.queueControl.action||row.review&&row.review.state).toLowerCase();if(ledgerDecision==='remove_from_list'||ledgerDecision==='removed_from_list')decision='removed_from_list';else if(ledgerDecision==='purge'||ledgerDecision==='permanent_excluded')decision='purge';else if(ledgerDecision==='reject'||ledgerDecision==='rejected')decision='reject';else if(ledgerDecision==='hold'||ledgerDecision==='held')decision='hold';else if(stage==='held'||stage==='hold')decision='hold';else if(stage==='rejected'||stage==='reject'||stage==='suppressed')decision='reject';else if(key&&PRODUCT_SECTION_MAP[key]&&stage!=='administrator_selection_pending')decision='slot_candidate';
  ...
  async function applySupplierResearchAction(action,urlOrUrls){
    ...
    try{
      var processed=0,failed=0,missing=0,durableWarnings=[],data=null,batchSize=25;
      for(var offset=0;offset<urls.length;offset+=batchSize){
        var chunk=urls.slice(offset,offset+batchSize);
        if(urls.length>batchSize)show('공급업체 후보 '+Math.min(offset+chunk.length,urls.length)+'/'+urls.length+'건 처리 중입니다.','info');
        data=await api(CONTROL,'research_candidate_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',urls:chunk,url:chunk[0],decision:action},45000);
        var part=data&&data.candidateAction||{};processed+=part.processed==null?chunk.length:Number(part.processed||0);failed+=Number(part.failed||0);missing+=Number(part.notFound||0);if(part.durableReadError)durableWarnings.push(text(part.durableReadError));
      }
      if(data){lastCountryPreview=data;lastCountryJson=data;renderResearchProgress(data);renderSummary(data,{candidates:data.candidates||[]},'공급업체 후보 관리 반영');renderAi(data.candidates||[],data.holdingCandidates||[],data.blockedCandidates||[]);rememberReport('country',data);var hasCandidates=Array.isArray(data.candidates)&&data.candidates.length>0,complete=data.status==='complete'||data.status==='committed';setButtonEnabled('runNowBtn',hasCandidates&&complete);setButtonEnabled('productResearchBtn',hasCandidates&&complete);saveReviewSnapshot();}
      var actionSummary='공급업체 후보 요청 '+urls.length+'건 · 성공 '+processed+'건 · 실패 '+failed+'건'+(missing?' · 현재 작업에서 찾지 못함 '+missing+'건':'')+(durableWarnings.length?' · 원장 조회 확인 필요':'')+'. '+supplierActionLabel(action)+' 처리했습니다. 상품 반입이나 사이트 공개는 실행하지 않았습니다.';
      show(actionSummary,failed||durableWarnings.length?'warn':'ok');
  ...
  function productCardHtml(row,index,mode,groupKey){
    ...
    var priority=text(row.priorityLabel||''),decisionLabel={slot_candidate:'섹션 배치 예정',hold:'보류',reject:'제외',purge:'영구 제외',removed_from_list:'목록에서만 제외',undecided:'미결정'}[decision]||decision,settlementHtml=affiliateSettlementSummary(row),singleFront='';
  ...
  function productGroupDetails(key,label,rows,kind){
    ...
    var candidateToolbar=kind==='candidate'?'<div class="section-product-toolbar candidate-bulk-toolbar"><label class="product-front-select-label"><input id="candidateSelectAll-'+esc(safeKey)+'" type="checkbox" data-candidate-select-all="'+esc(key)+'"> 이 후보군 전체 선택</label><span id="candidateSelectedCount-'+esc(safeKey)+'" class="pill info">선택 후보 0건</span><button type="button" class="product-section-ai-button" data-candidate-bulk="ai" data-candidate-group="'+esc(key)+'" disabled>선택 후보 AI로 18개 섹션 자동 배치</button><select class="product-management-select candidate-target-select" data-candidate-target="'+esc(key)+'"><option value="">수동 배치 섹션 선택</option>'+PRODUCT_SECTIONS.map(function(section){return'<option value="'+esc(section.key)+'">'+esc(section.label)+'</option>';}).join('')+'</select><button type="button" class="product-section-front-button" data-candidate-bulk="place" data-candidate-group="'+esc(key)+'" disabled>선택 후보를 18개 섹션 배치 예정으로 이동</button><button type="button" class="secondary" data-candidate-bulk="undecided" data-candidate-group="'+esc(key)+'" disabled>선택 후보 미배정 복원</button><button type="button" class="secondary" data-candidate-bulk="hold" data-candidate-group="'+esc(key)+'" disabled>선택 후보 보류</button><button type="button" class="danger" data-candidate-bulk="reject" data-candidate-group="'+esc(key)+'" disabled>선택 후보 제외</button><button type="button" class="danger" data-candidate-bulk="purge" data-candidate-group="'+esc(key)+'" disabled>선택 후보 영구 제외</button><button type="button" class="secondary" data-candidate-bulk="dismiss" data-candidate-group="'+esc(key)+'" disabled>선택 후보 삭제·재수집 허용</button><button type="button" class="secondary" data-candidate-bulk="list_delete" data-candidate-group="'+esc(key)+'" disabled>선택 후보 목록에서만 제거</button></div>':'';
  ...
  async function runCandidateBulkAction(groupKey,action){
    ...
    if((action==='hold'||action==='reject'||action==='purge'||action==='dismiss'||action==='list_delete')&&!window.confirm('선택한 후보 '+ids.length+'건을 '+(action==='hold'?'보류':action==='purge'?'영구 제외':action==='dismiss'?'후보 원장에서 삭제·재수집 허용':action==='list_delete'?'현재 후보 관리 목록에서만 제거':'제외')+' 처리합니다. 프론트 공개·빌드는 실행하지 않습니다. 계속할까요?'))return;
    ...
    try{
      if(action==='dismiss'){
        var deletedBulk=await managementCandidateBulk(ids,'dismiss','');
        succeeded=Number(deletedBulk&&deletedBulk.processed||0);failed=Number(deletedBulk&&deletedBulk.failed||0);
      }else if(ledgerRows.length===selectedRows.length&&ledgerRows.length){
    ...
    var actionLabel=action==='place'?sectionLabel(placement)+' 섹션으로 수동 배치':action==='undecided'?'미배정 후보로 복원':action==='hold'?'보류':action==='purge'?'영구 제외':action==='dismiss'?'후보 원장에서 삭제·재수집 허용':action==='list_delete'?'현재 후보 관리 목록에서만 제거':action==='reject'?'제외':action;
  ...
  async function applyProductDecision(productId,decision,placementKey,skipConfirm){var row=productById(productId),title=text(row&&row.productName||row&&row.title||productId);if((decision==='purge'||decision==='dismiss'||decision==='remove_from_list')&&row&&frontPublicationActive(row)&&!skipConfirm){if(!window.confirm('현재 프론트 매칭을 먼저 해제한 뒤 이 상품의 관리 상태를 변경합니다. 계속할까요?'))return false;skipConfirm=true;}var message={slot_candidate:'선택 상품을 '+sectionLabel(placementKey)+' 섹션의 비공개 배치 예정 목록으로 보내고 관리자 결정으로 고정합니다.',hold:'선택 상품을 보류 목록으로 이동하고 관리자 결정으로 고정합니다.',reject:'선택 상품을 제외 목록으로 이동하고 관리자 결정으로 고정합니다.',purge:'선택 상품을 영구 제외합니다. 같은 원장 후보의 자동 승격을 차단합니다.',dismiss:'선택 상품을 후보 원장에서 삭제합니다. 차단 기록은 남기지 않아 다음 리서치에서 다시 발견될 수 있습니다.',remove_from_list:'선택 상품을 현재 관리 목록에서만 제외합니다. 영구 차단은 하지 않습니다.',undecided:'선택 상품을 미배정 후보 목록으로 복원하고 관리자 결정으로 유지합니다.',ai_reclassify:'관리자 고정을 해제하고 다음 AI 자동화의 재분류 대상으로 돌립니다.'}[decision]||'상품 상태를 변경합니다.';if(!skipConfirm&&['reject','purge','dismiss','remove_from_list'].indexOf(decision)>=0&&!window.confirm(message+'\n\n'+title))return false;try{if(row&&['hold','reject','purge','dismiss','remove_from_list','undecided','ai_reclassify'].indexOf(decision)>=0&&frontPublicationActive(row))await deferFrontUnmatchBeforeManagementChange(row);if(row&&row.ledgerSource==='candidate'&&decision==='dismiss'){var deleted=await api(QUEUE_CONTROL,'bulk_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',candidateIds:[text(row.candidateId||row.id)],decision:'dismiss'},30000);await refreshCandidateLedgerProducts();show('상품 1건을 후보 원장에서 삭제했습니다. 다음 정상 리서치에서 다시 발견될 수 있습니다.','ok');return Number(deleted&&deleted.processed||0)>0;}if(row&&row.ledgerSource==='candidate'){var ledgerResult=await api(CONTROL,'product_candidate_ledger_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',candidateId:text(row.candidateId||row.id),decision:decision,placementKey:placementKey||''});await refreshCandidateLedgerProducts();var lp=ledgerResult.actionResult&&ledgerResult.actionResult.placement,ledgerMessage=decision==='slot_candidate'?'상품 1건을 '+sectionLabel(sectionKeyOf(lp)||placementKey)+' 섹션 후보로 지정했습니다.':decision==='hold'?'상품 1건을 보류 목록으로 이동했습니다.':decision==='reject'?'상품 1건을 제외 목록으로 이동했습니다.':decision==='purge'?'상품 1건을 영구 제외했습니다.':decision==='remove_from_list'?'상품 1건을 현재 관리 목록에서 제외했습니다.':decision==='ai_reclassify'?'상품 1건의 관리자 고정을 해제하고 AI 재분류 대상으로 돌렸습니다.':'상품 1건을 미배정 후보 목록으로 복원했습니다.';show(ledgerMessage+' 자동 공개나 결제는 실행하지 않았습니다.','ok');return true;}var data=await api(CONTROL,'product_candidate_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',productId:productId,decision:decision,placementKey:placementKey||''});lastProductJson=data;rememberReport('product',data);renderProductProgress(data);renderProducts(data.products||[]);saveReviewSnapshot();var placement=data.actionResult&&data.actionResult.placement,doneMessage=decision==='slot_candidate'?'상품 1건을 '+sectionLabel(sectionKeyOf(placement)||placementKey)+' 섹션에 관리자 우선으로 지정했습니다.':decision==='hold'?'상품 1건을 보류 목록으로 이동했습니다.':decision==='reject'?'상품 1건을 제외 목록으로 이동했습니다.':decision==='purge'?'상품 1건을 영구 제외했습니다.':decision==='dismiss'?'상품 1건을 후보 원장에서 삭제했습니다.':decision==='remove_from_list'?'상품 1건을 현재 관리 목록에서 제외했습니다.':decision==='ai_reclassify'?'상품 1건의 관리자 고정을 해제하고 AI 재분류 대상으로 돌렸습니다.':'상품 1건을 미배정 후보 목록으로 복원했습니다.';show(doneMessage+' 자동 공개나 결제는 실행하지 않았습니다.','ok');return true;}catch(e){show(text(e.message)||'상품 판정을 저장하지 못했습니다.','fail');return false;}}
  async function applyProductExclusionBulk(kind,decision){
    ...
    if(['reject','purge','dismiss','remove_from_list'].indexOf(decision)>=0){var label=decision==='purge'?'영구 제외':decision==='dismiss'?'삭제·재수집 허용':decision==='remove_from_list'?'목록에서만 제외':'제외 관리로 이동';if(!window.confirm('선택 '+ids.length+'건을 '+label+' 처리하시겠습니까?'))return;}
    try{var result=await managementCandidateBulk(ids,decision,'');var summary='요청 '+ids.length+'건 · 성공 '+Number(result.processed||0)+'건 · 실패 '+Number(result.failed||0)+'건',suffix=decision==='purge'?' · 영구 제외 기록을 남겼습니다.':decision==='dismiss'?' · 후보 원장에서 삭제했습니다. 다음 리서치에서 다시 발견될 수 있습니다.':decision==='remove_from_list'?' · 현재 관리 목록에서만 제외했습니다.':' · 관리자 후보 상태를 변경했습니다. 프론트 공개는 실행하지 않았습니다.';show(summary+suffix,result.failed?'warn':'ok');renderExcludedLedger();}
    ...
    try{
      ...
      if(needsFinalize){
        try{
          ...
          var finalized=await api(CONTROL,'product_front_finalize','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',operation:finalizeOperation,confirmation:unmatch?'SITE_UNPUBLISH':'SITE_PUBLISH',candidateIds:finalizeIds,changedCount:Math.max(aggregate.changedCandidateIds.length,aggregate.withdrawn,aggregate.persistedCandidateIds.length),ledgerMode:'candidate',compactResponse:true,authoritativeReplacement:replacementRun,replacementCandidateIds:replacementRun?targetRows.map(function(row){return text(row&&row.candidateId||row&&row.id);}).filter(Boolean):[],replacementSectionKeys:replacementRun?(mode==='all'?PRODUCT_SECTIONS.map(function(section){return section.key;}):sectionMode?[sectionKey]:sectionsMode?selectedSections:[]):[]},90000);
  ...
  function managementOperationAllowed(workspaceKey,operation){
    ...
    if(['hold','reject','restore','dismiss'].indexOf(op)>=0)return key==='product_candidate_management'||key==='distribution_18_sections'||key==='excluded_ledger_management'||key.indexOf('candidate_group_')===0||key.indexOf('section_')===0||key==='product_research_control';
    return false;
  }
  ...
  async function managementCandidateBulk(ids,decision,placementKey){
    ...
    for(var off=0;off<ledger.length;off+=100){var chunk=ledger.slice(off,off+100).map(function(row){return text(row.candidateId||row.id);}).filter(Boolean);try{var result=decision==='dismiss'?await api(QUEUE_CONTROL,'bulk_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',candidateIds:chunk,decision:'dismiss'},30000):await api(CONTROL,'product_candidate_ledger_bulk_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',candidateIds:chunk,decision:decision,placementKey:placementKey||''},30000);processed+=Number(result&&result.processed||0);failed+=Number(result&&result.failed||0);(result&&result.failures||[]).slice(0,20).forEach(function(f){failures.push(f);});}catch(error){failed+=chunk.length;failures.push({error:text(error&&error.message)||'bulk_failed'});}}
  ...
  function wireFrontSelectionControls(){
    ...
    var panel=$('productResearchPanel');if(!panel)return;
    panel.addEventListener('click',function(event){var selected=event.target&&event.target.closest?event.target.closest('[data-selected-products-front]'):null;if(!selected)return;event.preventDefault();event.stopPropagation();var sectionKey=selected.getAttribute('data-selected-products-front'),operation=selected.getAttribute('data-front-operation')==='unmatch'?'unmatch':'match',ids=selectedProductIds(sectionKey);if(operation==='unmatch'){frontUnmatchCandidateIds(ids).then(function(result){show('선택 상품 매칭 해제 · 성공 '+Number(result&&result.processed||0)+'건 · 실패 '+Number(result&&result.failed||0)+'건',Number(result&&result.failed||0)?'warn':'ok');}).catch(function(error){show(conciseRequestError(error&&error.message,error&&error.status)||'선택 상품 매칭 해제를 완료하지 못했습니다.','fail');});return;}runProductFrontSync('match','candidates','','',ids);});
