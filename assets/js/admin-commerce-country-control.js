/* IGDC Global/Region/Country Commerce Control v2.9.0
...
(function(){
  ...
  async function rawRequest(endpoint,action,method,params,body,token){
    ...
    if(!res.ok||!data||data.ok!==true){var er=new Error(data&&data.error||('HTTP '+res.status));er.status=res.status;er.code=data&&data.code;er.payload=data||null;throw er;}return data;
  }
  ...
  function rememberReport(kind,data){if(!data)return;presentJson(data);if(kind==='global'){lastGlobalJson=data;setButtonEnabled('globalSignalDownloadBtn',true);}else if(kind==='regional'){lastRegionalJson=data;setButtonEnabled('regionalSignalDownloadBtn',true);}else if(kind==='country'){lastCountryJson=data;setButtonEnabled('countryRunDownloadBtn',true);}else if(kind==='diagnostic'){lastDiagnosticJson=data;setButtonEnabled('diagnosticDownloadBtn',true);}else if(kind==='globalControl'){lastGlobalControlJson=data;setButtonEnabled('globalControlDownloadBtn',true);}else if(kind==='product'){lastProductJson=data;setButtonEnabled('productResearchDownloadBtn',true);}if(kind==='global'||kind==='regional')setButtonEnabled('downloadSignalJsonBtn',true);}
  function downloadRemembered(kind){var data=kind==='global'?lastGlobalJson:kind==='regional'?lastRegionalJson:kind==='country'?lastCountryJson:kind==='diagnostic'?lastDiagnosticJson:kind==='globalControl'?lastGlobalControlJson:kind==='product'?lastProductJson:lastJson;if(!data){show('해당 범위에서 다운로드할 JSON이 없습니다. 먼저 점검 또는 실행을 완료해 주세요.','warn');return;}presentJson(data);download();}

  function invalidateJson(){researchStopRequested=true;productStopRequested=true;lastJson=null;lastCountryJson=null;lastCountryPreview=null;lastProductJson=null;setDownloadEnabled(false);setButtonEnabled('countryRunDownloadBtn',false);setButtonEnabled('runNowBtn',false);setButtonEnabled('productResearchDownloadBtn',false);$('jsonOutput').textContent='';$('jsonPanel').classList.add('hidden');if($('countryResearchProgress'))$('countryResearchProgress').textContent='선택 범위가 바뀌었습니다. 새 범위의 저장 작업 상태를 읽어 주세요.';if($('productResearchState'))$('productResearchState').textContent='선택 범위가 바뀌었습니다. 이 범위의 상품 리서치 상태를 다시 읽어 주세요.';}
  function savedScope(){
  ...
  function renderScope(){var c=country(selectedCountry),ef=effective(selectedCountry,selectedSubdivision==='NATIONWIDE'?'':selectedSubdivision),specific=selectedSubdivision!=='NATIONWIDE'?setting('subdivision',{countryCode:selectedCountry,subdivisionCode:selectedSubdivision}):setting('country',{countryCode:selectedCountry}),masterMode=catalog&&catalog.master&&catalog.master.mode||'off',scheduleNote=(ef.mode==='auto'&&masterMode!=='auto')?' · 전 세계 예약 중지 상태이므로 정기 자동 실행은 멈춰 있고 수동 검색·실행만 가능합니다.':'';$('scopeTitle').textContent=scopeName();$('scopeMeta').innerHTML='실효 모드 <strong class="'+modeClass(ef.mode)+'">'+esc(modeText(ef.mode))+'</strong> · 기준 '+esc(ef.sourceId||'기본값')+' · 주기 '+esc(ef.intervalDays)+'일 · 1회 최대 '+esc(ef.maxCandidates)+'개'+esc(scheduleNote);$('scopeMode').value=specific&&specific.mode||'inherit';$('scopeInterval').value=String(specific&&specific.intervalDays||ef.intervalDays||1);$('maxCandidates').value=String(specific&&specific.maxCandidates||ef.maxCandidates||20);$('expandField').classList.toggle('hidden',!(c&&c.requiresSubdivision&&selectedSubdivision==='NATIONWIDE'));$('expandSubdivisions').value=String(!!(specific&&specific.expandSubdivisions));$('scopeLinks').innerHTML=[['상품 후보·중개수익 대기열','/commerce-candidate-pipeline.html','선택 국가·지역 후보'],['실상품 공급 개방 점검','/product-go-live-audit.html','신뢰·배송·수익·공개 게이트']].map(function(x){return'<button type="button" class="linkbtn" data-link="'+esc(x[1])+'"><strong>'+esc(x[0])+'</strong><small>'+esc(x[2])+' · '+esc(scopeName())+'</small></button>';}).join('');Array.prototype.forEach.call(document.querySelectorAll('[data-link]'),function(b){b.addEventListener('click',function(){rememberReviewReturn();location.href=scopedUrl(b.getAttribute('data-link'));});});}
  ...
  function renderAi(rows){
    ...
    $('aiRows').innerHTML=rows.length?rows.map(function(c){
      var ai=c.ai||c||{},trust=c.trust||{},supplier=c.supplier||{type:c.supplierType},e=c.evidence||{};
      function yes(v,label){return'<span class="pill '+(v?'good':'warn')+'">'+esc(label)+' '+(v?'확인':'미확인')+'</span> ';}
      function lines(v){return(Array.isArray(v)?v:[]).map(function(x){return'· '+esc(x);}).join('<br>');}
      var evidence=yes(e.official,'공식')+yes(e.responsibleEntity,'책임주체')+yes(e.directSales,'직판')+yes(e.payment,'결제')+yes(e.shipping,'배송')+yes(e.returns,'반품')+yes(e.refund,'환불')+yes(e.service,'고객지원');
      var score=Number(ai.trustScore!=null?ai.trustScore:(ai.score!=null?ai.score:(trust.trustScore||0)));
      var rating=Number(ai.rating10||trust.rating10||trustRating10(score));
      var rank=ai.rank||c.rank||trust.rank||'-';
      var recommendation=ai.recommendationLabel||trust.recommendationLabel||ai.recommendation||trust.recommendation||'보류 권고';
      var confidence=Number(ai.assessmentConfidence||trust.assessmentConfidence||0);
      var mode=ai.assessmentMode||trust.assessmentMode||'deterministic_fallback';
      var sourceLabel=mode==='openai_grounded'?'AI 추천':'규칙기반 임시추천';
      var summary=ai.aiSummary||trust.aiSummary||ai.reason||'';
      var strengths=Array.isArray(ai.strengths)?ai.strengths:(Array.isArray(trust.strengths)?trust.strengths:[]);
      var concerns=Array.isArray(ai.concerns)?ai.concerns:(Array.isArray(trust.concerns)?trust.concerns:[]);
      var nextChecks=Array.isArray(ai.nextChecks)?ai.nextChecks:(Array.isArray(trust.nextChecks)?trust.nextChecks:[]);
      var detail='<strong>'+esc(sourceLabel)+': '+esc(recommendation)+'</strong><br><span class="pill '+(rating>=8?'good':'warn')+'">신뢰 '+esc(rating)+'/10</span> <span class="small">'+esc(score)+'/100 · 순위 '+esc(rank)+(confidence?' · 판단확신 '+esc(confidence)+'%':'')+'</span><br><span class="small">'+esc(summary)+'</span>';
      if(strengths.length)detail+='<br><span class="small"><strong>강점</strong><br>'+lines(strengths)+'</span>';
      if(concerns.length)detail+='<br><span class="small"><strong>주의</strong><br>'+lines(concerns)+'</span>';
      if(nextChecks.length)detail+='<br><span class="small"><strong>다음 확인</strong><br>'+lines(nextChecks)+'</span>';
      ...
      var actions=actionId?'<button type="button" data-ai-action="accept_for_completion" data-id="'+esc(actionId)+'">업체 확인 계속</button> <button type="button" class="secondary" data-ai-action="hold" data-id="'+esc(actionId)+'">보류</button> <button type="button" class="danger" data-ai-action="reject" data-id="'+esc(actionId)+'">제외</button>':'<span class="small">미리보기 결과 · 저장되지 않음</span>';
      return'<tr data-review-anchor="supplier-'+esc(actionId||supplierTitle.replace(/[^0-9A-Za-z가-힣_-]+/g,'-').slice(0,48))+'"><td>'+supplierCell+'</td><td>'+esc(supplier.type||c.kind||'supplier')+'<br><span class="small">'+esc((supplier.detectedCountry||c.targetCountry||'-')+' / '+(supplier.detectedRegion||c.targetRegion||'NATIONWIDE'))+'</span></td><td>'+evidence+'<br><span class="small">상품 자동 반입 없음 · 거래는 공급업체에서 직접 진행</span></td><td>'+detail+'</td><td>'+esc(c.status||c.persistence||'-')+'<br><span class="small">'+esc(c.updatedAt||c.createdAt||ai.assessedAt||trust.assessedAt||'-')+'</span></td><td>'+actions+'</td></tr>';
    }).join(''):'<tr><td colspan="6" class="small">이 범위에서 생성된 책임 공급업체 비공개 후보가 없습니다.</td></tr>';
  ...
  function externalReviewAnchor(url,label,className){var safe=safeExternalUrl(url);if(!safe)return'';return'<a href="'+esc(safe)+'" class="'+esc(className||'')+'" data-review-link="1" rel="noreferrer external nofollow">'+esc(label)+'</a>';}
  ...
  function compactProductSnapshot(data){if(!data||typeof data!=='object')return null;return{ok:true,reportType:data.reportType||'igdc-country-product-reference-persisted-research',version:data.version||'',jobId:data.jobId||null,jobVersion:data.jobVersion||'',needsRefresh:data.needsRefresh===true,status:data.status||'not_started',startedAt:data.startedAt||null,finishedAt:data.finishedAt||null,updatedAt:data.updatedAt||null,scope:data.scope||scopeSnapshot('review-return-cache'),safety:data.safety||{},progress:data.progress||{},summary:data.summary||{},products:Array.isArray(data.products)?data.products.slice(0,300):[]};}
  function saveReviewSnapshot(){try{var product=compactProductSnapshot(lastProductJson);if(!product)return;sessionStorage.setItem(REVIEW_SNAPSHOT_KEY,JSON.stringify({scope:scopeSnapshot('review-return-cache'),savedAt:Date.now(),product:product}));}catch(_e){}}
  ...
  function renderProductProgress(data){
    data=data||{status:'not_started',summary:{},progress:{}};var summary=data.summary||{},progress=data.progress||{},discovery=progress.discovery||{},inspection=progress.inspection||{};
    var status=text(data.status||'not_started'),labels={not_started:'아직 상품 리서치를 시작하지 않았습니다.',discovering:'공식 업체 사이트에서 실제 상품 페이지를 찾는 중입니다. 발견된 상품은 검증 전 상태로 바로 표시합니다.',inspecting:'상품 페이지·이미지·영상·원본 링크를 확인하는 중입니다.',complete:'상품 이미지·상품명·영상·원본 링크 리서치를 완료했습니다.',cancelled:'상품 리서치가 중단됐습니다.'};
    if($('productResearchState'))$('productResearchState').textContent=(data.needsRefresh===true?'이전 상품 추출 결과입니다. 카테고리 페이지를 제외하고 실제 상품 상세페이지·썸네일을 다시 조사해야 합니다. · ':'')+(labels[status]||status)+' · 업체 '+Number(discovery.done||0)+'/'+Number(discovery.total||0)+' · 상품 점검 '+Number(inspection.done||0)+'/'+Number(inspection.total||0);
    if($('productResearchStats'))$('productResearchStats').innerHTML=stat('업체 확인',Number(summary.suppliersChecked||0)+'/'+Number(summary.suppliers||0),'공식 사이트')+stat('상품 발견',summary.discovered||0,'원본 페이지 후보')+stat('상품 점검',summary.inspected||0,'상세페이지 확인')+stat('이미지 확인',summary.withImage||0,'실제 상품 이미지')+stat('영상 확인',summary.withVideo||0,'직접 재생·원본 영상')+stat('관리자 검토 가능',summary.readyForAdminReview||0,'원본 링크 포함')+stat('슬롯 후보 지정',summary.slotCandidates||0,'아직 공개 전');
    setButtonEnabled('productResearchDownloadBtn',status!=='not_started');
    if($('productResearchBtn')&&!productLoopActive)$('productResearchBtn').textContent=data.needsRefresh===true?'상품 썸네일 다시 조사':((status==='discovering'||status==='inspecting')?'상품 조사 계속':'공식 상품 목록 리서치');
  }
  function renderProducts(rows){
    rows=Array.isArray(rows)?rows:[];var grid=$('productGrid');if(!grid)return;
    grid.innerHTML=rows.length?rows.map(function(row){
      var productUrl=safeExternalUrl(row.productUrl||row.url),imageUrl=safeExternalImageUrl(row.imageUrl||row.imageOriginalUrl),supplierUrl=safeExternalUrl(row.supplierSiteUrl),videoUrl=safeExternalVideoUrl(row.videoUrl||row.videoContentUrl||row.videoEmbedUrl),videoThumb=safeExternalImageUrl(row.videoThumbnailUrl||row.videoPosterUrl||imageUrl),decision=text(row.slotDecision||'undecided'),decisionLabel={slot_candidate:'슬롯 후보',hold:'보류',reject:'제외',undecided:'미결정'}[decision]||decision;
      var image=imageUrl?'<a href="'+esc(productUrl||imageUrl)+'" data-review-link="1" rel="noreferrer external nofollow" referrerpolicy="no-referrer"><img data-product-image="1" src="'+esc(imageUrl)+'" alt="'+esc(row.productName||'상품 이미지')+'" loading="lazy" /></a><div class="product-image-missing hidden">상품 이미지 원본을 불러오지 못했습니다.</div>':'<div class="product-image-missing">상품 이미지 확인 필요</div>';
      var direct=directVideoUrl(videoUrl),embed=embeddableVideoUrl(row.videoEmbedUrl||videoUrl),video='';
      if(direct)video='<div class="product-video-frame"><video controls playsinline preload="metadata" '+(videoThumb?'poster="'+esc(videoThumb)+'"':'')+'><source src="'+esc(direct)+'">브라우저에서 이 상품 영상을 재생할 수 없습니다.</video></div>';
      else if(embed)video='<div class="product-video-frame"><iframe loading="lazy" src="'+esc(embed)+'" title="'+esc((row.productName||'상품')+' 영상')+'" allow="fullscreen; picture-in-picture" referrerpolicy="no-referrer" sandbox="allow-scripts allow-same-origin allow-presentation"></iframe></div>';
      else if(videoUrl)video='<div class="product-video-frame">'+externalReviewAnchor(videoUrl,'▶ 상품 영상 원본 열기','product-video-link')+'</div>';
      var linkParts=[];if(productUrl)linkParts.push(externalReviewAnchor(productUrl,'상품 페이지 열기',''));if(imageUrl)linkParts.push(externalReviewAnchor(imageUrl,'이미지 원본 열기',''));if(videoUrl)linkParts.push(externalReviewAnchor(videoUrl,'상품 영상 열기',''));if(supplierUrl)linkParts.push(externalReviewAnchor(supplierUrl,'업체 사이트 확인',''));
      var ready=row.researchStatus==='ready_for_admin_review',actionable=row.inspectionComplete===true||ready||row.researchStatus==='needs_manual_review',slotAllowed=ready&&!!imageUrl&&!!productUrl;
      var actions='';if(row.id&&actionable){actions='<button type="button" data-product-action="slot_candidate" data-product-id="'+esc(row.id)+'" '+(slotAllowed?'':'disabled title="상품 상세페이지·실제 이미지 검증을 마친 뒤 슬롯 후보로 지정할 수 있습니다."')+'>슬롯 후보 지정</button><button type="button" class="secondary" data-product-action="hold" data-product-id="'+esc(row.id)+'">보류</button><button type="button" class="danger" data-product-action="reject" data-product-id="'+esc(row.id)+'">제외</button>';}else actions='<span class="product-verification-note">상세페이지 검증 중입니다. 원본 링크는 먼저 확인할 수 있지만 슬롯 후보 지정은 아직 잠겨 있습니다.</span>';
      var priorityLabel=text(row.priorityLabel||'');return'<article class="product-card" data-review-anchor="product-'+esc(row.id||productUrl||row.productName||'unknown')+'"><div class="product-image-frame">'+image+'</div>'+video+'<div class="product-body"><strong class="product-title">'+esc(row.productName||row.title||'상품명 확인 필요')+'</strong><div class="small">판매·책임 업체: '+esc(row.supplierName||'-')+'</div><div class="small"><span class="pill '+(ready?'good':'warn')+'">'+esc(row.researchStatus||'미확인')+'</span> <span class="pill info">'+esc(decisionLabel)+'</span>'+(priorityLabel?' <span class="pill good">'+esc(priorityLabel)+'</span>':'')+'</div><div class="product-links">'+(linkParts.join('')||'<span class="small">확인 가능한 외부 링크가 없습니다.</span>')+'</div><div class="small mono">'+esc(productUrl||'상품 원본 주소 확인 필요')+'</div><div class="product-actions">'+actions+'</div></div></article>';
    }).join(''):'<div class="small">아직 리서치된 실제 상품 후보가 없습니다.</div>';
    Array.prototype.forEach.call(grid.querySelectorAll('[data-product-image]'),function(img){img.addEventListener('error',function(){img.classList.add('hidden');var fallback=img.parentNode&&img.parentNode.nextElementSibling;if(fallback)fallback.classList.remove('hidden');});});
    Array.prototype.forEach.call(grid.querySelectorAll('[data-product-action]'),function(button){button.addEventListener('click',async function(){try{var data=await api(CONTROL,'product_candidate_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',productId:button.getAttribute('data-product-id'),decision:button.getAttribute('data-product-action')});lastProductJson=data;renderProductProgress(data);renderProducts(data.products||[]);rememberReport('product',data);show('상품 관리자 판정을 저장했습니다. 슬롯이나 공개 페이지에는 아직 반영하지 않았습니다.','ok');}catch(e){show(text(e.message)||'상품 판정을 저장하지 못했습니다.','fail');}});});
  }
  async function runProductResearch(options){
    options=options||{};if(!selectedCountry||productLoopActive)return;var requested=scopeSnapshot(),requestedKey=scopeKey(requested),restart=!!(lastProductJson&&(lastProductJson.status==='complete'||lastProductJson.needsRefresh===true));if(restart&&!options.automatic&&!window.confirm(lastProductJson&&lastProductJson.needsRefresh===true?'이전 결과에는 카테고리 페이지가 상품처럼 포함될 수 있습니다. 기존 비공개 결과를 새 상품 상세페이지·썸네일 조사 결과로 교체하시겠습니까?':'현재 완료된 상품 리서치 결과는 관리 DB에 보존되어 있습니다. 같은 국가 범위에서 새 상품 리서치를 시작하시겠습니까?'))return;if(restart&&options.automatic)return;
    hide();productStopRequested=false;productLoopActive=true;var button=$('productResearchBtn');button.disabled=true;button.textContent='실상품 리서치 진행 중';setButtonEnabled('productResearchPauseBtn',true);if($('productResearchState'))$('productResearchState').textContent=(options.automatic?'공급업체 검증 완료 후 실상품 리서치를 자동으로 시작했습니다. ':'')+'공식 상품 페이지·상품명·상품 이미지·상품 영상·원본 링크를 단계별로 확인합니다.';
    try{var data=await api(CONTROL,'product_research_begin','POST',{}, {countryCode:requested.country,subdivisionCode:requested.region,restart:restart});while(true){if(scopeKey()!==requestedKey){productStopRequested=true;show('상품 조사 중 선택 범위가 바뀌어 화면 진행을 중단했습니다. 서버 작업은 원래 범위에서 재개할 수 있습니다.','warn');break;}lastProductJson=data;renderProductProgress(data);renderProducts(data.products||[]);rememberReport('product',data);if(data.status==='complete'){show((data.products||[]).length?'실제 상품 이미지·상품명·상품 영상·원본 페이지·업체 사이트 리서치를 완료했습니다. 목록을 직접 확인해 슬롯 후보를 지정하세요.':'상품 리서치는 끝났지만 확인 가능한 실제 상품 목록을 만들지 못했습니다. 업체 공식 사이트와 JSON 경로를 점검하세요.',(data.products||[]).length?'ok':'warn');break;}if(productStopRequested){show('상품 조사를 일시정지했습니다. 지금까지의 결과는 관리 DB에 보존됐습니다.','warn');break;}await new Promise(function(resolve){setTimeout(resolve,300);});data=await api(CONTROL,'product_research_step','POST',{}, {countryCode:requested.country,subdivisionCode:requested.region,jobId:data.jobId});}}
    catch(e){try{var saved=await api(CONTROL,'product_research_status','GET',{country:requested.country,region:requested.region});lastProductJson=saved;renderProductProgress(saved);renderProducts(saved.products||[]);if(saved.status!=='not_started')setButtonEnabled('productResearchDownloadBtn',true);}catch(_statusError){}show((text(e&&e.message)||'상품 리서치 요청이 중단됐습니다.')+' 완료된 단계는 관리 DB에 보존됩니다.','fail');}
    finally{productLoopActive=false;button.disabled=false;button.textContent='공식 상품 목록 리서치';setButtonEnabled('productResearchPauseBtn',false);}
  }
  ...
  function renderResearchProgress(data){
    ...
    if($('countryResearchProgress'))$('countryResearchProgress').textContent=(labels[status]||status)+' · 검색 '+Number(s.done||0)+'/'+Number(s.total||0)+' · 증빙 '+Number(i.done||0)+'/'+Number(i.total||0)+' · AI 평가 '+Number(r.done||0)+'/'+Number(r.total||0)+' · 원천 후보 '+Number(data.summary&&data.summary.collected||0)+' · 최종 후보 '+Number(data.summary&&data.summary.ranked||0)+(data.lastError?' · 최근 오류: '+text(data.lastError.message):'');
  ...
  async function loadScope(reason){
    if(!selectedCountry)return;hide();$('refreshScopeBtn').disabled=true;
    var requested=scopeParams(),requestedKey=scopeKey(requested),failures=[];
    var scopeFallback={ok:true,country:country(requested.country),effective:effective(requested.country,requested.region==='NATIONWIDE'?'':requested.region),marketSignals:{},candidates:[]};
    var researchFallback={ok:true,status:'not_started',candidates:[],summary:{},progress:{}};
    var productFallback={ok:true,status:'not_started',products:[],summary:{},progress:{}};
    try{
      ...
      var summaryPart=await readScopePart('상품 후보 요약',REVIEW,'summary',requested,{ok:true,summary:{}});if(scopeKey()!==requestedKey)return;
      if(!summaryPart.ok)failures.push(summaryPart);renderSummary(summaryPart.data&&summaryPart.data.summary||{},scopeData,'저장 원장 기준');

      var queuePart=await readScopePart('상품 후보 대기열',REVIEW,'candidates',requested,{ok:true,candidates:[]});if(scopeKey()!==requestedKey)return;
      if(!queuePart.ok)failures.push(queuePart);renderQueue(queuePart.data&&queuePart.data.candidates||[]);
      ...
      if((researchData.status==='complete'||researchData.status==='committed')&&Array.isArray(researchData.candidates)&&researchData.candidates.length){lastCountryPreview=researchData;lastCountryJson=researchData;setButtonEnabled('countryRunDownloadBtn',true);renderSummary(researchData,{candidates:researchData.candidates},'완료된 단계별 검색 결과');renderAi(researchData.candidates);}

      var productPart=await readScopePart('상품 이미지·원본 링크 리서치',CONTROL,'product_research_status',requested,productFallback);if(scopeKey()!==requestedKey)return;
      if(!productPart.ok)failures.push(productPart);var productData=productPart.data||productFallback;lastProductJson=productData.status!=='not_started'?productData:null;renderProductProgress(productData);renderProducts(productData.products||[]);setButtonEnabled('productResearchBtn',researchData.status==='complete'||researchData.status==='committed'||!!lastProductJson);if(lastProductJson)setButtonEnabled('productResearchDownloadBtn',true);
      var canWrite=!!(acceptedSession&&acceptedSession.session&&acceptedSession.session.write),autoProductReady=!returnRestoreActive&&canWrite&&productData.status==='not_started'&&(researchData.status==='complete'||researchData.status==='committed')&&Array.isArray(researchData.candidates)&&researchData.candidates.length>0;
      if(autoProductReady&&!productLoopActive)setTimeout(function(){if(scopeKey()===requestedKey)runProductResearch({automatic:true});},180);

      if(failures.length){
        var delayed=failures.map(function(row){return row.label;}).join(' · '),prefix=reason==='ip'?'현재 접속 IP 범위 적용은 정상입니다. ':' ';
        show(prefix+'일부 상세 원장 응답이 지연되어 해당 부분만 보류했습니다: '+delayed+'. 다른 데이터는 유지됐으며 범위 읽기로 다시 확인할 수 있습니다.','warn');
      }else show(scopeName()+' 범위를 읽었습니다.','ok');
      restoreReviewReturn();
  ...
  async function runAutomation(dry){
    if(!selectedCountry||researchLoopActive)return;if(dry!==true){show('먼저 책임 공급업체 단계별 검색을 완료한 뒤 검색 결과 실행을 눌러 주세요.','warn');return;}
    ...
    try{
      ...
      try{var saved=await api(CONTROL,'research_status','GET',{country:requested.country,region:requested.region});lastCountryPreview=saved;rememberReport('country',saved);renderResearchProgress(saved);if(Array.isArray(saved.candidates)&&saved.candidates.length)renderAi(saved.candidates);}catch(_statusError){}
  ...
  async function commitCountryPreview(){
    ...
    if(!lastCountryPreview||!Array.isArray(lastCountryPreview.candidates)||!lastCountryPreview.candidates.length){show('실행할 검색 결과가 없습니다. 먼저 책임 공급업체 검색을 완료해 주세요.','warn');setButtonEnabled('runNowBtn',false);return;}
    ...
    try{
      ...
      if(scopeKey()!==requestedKey){invalidateJson();show('실행 중 선택 범위가 바뀌어 결과를 폐기했습니다. 현재 범위를 다시 읽어 주세요.','warn');return;}
      if(!data.scope)data.scope=requested;else{data.scope.source=requested.source;data.scope.detectedGeo=requested.detectedGeo;}
      lastCountryPreview=null;rememberReport('country',data);renderSummary(data,{candidates:data.candidates||[]},'검색 결과 실행 완료');renderAi(data.candidates||[]);
      try{await reloadData(false);await loadScope();}catch(_reloadError){}
      renderSummary(data,{candidates:data.candidates||[]},'검색 결과 실행 완료');renderAi(data.candidates||[]);presentJson(data);
      show('검색 결과를 비공개 책임 공급업체 후보 원장에 실행·저장했습니다. 상품 자동 반입이나 공개는 하지 않았습니다.',data.ok?'ok':'warn');
    }catch(e){
      ...
      rememberReport('country',failure);show(text(e&&e.message)||'검색 결과 실행에 실패했습니다. 오류 JSON을 내려받아 확인할 수 있습니다.','fail');
  ...
  async function diagnostic(){
    hide();$('diagnosticBtn').disabled=true;var requested=scopeSnapshot(),requestedKey=scopeKey(requested);
    try{var p={country:requested.country,region:requested.region};var all=await Promise.all([api(CONTROL,'diagnostic','GET',{}),api(REVIEW,'diagnostic','GET',p),api(CONTROL,'scope','GET',p),api(CONTROL,'signal_status','GET',{regionGroup:requested.regionGroup})]);if(scopeKey()!==requestedKey){invalidateJson();show('점검 중 선택 범위가 바뀌어 이전 결과를 폐기했습니다. 현재 범위에서 다시 점검해 주세요.','warn');return;}var report={ok:true,reportType:'igdc-integrated-commerce-control-diagnostic',generatedAt:new Date().toISOString(),scope:requested,control:all[0],queue:all[1],automationScope:all[2],marketSignals:all[3]};rememberReport('diagnostic',report);show('통합 점검 JSON을 생성했습니다.','ok');}
