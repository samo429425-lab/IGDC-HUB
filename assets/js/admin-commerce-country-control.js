/* IGDC Global/Region/Country Commerce Control v2.8.0
...
(function(){
  ...
  function externalReviewAnchor(url,label,className){var safe=safeExternalUrl(url);if(!safe)return'';return'<a href="'+esc(safe)+'" class="'+esc(className||'')+'" data-review-link="1" rel="noreferrer external nofollow" referrerpolicy="no-referrer">'+esc(label)+'</a>';}
  function rememberReviewReturn(){try{sessionStorage.setItem(REVIEW_RETURN_KEY,JSON.stringify({path:location.pathname+location.search,scrollY:Math.max(0,Math.round(window.scrollY||0)),at:Date.now()}));}catch(_e){}}
  function restoreReviewReturn(){var row=null;try{row=parseJson(sessionStorage.getItem(REVIEW_RETURN_KEY)||'');}catch(_e){}if(!row||row.path!==(location.pathname+location.search)||Date.now()-Number(row.at||0)>21600000)return;var y=Math.max(0,Number(row.scrollY||0));[40,180,520,1100].forEach(function(delay){setTimeout(function(){window.scrollTo({top:y,left:0,behavior:'auto'});},delay);});try{sessionStorage.removeItem(REVIEW_RETURN_KEY);}catch(_e){}}
  ...
  function renderProductProgress(data){
    ...
    if($('productResearchState'))$('productResearchState').textContent=(labels[status]||status)+' · 업체 '+Number(discovery.done||0)+'/'+Number(discovery.total||0)+' · 상품 점검 '+Number(inspection.done||0)+'/'+Number(inspection.total||0);
  ...
  function renderProducts(rows){
    ...
    grid.innerHTML=rows.length?rows.map(function(row){
      ...
      return'<article class="product-card"><div class="product-image-frame">'+image+'</div>'+video+'<div class="product-body"><strong class="product-title">'+esc(row.productName||row.title||'상품명 확인 필요')+'</strong><div class="small">판매·책임 업체: '+esc(row.supplierName||'-')+'</div><div class="small"><span class="pill '+(ready?'good':'warn')+'">'+esc(row.researchStatus||'미확인')+'</span> <span class="pill info">'+esc(decisionLabel)+'</span></div><div class="product-links">'+(linkParts.join('')||'<span class="small">확인 가능한 외부 링크가 없습니다.</span>')+'</div><div class="small mono">'+esc(productUrl||'상품 원본 주소 확인 필요')+'</div><div class="product-actions">'+actions+'</div></div></article>';
  ...
  async function runProductResearch(options){
    options=options||{};if(!selectedCountry||productLoopActive)return;var requested=scopeSnapshot(),requestedKey=scopeKey(requested),restart=!!(lastProductJson&&lastProductJson.status==='complete');if(restart&&!options.automatic&&!window.confirm('현재 완료된 상품 리서치 결과는 관리 DB에 보존되어 있습니다. 같은 국가 범위에서 새 상품 리서치를 시작하시겠습니까?'))return;if(restart&&options.automatic)return;
  ...
  async function loadScope(reason){
    ...
    try{
      ...
      var canWrite=!!(acceptedSession&&acceptedSession.session&&acceptedSession.session.write),autoProductReady=canWrite&&productData.status==='not_started'&&(researchData.status==='complete'||researchData.status==='committed')&&Array.isArray(researchData.candidates)&&researchData.candidates.length>0;
