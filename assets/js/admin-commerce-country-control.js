...
(function(){
  ...
  async function applyProductExclusionBulk(kind,decision){
    ...
    var labels={reject:'제외 관리로 이동',purge:'영구 제외',dismiss:'실제 삭제·재수집 허용',remove_from_list:'현재 관리 목록에서만 제거',undecided:'후보 목록으로 복원'};
    if(['reject','purge','dismiss','remove_from_list'].indexOf(decision)>=0&&!window.confirm('선택 '+ids.length+'건을 '+(labels[decision]||decision)+' 처리하시겠습니까?'+(decision==='dismiss'?'\n\n후보 원장과 관계 기록을 실제 삭제하며, 다음 공식 리서치에서 다시 발견될 수 있습니다.':decision==='purge'?'\n\n재수집 차단 기록은 유지됩니다.':'')))return;
    try{var selectedRows=ids.map(productById).filter(Boolean);await unmatchRowsBeforeManagementChange(selectedRows);var result=await managementCandidateBulk(ids,decision,'');var summary='요청 '+ids.length+'건 · 성공 '+Number(result.processed||0)+'건 · 실패 '+Number(result.failed||0)+'건';var suffix=decision==='purge'?' · 영구 제외·재수집 차단을 저장했습니다.':decision==='dismiss'?' · 후보 원장/관계를 삭제했습니다. 이후 리서치에서 재발견될 수 있습니다.':decision==='remove_from_list'?' · 원장을 보존하고 현재 관리 목록에서 숨겼습니다.':' · 관리자 후보 상태를 변경했습니다. 프론트 공개는 실행하지 않았습니다.';show(summary+suffix,result.failed?'warn':'ok');renderExcludedLedger();}
    ...
    try{
      if(unmatch||deleting||purging){
        ...
        for(var start=0;start<stageRows.length;start+=4){
          ...
          var selectedChunkRows=stageRows.slice(start,start+4),selectedChunkIds=selectedChunkRows.map(latestProductKey).filter(Boolean),selectedChunkRefs=latestRefsForRows(selectedChunkRows),selectedPass=0,selectedStalled=0,selectedRemaining=null;
      ...
      if(unmatch||deleting||purging||totalBlocked===0)selectedIds.forEach(function(id){delete latestSelectedProducts[id];});syncLatestProductQueueControls();
  ...
  async function managementCandidateBulk(ids,decision,placementKey){
    ...
    for(var off=0;off<ledger.length;off+=100){var chunk=ledger.slice(off,off+100).map(function(row){return text(row.candidateId||row.id);}).filter(Boolean);try{var result=decision==='dismiss'?await api(QUEUE_CONTROL,'bulk_action','POST',{}, {candidateIds:chunk,decision:'dismiss',countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE'},45000):decision==='undecided'?await authorityCandidateAction('undecided',chunk,'',{source:'administrator_restore'}):await api(CONTROL,'product_candidate_ledger_bulk_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',candidateIds:chunk,decision:decision,placementKey:placementKey||''},45000);processed+=Number(result&&result.processed||0);failed+=Number(result&&result.failed||result&&result.failures&&result.failures.length||0);(result&&result.failures||[]).slice(0,20).forEach(function(f){failures.push(f);});}catch(error){failed+=chunk.length;failures.push({error:text(error&&error.message)||'bulk_failed'});}}
