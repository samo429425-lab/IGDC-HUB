/* IGDC Global/Region/Country Commerce Control v3.11.0-3000-ledger-ai-execution
...
(function(){
  ...
  async function runCandidateBulkAction(groupKey,action){
    ...
    if((action==='hold'||action==='reject'||action==='list_delete')&&!window.confirm('선택한 후보 '+ids.length+'건을 '+(action==='hold'?'보류':action==='list_delete'?'현재 후보 목록에서 삭제·제외 관리로 이동':'제외')+' 처리합니다. 관리자 수동 결정으로 고정됩니다. 계속할까요?'))return;
    ...
    try{
      if(ledgerRows.length===selectedRows.length&&ledgerRows.length){
        ...
        var bulk=await api(CONTROL,'product_candidate_ledger_bulk_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',candidateIds:candidateIds,decision:action==='place'?'slot_candidate':action==='list_delete'?'reject':action,placementKey:placement||''},30000);
        ...
        for(var i=0;i<ids.length;i++){
          try{
            var itemData=await api(CONTROL,'product_candidate_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',productId:ids[i],decision:action==='place'?'slot_candidate':action==='list_delete'?'reject':action,placementKey:placement||''},30000);
            lastBulkData=itemData;succeeded+=1;
          }catch(itemError){failed+=1;}
        }
    ...
    var actionLabel=action==='place'?sectionLabel(placement)+' 섹션으로 수동 배치':action==='undecided'?'미배정 후보로 복원':action==='hold'?'보류':action==='list_delete'?'현재 후보 목록에서 삭제·제외 관리로 이동':'제외';var resultText='요청 '+ids.length+'건 · 성공 '+succeeded+'건 · 실패 '+failed+'건';if(succeeded)show(resultText+' · '+actionLabel+'했습니다. 자동 공개·결제는 실행하지 않았습니다.',failed?'warn':'ok');else show(resultText+' · 처리된 항목이 없습니다. 후보 상태·ID 또는 저장 권한을 확인해 주세요.','fail');
  }
  ...
  function managementOperationAllowed(workspaceKey,operation){
    ...
    if(op==='remove_from_list')return key==='whole_product_queue';
  ...
  async function executeManagementPreview(preview){
    ...
    renderQueue(queueCandidateRows);renderProducts(productRows);return{ok:failed===0,processed:processed,failed:failed,details:details,summary:'AI 승인 관리 실행 · 요청 대상 '+preview.totalTargets+'건 · 처리 '+processed+'건 · 실패 '+failed+'건 · 프론트 공개 없음'};
  }
