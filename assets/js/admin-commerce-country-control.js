/* IGDC Global/Region/Country Commerce Control v2.3.1
...
(function(){
  ...
  var selectedRegion='',selectedCountry='',selectedSubdivision='NATIONWIDE',regionOnly=true,lastJson=null,lastGlobalJson=null,lastRegionalJson=null,lastCountryJson=null,lastDiagnosticJson=null,selectionSource='initial',lastSignalReport=null;
  ...
  function invalidateJson(){lastJson=null;lastCountryJson=null;setDownloadEnabled(false);setButtonEnabled('countryRunDownloadBtn',false);$('jsonOutput').textContent='';$('jsonPanel').classList.add('hidden');}
  function savedScope(){
  ...
  async function runAutomation(dry){
    if(!selectedCountry)return;hide();var btn=dry?$('previewRunBtn'):$('runNowBtn'),requested=scopeSnapshot(),requestedKey=scopeKey(requested);btn.disabled=true;
    try{
      var data=await api(CONTROL,'run_now','POST',{}, {countryCode:requested.country,subdivisionCode:requested.region,dryRun:!!dry,force:true});
      if(scopeKey()!==requestedKey){invalidateJson();show('실행 중 선택 범위가 바뀌어 이전 결과를 폐기했습니다. 현재 범위에서 다시 실행해 주세요.','warn');return;}
      if(!data.scope)data.scope=requested;else{data.scope.source=requested.source;data.scope.detectedGeo=requested.detectedGeo;}
      rememberReport('country',data);renderSummary(data,{candidates:data.candidates||[]},dry?'현재 미리보기 결과':'현재 실행 결과');renderAi(data.candidates||[]);
      show(dry?'책임 공급업체 리서치 미리보기를 완료했습니다. 검색 수집·리서치 후보·부족 증빙을 확인할 수 있습니다.':'책임 공급업체 리서치를 실행했습니다. 현재 실행 결과와 JSON은 화면에 유지됩니다.',data.ok?'ok':'warn');
      if(!dry){try{await reloadData(false);await loadScope();}catch(_reloadError){}renderSummary(data,{candidates:data.candidates||[]},'현재 실행 결과');renderAi(data.candidates||[]);presentJson(data);}
    }catch(e){
      ...
      rememberReport('country',failure);show(text(e&&e.message)||'책임 공급업체 발굴에 실패했습니다. 오류 JSON을 내려받아 확인할 수 있습니다.','fail');
