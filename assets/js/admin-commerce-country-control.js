...
(function(){
  ...
  function setFrontSnapshotCount(key,count,source){if(!Object.prototype.hasOwnProperty.call(PRODUCT_SECTION_MAP,key))return;count=Math.max(0,Number(count)||0);frontSnapshotCounts[key]=count;frontSnapshotSource[key]=source||'';}
  ...
  async function refreshFrontSnapshotCounts(){
    ...
    PRODUCT_SECTIONS.forEach(function(row){frontSnapshotCounts[row.key]=0;frontSnapshotSource[row.key]='';});
    PRODUCT_SECTIONS.filter(function(row){return row.page==='home';}).forEach(function(row){setFrontSnapshotCount(row.key,countSnapshotSection(front,'home',row.section),'홈 프론트 스냅샷');});
    PRODUCT_SECTIONS.filter(function(row){return row.page==='distribution';}).forEach(function(row){var n=countSnapshotSection(distribution,'distribution',row.section);if(!n)n=countSnapshotSection(front,'distribution',row.section);setFrontSnapshotCount(row.key,n,'유통 프론트 스냅샷');});
    var networkCount=countSnapshotSection(network,'network','network-right');if(!networkCount)networkCount=countSnapshotSection(network,'','network-right');if(!networkCount)networkCount=snapshotTopItems(network).length;setFrontSnapshotCount('network|network-right',networkCount,'네트워크 프론트 스냅샷');
    setFrontSnapshotCount('social|rightPanel',countSnapshotSection(social,'social','rightPanel'),'소셜 프론트 스냅샷');
    var tourCount=countSnapshotSection(tour,'tour','tour');if(!tourCount)tourCount=snapshotSectionList(tour,'','right').length;setFrontSnapshotCount('tour|tour',tourCount,'투어 프론트 스냅샷');
