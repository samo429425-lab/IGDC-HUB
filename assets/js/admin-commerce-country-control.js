...
(function(){
  ...
  var MANAGEMENT_LEDGER_LIMIT=5000;
  var CANDIDATE_LEDGER_AI_BATCH=12;
  ...
  async function openSelectedQueueProductsInManager(){
    ...
    queueFocusedCandidateIds=ids.slice(0,MANAGEMENT_LEDGER_LIMIT);
  ...
  async function runSelectedQueueAiPlacement(){
    ...
    try{await refreshCandidateLedgerProducts({preserveResearchReport:true});}catch(_refreshError){}
    ids.forEach(function(id){candidateSelectedProducts[text(id)]=true;});
    await runCandidateLedgerAiPlacement(ids,'대기열 선택 상품 AI 자동 배치');
  }
  async function runSelectedQueueManualPlacement(){
    ...
    try{
      var result=await api(CONTROL,'product_candidate_ledger_bulk_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',candidateIds:ids.slice(0,MANAGEMENT_LEDGER_LIMIT),decision:'slot_candidate',placementKey:placement},45000);
  ...
  function syncFrontSelectionControls(){
    var busy=productLoopActive||productAutomationActive||productFrontSyncActive,matchBusy=busy,sections=selectedSectionKeys(),placed=productRows.filter(function(row){return productDecision(row)==='slot_candidate'&&!!assignedSectionKey(row);});
    ...
    setButtonEnabled('selectedSectionsMatchBtn',!matchBusy&&sections.length>0);setButtonEnabled('allSectionsMatchBtn',!matchBusy);setButtonEnabled('selectedSectionsUnmatchBtn',!busy&&sections.length>0);setButtonEnabled('allSectionsUnmatchBtn',!busy&&placed.length>0);
    ...
    PRODUCT_SECTIONS.forEach(function(section){var ids=selectedProductIds(section.key),rows=sectionRows(section.key),all=$('sectionProductSelectAll-'+section.key.replace(/[^a-z0-9_-]/gi,'_')),count=$('sectionProductSelectedCount-'+section.key.replace(/[^a-z0-9_-]/gi,'_'));if(all){all.checked=rows.length>0&&ids.length===rows.length;all.indeterminate=ids.length>0&&ids.length<rows.length;all.disabled=busy||!rows.length;}if(count)count.textContent='선택 상품 '+ids.length+'건';Array.prototype.forEach.call(document.querySelectorAll('[data-selected-products-front="'+section.key+'"]'),function(button){button.disabled=busy||!ids.length;});});
  }
  ...
  function productCardHtml(row,index,mode,groupKey){
    ...
    if(mode==='section'&&row.id&&decision==='slot_candidate')singleFront='<button type="button" class="product-section-unmatch-button" data-product-section-release="'+esc(row.id)+'">이 상품 섹션 해제</button><span class="small">이 버튼은 관리자 배치 예정에서 즉시 해제합니다. 기존 프론트 공개 제거는 다음 매칭/매칭 해제 실행에 반영됩니다.</span>';
  ...
  function productGroupDetails(key,label,rows,kind){
    ...
    if(kind==='section'&&validSection)autoButton='<button type="button" class="product-section-ai-button" data-section-ai-auto="'+esc(key)+'" '+(autoDisabled?'disabled':'')+'>AI 후보 보강(비공개)</button>';
    else if(kind==='candidate'&&validSection)autoButton='<button type="button" class="product-section-ai-button" data-section-ai-auto="'+esc(key)+'" '+(autoDisabled?'disabled':'')+'>AI 후보 재배치(비공개)</button>';
    ...
    else if(kind==='candidate'&&key==='unassigned')autoButton='<button type="button" class="product-section-ai-button" data-section-ai-auto="__all__" '+(autoDisabled||!count?'disabled':'')+'>미배정 AI 자동 배치(비공개)</button>';
    ...
    var frontButtons=kind==='section'&&validSection?'<button type="button" class="product-section-front-button" data-section-front-match="'+esc(key)+'" '+(matchBusy?'disabled':'')+'>이 섹션 프론트 매칭</button><button type="button" class="product-section-unmatch-button" data-section-front-unmatch="'+esc(key)+'" '+(controlsBusy?'disabled':'')+'>이 섹션 매칭 해제</button>':'';
    ...
    var candidateToolbar=kind==='candidate'?'<div class="section-product-toolbar candidate-bulk-toolbar"><label class="product-front-select-label"><input id="candidateSelectAll-'+esc(safeKey)+'" type="checkbox" data-candidate-select-all="'+esc(key)+'"> 이 후보군 전체 선택</label><span id="candidateSelectedCount-'+esc(safeKey)+'" class="pill info">선택 후보 0건</span><button type="button" class="product-section-ai-button" data-candidate-bulk="ai" data-candidate-group="'+esc(key)+'" disabled>선택 후보 AI 자동 배치(비공개)</button><select class="product-management-select candidate-target-select" data-candidate-target="'+esc(key)+'"><option value="">수동 배치 섹션 선택</option>'+PRODUCT_SECTIONS.map(function(section){return'<option value="'+esc(section.key)+'">'+esc(section.label)+'</option>';}).join('')+'</select><button type="button" class="product-section-front-button" data-candidate-bulk="place" data-candidate-group="'+esc(key)+'" disabled>선택 후보 수동 배치</button><button type="button" class="secondary" data-candidate-bulk="undecided" data-candidate-group="'+esc(key)+'" disabled>선택 후보 미배정 복원</button><button type="button" class="secondary" data-candidate-bulk="hold" data-candidate-group="'+esc(key)+'" disabled>선택 후보 보류</button><button type="button" class="danger" data-candidate-bulk="reject" data-candidate-group="'+esc(key)+'" disabled>선택 후보 제외</button><button type="button" class="danger" data-candidate-bulk="purge" data-candidate-group="'+esc(key)+'" disabled>선택 후보 영구 제외</button><button type="button" class="secondary" data-candidate-bulk="list_delete" data-candidate-group="'+esc(key)+'" disabled>선택 후보 목록에서만 제거</button></div>':'';
  ...
  function syncCandidateSelectionControls(groupKey){var rows=candidateGroupRows(groupKey),ids=selectedCandidateIds(groupKey),safeKey=groupKey.replace(/[^a-z0-9_-]/gi,'_'),all=$('candidateSelectAll-'+safeKey),count=$('candidateSelectedCount-'+safeKey),busy=productAutomationActive||productFrontSyncActive,target=document.querySelector('[data-candidate-target="'+groupKey+'"]'),targetKey=text(target&&target.value),selectedMap={};ids.forEach(function(id){selectedMap[id]=true;});if(all){all.checked=rows.length>0&&ids.length===rows.length;all.indeterminate=ids.length>0&&ids.length<rows.length;all.disabled=busy||!rows.length;}Array.prototype.forEach.call(document.querySelectorAll('[data-candidate-select]'),function(box){if(box.getAttribute('data-candidate-select')===groupKey)box.checked=selectedMap[text(box.value)]===true;});if(count)count.textContent='선택 후보 '+ids.length+'건';Array.prototype.forEach.call(document.querySelectorAll('[data-candidate-group="'+groupKey+'"]'),function(button){var action=button.getAttribute('data-candidate-bulk');button.disabled=busy||!ids.length||(action==='place'&&!PRODUCT_SECTION_MAP[targetKey]);});}
  async function runCandidateLedgerAiPlacement(idsInput,label){
    var ids=Array.from(new Set((Array.isArray(idsInput)?idsInput:[]).map(text).filter(Boolean))).slice(0,MANAGEMENT_LEDGER_LIMIT);
    if(!ids.length){show('AI 자동 배치할 상품 후보를 선택해 주세요.','warn');return{requested:0,processed:0,failed:0};}
    if(productAutomationActive||productFrontSyncActive){show('현재 다른 상품 배치·프론트 작업이 진행 중입니다. 완료 후 다시 실행해 주세요.','warn');return{requested:ids.length,processed:0,failed:ids.length};}
    productAutomationActive=true;renderProducts(productRows);
    var processed=0,failed=0,failures=[];
    try{
      for(var offset=0;offset<ids.length;offset+=CANDIDATE_LEDGER_AI_BATCH){
      ...
      await refreshCandidateLedgerProducts({preserveResearchReport:true});
      show((label||'선택 후보 AI 자동 배치')+' · 요청 '+ids.length+'건 · 처리 '+Math.max(0,processed)+'건 · 실패 '+failed+'건. 이 단계는 비공개 18개 섹션 배치만 갱신하며 프론트 공개는 실행하지 않습니다.',failed?'warn':'ok');
      return{requested:ids.length,processed:processed,failed:failed,failures:failures};
    }finally{productAutomationActive=false;renderProducts(productRows);syncFrontSelectionControls();}
  }
  async function runCandidateBulkAction(groupKey,action){
    ...
    if(action==='ai'){await runCandidateLedgerAiPlacement(ids,'선택 후보 AI 자동 배치');var stillVisible={};candidateGroupRows(groupKey).forEach(function(row){stillVisible[text(row.id)]=true;});ids.forEach(function(id){if(!stillVisible[id])delete candidateSelectedProducts[id];});saveReviewSnapshot();syncCandidateSelectionControls(groupKey);return;}
  ...
  async function runProductFrontSync(operation,mode,sectionKey,productId,selection){
    ...
    if(!targetIds.length&&!pendingManagement.length&&!replacementRun&&!(unmatch&&(sectionMode||sectionsMode||mode==='all'))){show(unmatch?'선택 범위에 매칭 해제할 상품이 없습니다.':'선택 범위에 최종 재검증·프론트 적용할 상품이 없습니다.','warn');return;}
