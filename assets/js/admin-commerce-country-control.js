...
(function(){
  ...
  var PRODUCT_SECTIONS=Object.freeze([
    {key:'home|home_1',page:'home',section:'home_1',label:'홈 · home_1'},{key:'home|home_2',page:'home',section:'home_2',label:'홈 · home_2'},{key:'home|home_3',page:'home',section:'home_3',label:'홈 · home_3'},{key:'home|home_4',page:'home',section:'home_4',label:'홈 · home_4'},{key:'home|home_5',page:'home',section:'home_5',label:'홈 · home_5'},{key:'home|home_right_top',page:'home',section:'home_right_top',label:'홈 · 우측 하단 (자동차·웹툰·패션)'},{key:'home|home_right_middle',page:'home',section:'home_right_middle',label:'홈 · 우측 중단 (푸드·리빙·책방)'},{key:'home|home_right_bottom',page:'home',section:'home_right_bottom',label:'홈 · 우측 상단 (지식·건강·기타)'},
  ...
  function productAutoRevenuePriority(row){var settlement=row&&row.affiliateSettlement||{},stage=affiliateStage(row),stageScore={formal_partner:4,online_affiliate_active:3,referral_verified:2,connection_required:0}[stage]||0,value=row&&row.valueAssessment||{},ranking=row&&row.ranking||{};return stageScore*100000+(settlement.settlementReady===true?50000:0)+Number(value.portfolioPriorityScore||row&&row.rankingScore||ranking.finalScore||0)*100+Number(value.revenue&&value.revenue.revenueCertaintyScore||ranking.revenueCertainty||0)*10+Number(value.components&&value.components.essentiality||ranking.essentiality||0);}
  ...
  function openSelectedQueueProductsInManager(){
    ...
    queueFocusedCandidateIds=ids.slice(0,3000);var panel=$('productCandidatePanel');if(panel){panel.classList.remove('hidden');panel.open=true;renderProductCandidateGroups();panel.scrollIntoView({behavior:'smooth',block:'start'});}show('선택한 대기열 상품 '+ids.length+'건을 상품 관리목록에서 표시했습니다. 이미 비공개 후보 원장에 있는 상품이므로 중복 수입하지 않았습니다.','ok');
  }
  ...
  function hydrateReviewSnapshot(){
    ...
    candidateSelectedProducts=Object.assign({},snapshot.candidateSelectedProducts||{});frontSelectedSections=Object.assign({},snapshot.frontSelectedSections||{});frontSelectedProducts=Object.assign({},snapshot.frontSelectedProducts||{});productPlacementSelections=Object.assign({},snapshot.productPlacementSelections||{});productPlacementDirty=Object.assign({},snapshot.productPlacementDirty||{});queueFocusedCandidateIds=Array.isArray(snapshot.queueFocusedCandidateIds)?snapshot.queueFocusedCandidateIds.map(text).filter(Boolean).slice(0,250):[];sessionResumeActive=true;sessionUiResume=snapshot;sessionDataResumeAvailable=!!memory;lastPassiveRestoreAt=Date.now();
  ...
  function productGroupDetails(key,label,rows,kind){
    ...
    var candidateToolbar=kind==='candidate'&&key!=='queue_selected'?'<div class="section-product-toolbar candidate-bulk-toolbar"><label class="product-front-select-label"><input id="candidateSelectAll-'+esc(safeKey)+'" type="checkbox" data-candidate-select-all="'+esc(key)+'"> 이 후보군 전체 선택</label><span id="candidateSelectedCount-'+esc(safeKey)+'" class="pill info">선택 후보 0건</span><button type="button" class="product-section-ai-button" data-candidate-bulk="ai" data-candidate-group="'+esc(key)+'" disabled>선택 후보 AI 자동 배치</button><select class="product-management-select candidate-target-select" data-candidate-target="'+esc(key)+'"><option value="">수동 배치 섹션 선택</option>'+PRODUCT_SECTIONS.map(function(section){return'<option value="'+esc(section.key)+'">'+esc(section.label)+'</option>';}).join('')+'</select><button type="button" class="product-section-front-button" data-candidate-bulk="place" data-candidate-group="'+esc(key)+'" disabled>선택 후보 수동 배치</button><button type="button" class="secondary" data-candidate-bulk="undecided" data-candidate-group="'+esc(key)+'" disabled>선택 후보 미배정 복원</button><button type="button" class="secondary" data-candidate-bulk="hold" data-candidate-group="'+esc(key)+'" disabled>선택 후보 보류</button><button type="button" class="danger" data-candidate-bulk="reject" data-candidate-group="'+esc(key)+'" disabled>선택 후보 제외</button><button type="button" class="danger" data-candidate-bulk="list_delete" data-candidate-group="'+esc(key)+'" disabled>선택 후보 목록 삭제</button></div>':'';
  ...
  async function runCandidateBulkAction(groupKey,action){
    ...
    if((action==='hold'||action==='reject'||action==='list_delete')&&!window.confirm('선택한 후보 '+ids.length+'건을 '+(action==='hold'?'보류':action==='list_delete'?'현재 후보 관리 목록에서 제거':'제외')+' 처리합니다. 프론트 공개·빌드는 실행하지 않습니다. 계속할까요?'))return;
    ...
    var actionLabel=action==='place'?sectionLabel(placement)+' 섹션으로 수동 배치':action==='undecided'?'미배정 후보로 복원':action==='hold'?'보류':action==='list_delete'?'현재 후보 관리 목록에서 제거':action==='reject'?'제외':action;
  ...
  async function runProductAiAutomation(mode,sectionKey,productIds,options){
    ...
    /* Top-level AI automation is a full portfolio refresh, not merely an
       "empty slots" filler.  Process it in 10-row safe batches so existing
       unlocked AI placements can be re-ranked against new revenue-capable
       candidates without creating one long serverless request.  Manual/admin
       locks and active Front publication locks are still preserved server-side. */
    var fullRebalanceBatch=!sectionMode&&!repairMode&&!selectedMode&&mode==='all';
    var batchPlacement=!sectionMode&&!repairMode&&selectedMode&&selectedAllUndecided;
    var batchTargets=fullRebalanceBatch?productRows.filter(function(row){var d=productDecision(row);return !productManagementLocked(row)&&(d==='undecided'||d==='slot_candidate'||d==='hold');}).slice().sort(function(a,b){return productAutoRevenuePriority(b)-productAutoRevenuePriority(a)||text(a.id).localeCompare(text(b.id));}).map(function(row){return text(row.id);}).filter(Boolean):(batchPlacement?selectedIds:[]);
    if((fullRebalanceBatch||batchPlacement)&&!batchTargets.length){show(fullRebalanceBatch?'현재 AI 재평가할 상품 후보가 없습니다.':'현재 미배정 후보 중 AI 자동 배치할 상품이 없습니다.','warn');return;}
    ...
    try{
      ...
      if(fullRebalanceBatch||batchPlacement){
        var batchSize=10,assignedTotal=0,pendingTotal=0,heldTotal=0,changedTotal=0,manualPreserved=0;autoFinalizeDraft=false;aiRunToken=batchPlacement?('ai_draft_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,10)):'';
        for(var off=0;off<batchTargets.length;off+=batchSize){
          var batch=batchTargets.slice(off,off+batchSize);
          if(state){state.className='product-auto-state running';state.textContent=label+(fullRebalanceBatch?' 수익·정책 재정렬 ':' 정책 임시 정리 ')+Math.min(off+batch.length,batchTargets.length)+'/'+batchTargets.length+'건 · 10건 단위 안전 처리';}
          data=await api(CONTROL,'product_ai_automation','POST',{}, fullRebalanceBatch?{countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',mode:'rebalance_batch',productIds:batch,batchSize:batchSize,compactResponse:true}:{countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',mode:'placement_batch',productIds:batch,batchSize:batchSize,deferQueueSync:true,aiRunToken:aiRunToken,targetTotal:batchTargets.length,draftMode:'manual',automaticFinalize:false});
          var partResult=data&&data.aiAutomationResult||{};
          assignedTotal+=Number(partResult.assigned||0);pendingTotal+=Number(partResult.pendingEvidenceAssigned||0);heldTotal+=Number(partResult.held||0);changedTotal+=Number(partResult.changed||0);manualPreserved=Math.max(manualPreserved,Number(partResult.manualPreserved||0));queueRemaining=Number(partResult.temporaryPendingItems||partResult.queueSyncRemaining||0);if(fullRebalanceBatch){queueSyncedTotal+=Number(partResult.queueSynced||0);queueFailedTotal+=Number(partResult.queueSyncFailed||0);}temporaryCompleted=Number(partResult.temporaryCompletedItems||temporaryCompleted+batch.length);
        ...
        if(fullRebalanceBatch){
          if(state){state.className='product-auto-state complete';state.textContent=label+' 수익·정책 재정렬 '+temporaryCompleted+'건 완료 · 관리자/공개 고정 상품은 보존했습니다.';}
        ...
        result={assigned:assignedTotal,pendingEvidenceAssigned:pendingTotal,unassigned:globalUnassigned,held:heldTotal,manualPreserved:manualPreserved,changed:changedTotal,settlementReady:Number(sm.settlementReadyProducts||0),queueSynced:queueSyncedTotal,queueSyncFailed:queueFailedTotal,queueSyncRemaining:queueRemaining,temporaryCompletedItems:temporaryCompleted,temporary:batchPlacement,rebalancedExistingAiPlacements:fullRebalanceBatch};
      }else{
  ...
  function pendingProductManagementChanges(mode,sectionKey,productId,selection){
    var selectedSections=new Set((mode==='sections'&&Array.isArray(selection)?selection:[]).map(text)),selectedProducts=new Set((mode==='candidates'&&Array.isArray(selection)?selection:[]).map(text)),changes=[];
    Object.keys(productPlacementDirty||{}).forEach(function(id){
      if(productPlacementDirty[id]!==true)return;var row=productById(id),value=text(productPlacementSelections[id]);if(!row||!value||value.indexOf('affiliate:')===0)return;
      var currentKey=assignedSectionKey(row),desiredKey=value.indexOf('section:')===0?value.slice(8):'',include=mode==='all';
      if(mode==='section')include=currentKey===sectionKey||desiredKey===sectionKey;
      else if(mode==='sections')include=selectedSections.has(currentKey)||selectedSections.has(desiredKey);
      else if(mode==='candidate')include=text(id)===text(productId);
      else if(mode==='candidates')include=selectedProducts.has(text(id));
      if(!include)return;
      var decision=value.indexOf('section:')===0?'slot_candidate':value;
      if(['slot_candidate','hold','reject','purge','undecided','ai_reclassify'].indexOf(decision)<0)return;
      changes.push({id:text(id),candidateId:text(row&&row.candidateId||row&&row.id),row:row,value:value,decision:decision,placementKey:decision==='slot_candidate'?desiredKey:'',currentKey:currentKey,desiredKey:desiredKey});
    });
    return changes;
  }
  async function persistPendingProductManagementChanges(changes){
    changes=Array.isArray(changes)?changes:[];if(!changes.length)return{changedCount:0,changedCandidateIds:[],placementProductIds:[],placementCandidateIds:[]};
    var groups={},changedCandidateIds=[],placementProductIds=[],placementCandidateIds=[];
    changes.forEach(function(change){
      if(change.decision==='purge'&&frontPublicationActive(change.row)){var e=new Error('현재 프론트에 매칭된 상품은 먼저 매칭 해제한 뒤 영구 제외할 수 있습니다.');e.status=409;throw e;}
      var key=change.decision+'|'+(change.placementKey||'');if(!groups[key])groups[key]={decision:change.decision,placementKey:change.placementKey||'',ids:[],changes:[]};groups[key].ids.push(change.id);groups[key].changes.push(change);
    });
    var keys=Object.keys(groups);
    for(var i=0;i<keys.length;i++){
      var group=groups[keys[i]],result=await managementCandidateBulk(group.ids,group.decision,group.placementKey);
      if(Number(result&&result.failed||0)>0){var err=new Error('관리자 수정사항 저장 중 '+Number(result.failed||0)+'건이 실패했습니다. 프론트 실행을 중단하고 저장되지 않은 수정사항을 그대로 유지합니다.');err.status=409;err.failures=result&&result.failures||[];throw err;}
      group.changes.forEach(function(change){
        delete productPlacementDirty[change.id];
        if(change.candidateId)changedCandidateIds.push(change.candidateId);
        if(change.decision==='slot_candidate'){placementProductIds.push(change.id);if(change.candidateId)placementCandidateIds.push(change.candidateId);}
      });
    }
    saveReviewSnapshot();
    return{changedCount:changes.length,changedCandidateIds:Array.from(new Set(changedCandidateIds)),placementProductIds:Array.from(new Set(placementProductIds)),placementCandidateIds:Array.from(new Set(placementCandidateIds))};
  }
  async function runProductFrontSync(operation,mode,sectionKey,productId,selection){
    ...
    var pendingChanges=unmatch?[]:pendingProductManagementChanges(mode,sectionKey,productId,selection),selectedProductMap={};selectedProducts.forEach(function(id){selectedProductMap[text(id)]=true;});
    var previewTargetRows=productRows.filter(function(row){var key=assignedSectionKey(row),id=text(row&&row.id),placed=productDecision(row)==='slot_candidate'&&!!key;if(!key)return false;var eligible=unmatch?(placed&&frontPublicationActive(row)):(placed||frontPublicationLive(row));if(!eligible)return false;if(candidateMode)return id===text(productId);if(candidatesMode)return selectedProductMap[id]===true;if(sectionMode)return key===sectionKey;if(sectionsMode)return selectedSections.indexOf(key)>=0;return true;});
    if(!previewTargetRows.length&&!pendingChanges.length){show(unmatch?'선택 범위에 매칭 해제할 상품이 없습니다.':'선택 범위에 최종 재검증·프론트 적용할 상품이 없습니다.','warn');return;}
    var explanation=unmatch?label+'의 기존 프론트 매칭 해제를 요청합니다.':label+'의 현재 배치 상품을 프론트 공개 원장에 적용합니다.'+(pendingChanges.length?' 화면에서 수정한 '+pendingChanges.length+'건을 먼저 관리자 원장에 저장한 뒤 최신 원장을 다시 읽어 실행합니다.':'')+' 최근 검증이 살아 있는 상품은 저장된 검증을 재사용하고, 오래됐거나 실제 공개 중인 상품만 다시 확인합니다. 삭제·잘못된 접근·판매처 홈 리다이렉트 상품은 제거하고 대체 후보가 없으면 기존 샘플 fallback을 유지합니다.';
    if(!window.confirm(explanation+'\n\n'+(unmatch?'프론트에서 해당 매칭을 해제하시겠습니까?\n변경은 소량 묶음으로 저장한 뒤 마지막에 스냅샷 빌드를 1회만 실행합니다.':'프론트 실상품 매칭 절차를 실행하시겠습니까?\n화면 수정사항을 먼저 확정 저장하고, 그 저장 결과를 기준으로 마지막 빌드 1회로 반영합니다.')))return;
    productFrontSyncActive=true;
    var state=$('productFrontSyncState'),batchSize=6,maxConcurrent=2,requestTimeout=75000,aggregate={requested:0,queued:0,persisted:0,pendingBuild:0,blocked:0,revalidated:0,remoteChecked:0,freshReused:0,invalid:0,inconclusive:0,withdrawn:0,changedSection:0,items:[],persistedCandidateIds:[],changedCandidateIds:[],preparation:{requested:0,prepared:0,blocked:0},batchErrors:[]},publishPersistedMap={},changedMap={},completed=0,pendingCommit={changedCount:0,changedCandidateIds:[],placementProductIds:[],placementCandidateIds:[]},targetRows=[],targetIds=[];
    try{
      if(pendingChanges.length){if(state){state.className='front-sync-state running';state.textContent=label+' · 화면 수정 '+pendingChanges.length+'건을 관리자 원장에 먼저 저장하는 중';}pendingCommit=await persistPendingProductManagementChanges(pendingChanges);await refreshCandidateLedgerProducts({preserveResearchReport:true});}
      var forcedPlacementMap={};(pendingCommit.placementProductIds||[]).forEach(function(id){forcedPlacementMap[text(id)]=true;});(pendingCommit.placementCandidateIds||[]).forEach(function(id){forcedPlacementMap[text(id)]=true;});
      targetRows=productRows.filter(function(row){
        var key=assignedSectionKey(row),id=text(row&&row.id),candidateId=text(row&&row.candidateId||row&&row.id),placed=productDecision(row)==='slot_candidate'&&!!key,forced=forcedPlacementMap[id]===true||forcedPlacementMap[candidateId]===true;
        if(!key)return false;var eligible=unmatch?(placed&&frontPublicationActive(row)):(placed||frontPublicationLive(row));if(!eligible)return false;if(forced&&!unmatch)return true;if(candidateMode)return id===text(productId);if(candidatesMode)return selectedProductMap[id]===true;if(sectionMode)return key===sectionKey;if(sectionsMode)return selectedSections.indexOf(key)>=0;return true;
      });
      targetIds=Array.from(new Set(targetRows.map(function(row){return text(row&&row.candidateId||row&&row.id);}).filter(Boolean)));
      (pendingCommit.changedCandidateIds||[]).forEach(function(id){if(id)changedMap[text(id)]=true;});
      if(!targetIds.length&&Number(pendingCommit.changedCount||0)<=0){show(unmatch?'선택 범위에 매칭 해제할 상품이 없습니다.':'저장 후 다시 확인한 결과 프론트에 적용할 상품이 없습니다.','warn');return;}
      if(state){state.className='front-sync-state running';state.textContent=label+' '+(unmatch?'매칭 해제':'프론트 실상품 매칭')+' · '+targetIds.length+'건을 처리합니다.'+(pendingCommit.changedCount?' 화면 수정 '+pendingCommit.changedCount+'건 저장 완료.':'')+' 최근 정상 검증은 재사용하고 필요한 상품만 다시 확인합니다.';}
      ...
      var needsFinalize=aggregate.persistedCandidateIds.length>0||aggregate.changedCandidateIds.length>0||aggregate.withdrawn>0||Number(pendingCommit.changedCount||0)>0;
      if(needsFinalize){
        try{
      ...
      var result=aggregate,requested=Math.max(Number(result.requested||0),Number(pendingCommit.changedCount||0)),displayTotal=Math.max(targetIds.length,Number(pendingCommit.changedCount||0)),prepared=Number(result.preparation&&result.preparation.prepared||0),queued=Number(result.queued||0),persisted=Number(result.persisted||0),pendingBuild=Number(result.pendingBuild||0),blocked=Number(result.blocked||0),reasonCounts={};
      ...
      if(state){state.className='front-sync-state '+(batchErrorCount||blocked||pendingBuild?'warn':'complete');state.textContent=label+' · 요청 '+requested+'/'+displayTotal+'건 · 원장 변경 '+result.changedCandidateIds.length+'건 · 빌드 호출 '+queued+'건 · 차단 '+blocked+'건'+detail+reasonText+(batchErrorCount?' · 묶음 오류 '+batchErrorCount+'건(나머지는 계속 처리됨)':'');}
      var message=(unmatch?'프론트 매칭 해제':'프론트 실상품 갱신 적용')+' '+requested+'/'+displayTotal+'건 처리 · 검증 재사용 '+Number(result.freshReused||0)+'건 · 실제 재확인 '+Number(result.remoteChecked||0)+'건 · 삭제/불량 '+Number(result.invalid||0)+'건 · 기존 매칭 제거 '+Number(result.withdrawn||0)+'건 · 원장 변경 '+result.changedCandidateIds.length+'건.';
