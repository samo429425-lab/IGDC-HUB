...
(function(){
  ...
  async function runLatestProductQueueAction(operation,scopeMode,options){
    ...
    latestQueueActionActive=true;syncLatestProductQueueControls();var totalHandled=0,totalFailed=0,totalBlocked=0,totalEligible=0,blockedReasonSamples=[],deletedUiKeys={},state=$('productQueueStageState'),manualState=$('latestManualActionState'),requestedScope=scopeSnapshot(),requestedKey=scopeKey(requestedScope);
    try{
      if(unmatch||deleting||purging){
        ...
        for(var start=0;start<stageRows.length;start+=4){
          ...
          while(selectedPass<24){
            if(scopeKey()!==requestedKey)throw new Error('대기열 작업 중 선택 범위가 변경됐습니다.');
            if(state)state.textContent=stageLabel+' 후보 관리목록 등록 · '+Math.min(start+selectedChunkRows.length,stageRows.length)+'/'+stageRows.length+'건 · 10건 단위 저장';
      ...
      var finalText=(purging?'최신 상품 영구 제외':(deleting?'최신 상품 목록 삭제':(unmatch?'최신 상품 후보 등록 해제':'최신 상품 후보 관리목록 등록')))+' 완료 · '+(allMode?'현재 리서치 전체':'선택 '+ids.length+'건')+' · 반영 '+totalHandled+'건'+(totalBlocked?' · 검증 보류 '+totalBlocked+'건':'')+(totalFailed?' · 실패 '+totalFailed+'건':'');if(!unmatch&&!deleting&&!purging&&totalHandled===0&&totalBlocked>0)finalText+=' · 선택 상품은 재검증했지만 실제 상품 상세페이지 조건을 아직 통과하지 못했습니다.';if(blockedReasonSamples.length)finalText+=' · 보류 사유 '+blockedReasonSamples.join(', ');if(state)state.textContent=finalText;if(manualState)manualState.textContent=finalText;
      var finalMessage=purging?'선택 상품을 영구 제외했습니다. 같은 상품은 이후 자동 리서치에서 다시 후보로 승격하지 않습니다.':deleting?'선택 상품을 현재 최신 리서치 목록에서 삭제했습니다.':unmatch?'선택 상품의 후보 등록을 해제했습니다.':totalHandled?'최신 상품 '+totalHandled+'건을 전체 상품 후보·배치 관리 목록에 반영했습니다.':totalBlocked?'선택 상품 '+totalBlocked+'건을 재검증했지만 실제 상품 상세페이지 확인 조건을 통과하지 못해 후보 등록을 보류했습니다.':'선택 상품 후보 등록 결과 반영된 항목이 없습니다.';if(totalBlocked&&deleting)finalMessage+=' · 후속 배치·공개·관리자 고정 '+totalBlocked+'건은 그대로 보존했습니다.';if(blockedReasonSamples.length)finalMessage+=' · 보류 사유: '+blockedReasonSamples.join(', ');show(finalMessage,(totalFailed||totalBlocked)?'warn':'ok');return{ok:totalFailed===0,handled:totalHandled,failed:totalFailed,blocked:totalBlocked,eligible:totalEligible,candidateIds:typeof stagedCandidateIds==='object'?Object.keys(stagedCandidateIds):[]};
  ...
  async function runLatestAi(scopeMode){
    ...
    var candidateIds=Array.from(new Set((Array.isArray(staged.candidateIds)?staged.candidateIds:[]).concat(candidateIdsForLatestRows(rows)).map(text).filter(Boolean)));if(!candidateIds.length){show((allMode?'최신 상품':'선택 상품')+' 중 비공개 후보 관리목록 등록이 확인된 후보가 없습니다. 대기열·배치 전환 검증으로 연결 상태를 확인해 주세요.','warn');return;}
