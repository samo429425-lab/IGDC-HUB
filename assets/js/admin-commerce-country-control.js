/* IGDC Global/Region/Country Commerce Control v3.13.0-fast-ledger-runtime-diagnostic
...
(function(){
  ...
  async function runtimeDiagnostic(){
    var button=$('runtimeDiagnosticBtn'),status=$('runtimeDiagnosticState');if(button)button.disabled=true;if(status)status.textContent='Netlify 런타임·관리 DB 응답 시간을 점검 중입니다.';
    try{
      var data=await api(RUNTIME_DIAGNOSTIC,'probe','GET',{country:selectedCountry||'',region:selectedSubdivision||'NATIONWIDE'},null,25000);
      lastJson=data;setDownloadEnabled(true);if($('jsonOutput'))$('jsonOutput').textContent=JSON.stringify(data,null,2);if($('jsonPanel'))$('jsonPanel').classList.remove('hidden');
      ...
      if(status)status.textContent='Netlify 런타임 점검 완료 · 배포 '+text(data.deploy&&data.deploy.commitRef||'-').slice(0,10)+' · DB 점검 '+probes.length+'개 · 지연/오류 '+slow.length+'개';
      show(slow.length?'Netlify/관리 DB 점검에서 지연 또는 오류 '+slow.length+'개를 확인했습니다. 아래 JSON을 다운로드해 전달해 주세요.':'Netlify 런타임과 관리 DB 기본 경로가 정상 응답했습니다. 아래 JSON에 각 단계 시간이 기록됐습니다.',slow.length?'warn':'ok');
    }catch(error){if(status)status.textContent='Netlify 런타임 점검 실패 · '+conciseRequestError(error&&error.message,error&&error.status);show(status?status.textContent:'Netlify 런타임 점검에 실패했습니다.','fail');}
    finally{if(button)button.disabled=false;}
  }
