/* IGDC Global/Region/Country Commerce Control v2.4.0
...
(function(){
  ...
  var selectedRegion='',selectedCountry='',selectedSubdivision='NATIONWIDE',regionOnly=true,lastJson=null,lastGlobalJson=null,lastRegionalJson=null,lastCountryJson=null,lastDiagnosticJson=null,selectionSource='initial',lastSignalReport=null,lastCountryPreview=null;
  ...
  function invalidateJson(){lastJson=null;lastCountryJson=null;lastCountryPreview=null;setDownloadEnabled(false);setButtonEnabled('countryRunDownloadBtn',false);setButtonEnabled('runNowBtn',false);$('jsonOutput').textContent='';$('jsonPanel').classList.add('hidden');}
  function savedScope(){
  ...
  async function loadScope(){if(!selectedCountry)return;hide();$('refreshScopeBtn').disabled=true;var requested=scopeParams(),requestedKey=scopeKey(requested);try{var all=await Promise.all([api(REVIEW,'summary','GET',requested),api(REVIEW,'candidates','GET',requested),api(CONTROL,'scope','GET',requested)]);if(scopeKey()!==requestedKey)return;renderSummary(all[0].summary||{},all[2],'저장 원장 기준');renderQueue(all[1].candidates||[]);renderAi(all[2].candidates||[]);renderActiveSignals(all[2].marketSignals||{});show(scopeName()+' 범위를 읽었습니다.','ok');}catch(e){if(scopeKey()===requestedKey)show(text(e.message)||'범위를 읽지 못했습니다.','fail');}finally{$('refreshScopeBtn').disabled=false;}}
  async function runAutomation(dry){
    if(!selectedCountry)return;
    if(dry!==true){show('먼저 책임 공급업체 검색으로 후보 목록을 만든 뒤 검색 결과 실행을 눌러 주세요.','warn');return;}
    hide();var btn=$('previewRunBtn'),requested=scopeSnapshot(),requestedKey=scopeKey(requested);btn.disabled=true;setButtonEnabled('runNowBtn',false);lastCountryPreview=null;
    try{
      var data=await api(CONTROL,'run_now','POST',{}, {countryCode:requested.country,subdivisionCode:requested.region,dryRun:true,force:true});
      if(scopeKey()!==requestedKey){invalidateJson();show('검색 중 선택 범위가 바뀌어 이전 결과를 폐기했습니다. 현재 범위에서 다시 검색해 주세요.','warn');return;}
      if(!data.scope)data.scope=requested;else{data.scope.source=requested.source;data.scope.detectedGeo=requested.detectedGeo;}
      lastCountryPreview=data;
      rememberReport('country',data);
      renderSummary(data,{candidates:data.candidates||[]},'현재 검색 결과');
      renderAi(data.candidates||[]);
      setButtonEnabled('runNowBtn',Array.isArray(data.candidates)&&data.candidates.length>0);
      show((data.candidates||[]).length?'책임 공급업체 검색을 완료했습니다. 목록과 JSON을 확인한 뒤 검색 결과 실행을 눌러 비공개 후보 원장에 저장하세요.':'검색은 완료됐지만 실행할 후보 목록이 없습니다. JSON의 수집·검토 단계와 오류를 확인해 주세요.',(data.candidates||[]).length?'ok':'warn');
    }catch(e){
      var failure=e&&e.payload&&typeof e.payload==='object'?e.payload:{ok:false,reportType:'igdc-country-responsible-supplier-discovery-error',generatedAt:new Date().toISOString(),scope:requested,error:text(e&&e.message),code:text(e&&e.code)};
      lastCountryPreview=null;setButtonEnabled('runNowBtn',false);rememberReport('country',failure);show(text(e&&e.message)||'책임 공급업체 검색에 실패했습니다. 오류 JSON을 내려받아 확인할 수 있습니다.','fail');
  ...
  async function commitCountryPreview(){
    ...
    try{
      var data=await api(CONTROL,'commit_preview','POST',{}, {countryCode:requested.country,subdivisionCode:requested.region,candidates:lastCountryPreview.candidates,sourceRunId:lastCountryPreview.runId||null,sourceStartedAt:lastCountryPreview.startedAt||null});
