/* IGDC Global/Region/Country Commerce Control v3.10.11-explicit-research-queue
...
(function(){
  ...
  function invalidateJson(){closeManagementPanels();closeSupplierControlQueue();supplierControlScopeKey='';closeQueuePanel();queueControlScopeKey='';queueCandidateRows=[];queueFocusedCandidateIds=[];supplierActiveRows=[];supplierHoldingRows=[];supplierBlockedRows=[];productRows=[];latestProductRows=[];latestSelectedProducts={};latestQueueActionActive=false;latestRefreshActive=false;productPlacementSelections={};frontSelectedSections={};frontSelectedProducts={};researchStopRequested=true;productStopRequested=true;productPausedForQueue=false;productPauseServerConfirmed=false;lastCountryJson=null;lastCountryPreview=null;lastProductJson=null;lastScopeCache=null;lastScopeAuditJson=null;activeScopeAudit='';if($('scopeAuditPanel'))$('scopeAuditPanel').classList.add('hidden');setButtonEnabled('countryRunDownloadBtn',false);setButtonEnabled('runNowBtn',false);setButtonEnabled('productResearchDownloadBtn',false);resetJsonPanelForScope();if($('countryResearchProgress'))$('countryResearchProgress').textContent='선택 범위가 바뀌었습니다. 새 범위의 저장 작업 상태를 읽어 주세요.';if($('productResearchState'))$('productResearchState').textContent='선택 범위가 바뀌었습니다. 이 범위의 상품 리서치 상태를 다시 읽어 주세요.';}
  function savedScope(){
  ...
  function openSelectedQueueProductsInManager(){
    var ids=selectedQueueIds();if(!ids.length){show('상품 관리목록에서 확인할 대기열 상품을 선택해 주세요.','warn');return;}
    queueFocusedCandidateIds=ids.slice(0,250);var panel=$('productCandidatePanel');if(panel){panel.classList.remove('hidden');panel.open=true;renderProductCandidateGroups();panel.scrollIntoView({behavior:'smooth',block:'start'});}show('선택한 대기열 상품 '+ids.length+'건을 상품 관리목록에서 표시했습니다. 이미 비공개 후보 원장에 있는 상품이므로 중복 수입하지 않았습니다.','ok');
  }
  function queueActionMessage(action,count){
    ...
    if(action==='purge')return '선택 '+count+'건을 영구 제외합니다. 원장에는 제외 기록을 남겨 이후 자동 리서치가 같은 후보를 다시 올리지 않도록 보존합니다. 계속하시겠습니까?';
  ...
  function syncFrontSelectionControls(){
    ...
    var master=$('allSectionsSelect');if(master){master.checked=sections.length===PRODUCT_SECTIONS.length;master.indeterminate=sections.length>0&&sections.length<PRODUCT_SECTIONS.length;master.disabled=busy||placed.length===0;}
    setButtonEnabled('selectedSectionsMatchBtn',!matchBusy&&sections.some(function(key){return sectionRows(key).some(function(row){return productDecision(row)==='slot_candidate'||frontPublicationActive(row);});}));setButtonEnabled('allSectionsMatchBtn',!matchBusy&&refreshable.length>0);setButtonEnabled('selectedSectionsUnmatchBtn',!busy&&sections.some(function(key){return sectionRows(key).some(frontPublicationActive);}));setButtonEnabled('allSectionsUnmatchBtn',!busy&&active.length>0);
  ...
  function productCardHtml(row,index,mode,groupKey){
    ...
    if(mode==='section'&&row.id&&decision==='slot_candidate')singleFront='<button type="button" class="product-section-front-button" data-product-front-match="'+esc(row.id)+'" '+(productFrontSyncActive?'disabled':'')+'>이 상품만 프론트 매칭</button><button type="button" class="product-section-unmatch-button" data-product-front-unmatch="'+esc(row.id)+'" '+(productFrontSyncActive||!frontPublicationActive(row)?'disabled':'')+'>이 상품만 매칭 해제</button>';
  ...
  function productGroupDetails(key,label,rows,kind){
    var overflow=Math.max(0,rows.length-100),count=rows.length,validSection=!!PRODUCT_SECTION_MAP[key],capacity=kind==='section'?'<span class="product-section-capacity '+(count>=100?'warn':'')+'">'+count+'/100</span>':'<span class="small">'+count+'건</span>';
    ...
    var sectionSelect=kind==='section'&&validSection?'<label class="section-select-label"><input type="checkbox" data-section-front-select="1" value="'+esc(key)+'" '+(frontSelectedSections[key]===true?'checked ':'')+(controlsBusy||!count?'disabled ':'')+'> 섹션 선택</label>':'',frontButtons=kind==='section'&&validSection?'<button type="button" class="product-section-front-button" data-section-front-match="'+esc(key)+'" '+(matchBusy||!count?'disabled':'')+'>이 섹션 프론트 매칭</button><button type="button" class="product-section-unmatch-button" data-section-front-unmatch="'+esc(key)+'" '+(controlsBusy||!rows.some(frontPublicationActive)?'disabled':'')+'>이 섹션 매칭 해제</button>':'',policyButton=validSection?'<button type="button" class="ghost policy-workspace-button" data-policy-workspace="section_'+esc(key.replace(/[^a-z0-9_-]/gi,'_'))+'" data-policy-workspace-label="'+esc(label)+' 섹션 운영 대화" data-policy-scope="country">AI 운영·정책 대화</button>':'';
    var safeKey=key.replace(/[^a-z0-9_-]/gi,'_'),candidateToolbar=kind==='candidate'&&key!=='queue_selected'?'<div class="section-product-toolbar candidate-bulk-toolbar"><label class="product-front-select-label"><input id="candidateSelectAll-'+esc(safeKey)+'" type="checkbox" data-candidate-select-all="'+esc(key)+'"> 이 후보군 전체 선택</label><span id="candidateSelectedCount-'+esc(safeKey)+'" class="pill info">선택 후보 0건</span><button type="button" class="product-section-ai-button" data-candidate-bulk="ai" data-candidate-group="'+esc(key)+'" disabled>선택 후보 AI 자동 배치</button><select class="product-management-select candidate-target-select" data-candidate-target="'+esc(key)+'"><option value="">수동 배치 섹션 선택</option>'+PRODUCT_SECTIONS.map(function(section){return'<option value="'+esc(section.key)+'">'+esc(section.label)+'</option>';}).join('')+'</select><button type="button" class="product-section-front-button" data-candidate-bulk="place" data-candidate-group="'+esc(key)+'" disabled>선택 후보 수동 배치</button><button type="button" class="secondary" data-candidate-bulk="undecided" data-candidate-group="'+esc(key)+'" disabled>선택 후보 미배정 복원</button><button type="button" class="secondary" data-candidate-bulk="hold" data-candidate-group="'+esc(key)+'" disabled>선택 후보 보류</button><button type="button" class="danger" data-candidate-bulk="reject" data-candidate-group="'+esc(key)+'" disabled>선택 후보 제외</button></div>':'',productToolbar=kind==='section'&&validSection?'<div class="section-product-toolbar"><label class="product-front-select-label"><input id="sectionProductSelectAll-'+esc(safeKey)+'" type="checkbox" data-section-products-select-all="'+esc(key)+'"> 이 섹션 상품 전체 선택</label><span id="sectionProductSelectedCount-'+esc(safeKey)+'" class="pill info">선택 상품 0건</span><button type="button" class="product-section-front-button" data-selected-products-front="'+esc(key)+'" data-front-operation="match" disabled>선택 상품 매칭</button><button type="button" class="product-section-unmatch-button" data-selected-products-front="'+esc(key)+'" data-front-operation="unmatch" disabled>선택 상품 매칭 해제</button><button type="button" class="product-section-front-button" data-section-front-match="'+esc(key)+'" '+(matchBusy||!count?'disabled':'')+'>이 섹션 전체 상품 매칭</button><button type="button" class="product-section-unmatch-button" data-section-front-unmatch="'+esc(key)+'" '+(controlsBusy||!rows.some(frontPublicationActive)?'disabled':'')+'>이 섹션 전체 상품 매칭 해제</button></div>':'';
    return'<details class="product-group" data-product-group="'+esc(key)+'" data-product-group-kind="'+esc(kind)+'"><summary><span>'+sectionSelect+' <strong>'+esc(label)+'</strong> '+capacity+'</span><span class="product-group-summary-actions">'+policyButton+autoButton+frontButtons+'<span class="pill info" data-group-toggle-label>목록 펼치기</span></span></summary><div class="product-group-body">'+candidateToolbar+productToolbar+'<div class="product-group-grid"></div>'+(overflow?'<div class="product-group-overflow">이 섹션은 화면 안정화를 위해 상위 100개만 표시합니다. 나머지 '+overflow+'건은 순위·중복 정리 후 다음 관리 묶음에서 확인합니다.</div>':'')+'</div></details>';}
  ...
  function candidateGroupRows(groupKey){return productRows.filter(candidatePoolRow).filter(function(row){return candidateGroupKey(row)===groupKey;});}
  ...
  function syncCandidateSelectionControls(groupKey){var rows=candidateGroupRows(groupKey),ids=selectedCandidateIds(groupKey),safeKey=groupKey.replace(/[^a-z0-9_-]/gi,'_'),all=$('candidateSelectAll-'+safeKey),count=$('candidateSelectedCount-'+safeKey),busy=productAutomationActive||productFrontSyncActive||productLoopActive,selectedMap={};ids.forEach(function(id){selectedMap[id]=true;});if(all){all.checked=rows.length>0&&ids.length===rows.length;all.indeterminate=ids.length>0&&ids.length<rows.length;all.disabled=busy||!rows.length;}Array.prototype.forEach.call(document.querySelectorAll('[data-candidate-select]'),function(box){if(box.getAttribute('data-candidate-select')===groupKey)box.checked=selectedMap[text(box.value)]===true;});if(count)count.textContent='선택 후보 '+ids.length+'건';Array.prototype.forEach.call(document.querySelectorAll('[data-candidate-group="'+groupKey+'"]'),function(button){button.disabled=busy||!ids.length;});}
  async function runCandidateBulkAction(groupKey,action){
    ...
    if((action==='hold'||action==='reject')&&!window.confirm('선택한 후보 '+ids.length+'건을 '+(action==='hold'?'보류':'제외')+' 처리합니다. 관리자 수동 결정으로 고정됩니다. 계속할까요?'))return;
    ...
    try{
      if(ledgerRows.length===selectedRows.length&&ledgerRows.length){
        var candidateIds=Array.from(new Set(ledgerRows.map(function(row){return text(row.candidateId||row.id);}).filter(Boolean)));
        var bulk=await api(CONTROL,'product_candidate_ledger_bulk_action','POST',{}, {countryCode:selectedCountry,subdivisionCode:selectedSubdivision||'NATIONWIDE',candidateIds:candidateIds,decision:action==='place'?'slot_candidate':action,placementKey:placement||''},30000);
        ...
        for(var i=0;i<ids.length;i++){var ok=await applyProductDecision(ids[i],action==='place'?'slot_candidate':action,placement,true);if(ok)succeeded+=1;else failed+=1;}
      }
  ...
  function wireProductGroupDetails(root,groups,kind){Array.prototype.forEach.call(root.querySelectorAll('details[data-product-group]'),function(panel){panel.addEventListener('toggle',function(){var label=panel.querySelector('[data-group-toggle-label]');syncDetailsToggle(panel,label);if(panel.open){closeOtherHeavyPanels(kind==='candidate'?$('productCandidatePanel'):null,panel);Array.prototype.forEach.call(document.querySelectorAll('details[data-product-group][open]'),function(other){if(other!==panel){other.open=false;var otherGrid=other.querySelector('.product-group-grid');if(otherGrid)otherGrid.innerHTML='';}});var key=panel.getAttribute('data-product-group'),grid=panel.querySelector('.product-group-grid'),rows=(groups[key]||[]).slice(0,100);renderInBatches(grid,rows,function(row,index){return productCardHtml(row,index,kind,key);},100);}else{productRenderEpoch+=1;var grid=panel.querySelector('.product-group-grid');if(grid)grid.innerHTML='';}});});}
  ...
  function renderProductExcludedTable(kind){var rows=productExcludedRows(kind),body=$(kind==='hold'?'productHoldRows':'productRejectRows');if(!body)return;body.innerHTML=rows.length?rows.map(function(row,index){var url=safeExternalUrl(row.productUrl||row.url),title=text(row.productName||row.title||row.id),decision=productDecision(row),status=decision==='purge'?'영구 제외':(decision==='reject'?'제외':'보류');return'<tr data-review-anchor="product-excluded-'+esc(kind)+'-'+esc(reviewAnchorToken(row.id||url||title,index))+'"><td class="queue-number">'+(index+1)+'</td><td class="supplier-select-cell"><input type="checkbox" data-product-exclusion-select="'+esc(kind)+'" value="'+esc(row.id)+'" aria-label="'+esc(title)+' 선택"></td><td>'+(url?externalReviewAnchor(url,title,'queue-primary-link'):'<strong>'+esc(title)+'</strong>')+(url?'<br>'+externalReviewAnchor(url,'상품 원본 열기','queue-review-link'):'')+'</td><td>'+esc(row.supplierName||'-')+'</td><td><span class="product-exclusion-status '+(decision==='purge'?'permanent':'')+'">'+esc(status)+'</span><br><span class="small">'+esc(row.decisionAt||'-')+'</span></td></tr>';}).join(''):'<tr><td colspan="5" class="small">현재 '+(kind==='hold'?'보류':'제외')+' 상품이 없습니다.</td></tr>';syncProductExclusionSelection(kind);}
  ...
  function ensureLatestProductQueueControls(){
    ...
    if(!toolbar){toolbar=document.createElement('div');toolbar.id='latestProductQueueToolbar';toolbar.className='section-product-toolbar candidate-bulk-toolbar';toolbar.innerHTML='<label class="product-front-select-label"><input id="latestProductSelectAll" type="checkbox"> 최신 상품 전체 선택</label><span id="latestProductSelectedCount" class="pill info">선택 상품 0건</span><button id="latestSelectedQueueStageBtn" type="button" class="product-section-front-button" disabled>선택 상품 전체상품 대기열 등록</button><button id="latestSelectedAiBtn" type="button" class="product-section-ai-button" disabled>선택 상품 AI 검증·자동 배치</button><button id="latestAllQueueStageBtn" type="button" class="secondary" disabled>최신 상품 전체상품 대기열 등록</button><button id="latestAllAiBtn" type="button" class="product-section-ai-button" disabled>최신 상품 전체 AI 검증·자동 배치</button><button id="latestSelectedQueueUnmatchBtn" type="button" class="product-section-unmatch-button" disabled>선택 상품 대기열 매칭 해제</button><button id="latestSelectedQueueDeleteBtn" type="button" class="danger" disabled>선택 상품 목록 삭제</button>';grid.parentNode.insertBefore(toolbar,grid);}
  }
  function syncLatestProductQueueControls(){
    ensureLatestProductQueueControls();var ids=latestSelectedIds(),all=$('latestProductSelectAll'),count=$('latestProductSelectedCount'),status=text(lastProductJson&&lastProductJson.status),queueReady=status==='complete'||(productPausedForQueue&&productPauseServerConfirmed),generalBusy=latestQueueActionActive||productQueueStageActive||productLoopActive||productAutomationActive,stageBusy=generalBusy||!queueReady,hasRows=latestProductRows.length>0;
    /* Selection and safe unmatch are ledger-management controls and remain available
       independently of the research->queue write gate.  New queue registration and
       AI placement still require a completed or durably paused research job. */
    if(all){all.checked=hasRows&&ids.length===latestProductRows.length;all.indeterminate=ids.length>0&&ids.length<latestProductRows.length;all.disabled=generalBusy||!hasRows;}
    if(count)count.textContent='선택 상품 '+ids.length+'건';
    if($('latestSelectedQueueStageBtn'))$('latestSelectedQueueStageBtn').disabled=stageBusy||!ids.length;
    if($('latestAllQueueStageBtn'))$('latestAllQueueStageBtn').disabled=stageBusy||!hasRows;
    if($('latestSelectedAiBtn'))$('latestSelectedAiBtn').disabled=stageBusy||!ids.length;
    if($('latestAllAiBtn'))$('latestAllAiBtn').disabled=stageBusy||!hasRows;
    if($('latestSelectedQueueUnmatchBtn'))$('latestSelectedQueueUnmatchBtn').disabled=generalBusy||!ids.length;
    if($('latestSelectedQueueDeleteBtn'))$('latestSelectedQueueDeleteBtn').disabled=generalBusy||!ids.length;
    Array.prototype.forEach.call(document.querySelectorAll('[data-latest-queue-select]'),function(box){box.checked=latestSelectedProducts[box.value]===true;box.disabled=generalBusy;});
  }
  async function runLatestProductQueueAction(operation,scopeMode){
    ...
    if(unmatch&&!window.confirm('선택 상품 '+ids.length+'건의 비공개 상품 후보·중개수익 대기열 매칭만 해제합니다. 최신 리서치 목록과 리서치 진행상태는 그대로 유지합니다. 계속할까요?'))return;
    if(deleting&&!window.confirm('선택 상품 '+ids.length+'건을 현재 최신 상품 리서치 목록에서 삭제합니다. 비공개 대기열의 미결정 항목은 함께 정리하고, 이미 배치·공개·관리자 고정 등 후속 상태가 있는 상품은 그 후속 상태만 안전하게 보존합니다. 영구 제외가 아니므로 다음 상품 리서치에서 다시 발견될 수 있습니다. 계속할까요?'))return;
    latestQueueActionActive=true;syncLatestProductQueueControls();var totalHandled=0,totalFailed=0,totalBlocked=0,totalEligible=0,state=$('productQueueStageState'),requestedScope=scopeSnapshot(),requestedKey=scopeKey(requestedScope);
    try{
      if(unmatch||deleting){
        for(var off=0;off<selectedRows.length;off+=100){
          if(scopeKey()!==requestedKey)throw new Error('대기열 작업 중 선택 범위가 변경됐습니다.');
          var rowsChunk=selectedRows.slice(off,off+100),chunkIds=rowsChunk.map(latestProductKey).filter(Boolean),chunkRefs=latestRefsForRows(rowsChunk);
          if(state)state.textContent=(deleting?'최신 상품 대기열 삭제 ':'최신 상품 대기열 매칭 해제 ')+Math.min(off+rowsChunk.length,selectedRows.length)+'/'+selectedRows.length+'건';
        ...
        var pass=0,lastRemaining=null,stalled=0,maxPasses=320;
        while(pass<maxPasses){
          if(scopeKey()!==requestedKey)throw new Error('대기열 작업 중 선택 범위가 변경됐습니다.');
          if(state)state.textContent='최신 상품 전체 대기열 등록 · 현재 리서치 원장을 10건 단위로 안전 등록합니다.';
          var allResult=await api(CONTROL,'product_research_stage_current','POST',{}, {countryCode:requestedScope.country,subdivisionCode:requestedScope.region,jobId:lastProductJson&&lastProductJson.jobId||'',source:'latest_list_manual',operation:'stage'},20000),allPart=allResult&&allResult.partialQueue||{};
          if(allPart.migration===true){if(state)state.textContent='기존 대용량 상품 원장을 국가·지역 분할 원장으로 안전 전환 중 · '+Number(allPart.migrated||0)+'/'+Number(allPart.migrationTotal||0)+'건';pass+=1;await new Promise(function(resolve){setTimeout(resolve,180);});continue;}
          totalHandled+=Number(allPart.handled||0);totalFailed+=Number(allPart.failed||0);totalBlocked+=Number(allPart.blocked||0);totalEligible=Math.max(totalEligible,Number(allPart.eligible||0));
          var remaining=Number(allPart.remaining||0);if(state)state.textContent='최신 상품 전체 대기열 등록 · 처리 '+Number(allPart.done||0)+'/'+Number(allPart.eligible||0)+' · 남음 '+remaining;
          if(allPart.complete===true||remaining<=0)break;
          if(lastRemaining!=null&&remaining>=lastRemaining&&Number(allPart.handled||0)<=0)stalled+=1;else stalled=0;
          if(stalled>=4)throw new Error('대기열 등록 진행이 반복 정체되어 안전하게 중단했습니다. 저장된 위치부터 다시 실행할 수 있습니다.');
          lastRemaining=remaining;pass+=1;await new Promise(function(resolve){setTimeout(resolve,180);});
        }
        if(pass>=maxPasses)throw new Error('대기열 안전 반복 한도에 도달했습니다. 현재 저장 지점부터 다시 실행할 수 있습니다.');
      }else{
        for(var start=0;start<selectedRows.length;start+=100){
          var selectedChunkRows=selectedRows.slice(start,start+100),selectedChunkIds=selectedChunkRows.map(latestProductKey).filter(Boolean),selectedChunkRefs=latestRefsForRows(selectedChunkRows),selectedPass=0,selectedStalled=0,selectedRemaining=null;
          while(selectedPass<24){
            if(scopeKey()!==requestedKey)throw new Error('대기열 작업 중 선택 범위가 변경됐습니다.');
            if(state)state.textContent='선택 상품 대기열 등록 · 선택 '+Math.min(start+selectedChunkRows.length,selectedRows.length)+'/'+selectedRows.length+'건 · 10건 단위 저장';
            ...
            totalHandled+=Number(selectedPart.handled||0);totalFailed+=Number(selectedPart.failed||0);totalBlocked+=Number(selectedPart.blocked||0);totalEligible+=Number(selectedPart.eligible||0);
            ...
            if(selectedStalled>=3)throw new Error('선택 상품 대기열 등록이 반복 정체되어 저장 상태를 유지한 채 중단했습니다.');
            selectedRemaining=selectedRemain;selectedPass+=1;await new Promise(function(resolve){setTimeout(resolve,160);});
          }
          if(selectedPass>=24)throw new Error('선택 상품 대기열 안전 반복 한도에 도달했습니다.');
        }
      }
      ...
      if(!unmatch||deleting){selectedIds.forEach(function(id){delete latestSelectedProducts[id];});}
      if(state)state.textContent=(deleting?'최신 상품 대기열 삭제':(unmatch?'최신 상품 대기열 매칭 해제':'최신 상품 대기열 등록'))+' 완료 · '+(allMode?'현재 리서치 전체':'선택 '+ids.length+'건')+' · 반영 '+totalHandled+'건'+(totalBlocked?' · 보호/차단 '+totalBlocked+'건':'')+(totalFailed?' · 실패 '+totalFailed+'건':'');
    ...
    }catch(e){if(state)state.textContent='최신 상품 대기열 작업 중단: '+conciseRequestError(e&&e.message,e&&e.status);show(conciseRequestError(e&&e.message,e&&e.status),'warn');return{ok:false,error:e};}
  ...
  async function runLatestAi(scopeMode){
    ...
    var candidateIds=candidateIdsForLatestRows(rows);if(!candidateIds.length){show((allMode?'최신 상품':'선택 상품')+' 중 비공개 대기열 등록이 확인된 후보가 없습니다. 대기열·배치 전환 검증으로 연결 상태를 확인해 주세요.','warn');return;}
    ...
    await runProductAiAutomation('products','',candidateIds);try{await runProductTransitionAudit({silent:true});}catch(_auditError){}
  }
  ...
  async function runProductTransitionAudit(options){
    options=options||{};if(!selectedCountry)return null;var button=$('productTransitionAuditBtn'),state=$('productTransitionAuditState'),grid=$('productTransitionAuditGrid'),requested=scopeSnapshot(),requestedKey=scopeKey(requested);if(button)button.disabled=true;if(state&&!options.silent){state.className='product-transition-audit-state';state.textContent='현재 리서치 상태·최신 상품·비공개 후보 원장을 다시 읽어 대기열→후보→18개 섹션 연결을 교차 검증 중입니다.';}
    try{
      ...
      var phase=text(status&&status.status),phaseReady=phase==='complete'||(status&&status.pause&&status.pause.paused===true),auditOk=linkGap===0&&classificationGap===0;
      if(grid)grid.innerHTML=transitionAuditMetric('최신 상품',latestProductRows.length,'이번 리서치 목록','info')+transitionAuditMetric('대기열 적격',eligible,'검증 완료 기준','info')+transitionAuditMetric('이번 대기열 등록',staged,'현재 리서치 실행 원장',staged&&linkGap===0?'good':'info')+transitionAuditMetric('리서치↔대기열 연결',currentLinked,staged?'등록 '+staged+'건과 교차 확인':'등록 전 상태',linkGap===0?'good':'warn')+transitionAuditMetric('후보 관리 목록',active,'미배정·배치·복구 포함','info')+transitionAuditMetric('보류·제외',excluded,'후보 원장에는 보존','info')+transitionAuditMetric('18개 섹션 배치',assigned,'프론트 매칭 전 단계',repair===0?'good':'info')+transitionAuditMetric('배정정보 복구',repair,'AI/수동 재배치 대상',repair===0?'good':'warn');
      ...
      return{ok:auditOk,scope:requested,status:phase,phaseReady:phaseReady,latest:latestProductRows.length,eligible:eligible,staged:staged,currentLinked:currentLinked,total:total,active:active,excluded:excluded,assigned:assigned,repair:repair,issues:issues};
    }catch(e){if(state){state.className='product-transition-audit-state warn';state.textContent='대기열·배치 전환 검증 실패: '+conciseRequestError(e&&e.message,e&&e.status)+' · 기존 원장은 변경하지 않았습니다.';}if(!options.silent)show(conciseRequestError(e&&e.message,e&&e.status),'warn');return{ok:false,error:e};}
    finally{if(button)button.disabled=false;syncLatestProductQueueControls();}
  }
  ...
  async function runProductAiAutomation(mode,sectionKey,productIds){
    ...
    var candidateLedgerMode=candidateLedgerViewActive(),pendingDraft=productAiDraft(lastProductJson);if(!candidateLedgerMode&&Number(pendingDraft.pendingItems||0)>0){show('이전에 정상 완료된 AI 임시 정리 '+Number(pendingDraft.pendingItems||0)+'건이 최종 확정 대기 중입니다. 먼저 `AI 정리 최종 완료`를 눌러 확정한 뒤 다음 정리를 시작해 주세요.','warn');syncProductAiFinalizeControl();return;}
    if(candidateLedgerMode){
      ...
      await runCandidateLedgerAiRecovery(targets,{label:selectedModeLedger?'선택 상품':'상품 후보',confirm:true});return;
    }
  ...
  async function runProductFrontSync(operation,mode,sectionKey,productId,selection){
    ...
    try{
      ...
      aggregate.persistedCandidateIds=Object.keys(publishPersistedMap);aggregate.changedCandidateIds=Object.keys(changedMap);
