...
/* ===== FINAL RULE PATCH ===== */
(function(){
  window.__MARU_VOICE_ENABLED__ = window.__MARU_VOICE_ENABLED__ ?? true;

  function hideExtension(){
    if (window.MaruDetachedPane?.hide) MaruDetachedPane.hide();
  }
  function showExtension(opts={}){
    if (window.MaruDetachedPane?.open) {
      MaruDetachedPane.open({
        width: 'modal',
        heightRatio: 0.66,
        draggable: true,
        topFromInputRatio: 0.5,
        ...opts
      });
    }
  }
  ...
  window.__MARU_SET_VOICE__ = function(on){
    window.__MARU_VOICE_ENABLED__ = !!on;
  };

  window.__MARU_RESPOND__ = function(payload){
    const hasExt = !!window.MaruDetachedPane?.isOpen?.();
    if (hasExt) {
      window.maruVoiceSpeak?.(payload.text || '');
      return;
    }
    if (window.isCountryOpen?.() && window.countryHasData?.(payload.key)){
      window.maruVoiceSpeak?.(window.readCountry(payload.key));
      return;
    }
    if (window.isRegionOpen?.() && window.regionHasData?.(payload.key)){
      window.maruVoiceSpeak?.(window.readRegion(payload.key));
      return;
    }
    showExtension({ content: payload.answer || '준비된 자료가 없습니다.' });
    window.maruVoiceSpeak?.(payload.answer || '준비된 자료가 없습니다.');
  };
})();


/* === PATCH: UNCONDITIONAL TTS (NO INPUT TOUCH) === */
(function(){
  if (window.__MARU_TTS_PATCHED__) return;
  window.__MARU_TTS_PATCHED__ = true;

  function speakAlways(text){
    if (!text) return;
    if (typeof window.maruVoiceSpeak === 'function') {
      window.maruVoiceSpeak(text);
    }
  }

  const origRespond = window.MaruAddon && window.MaruAddon.respond;
  if (window.MaruAddon) {
    window.MaruAddon.respond = function(payload){
      try {
        const text =
          payload?.answer ||
          payload?.text ||
          payload?.message ||
          '';
        speakAlways(text || '준비된 자료가 없습니다.');
      } catch(e){}
      return origRespond ? origRespond.apply(this, arguments) : undefined;
    };
  }

  document.addEventListener('maru:answer', function(e){
    speakAlways(e.detail?.text || e.detail || '준비된 자료가 없습니다.');
  });
})();
