/*
 * IGDC / MARU MediaHub playback controller v4.0.0 fullscreen-stable + panel-safe-layout + idle-autohide + resume restore
...
(function (global, document) {
  ...
  function primeParentFullscreenPermission() {
    var ctx = parentFrameContext();
    if (!ctx || !ctx.frame) return false;
    try {
      var frame = ctx.frame;
      var current = String(frame.getAttribute('allow') || '');
      var tokens = current.split(';').map(function(v){ return v.trim(); }).filter(Boolean);
      ['autoplay','fullscreen','picture-in-picture','encrypted-media'].forEach(function(token){
        if (!tokens.some(function(v){ return v === token || v.indexOf(token + ' ') === 0; })) tokens.push(token);
      });
      frame.setAttribute('allow', tokens.join('; '));
      frame.setAttribute('allowfullscreen','');
      frame.setAttribute('webkitallowfullscreen','');
      try { frame.allowFullscreen = true; } catch (_) {}
      return true;
    } catch (_) { return false; }
  }
  ...
  function standaloneDisplayMode() {
    try {
      return !!((global.matchMedia && (global.matchMedia('(display-mode: standalone)').matches || global.matchMedia('(display-mode: fullscreen)').matches)) || (global.navigator && global.navigator.standalone === true));
    } catch (_) { return false; }
  }
  function requestMobileNativeFullscreen() {
    if (!isMobileSession() || !state.detail) return false;
    // Installed/standalone web apps already have no browser address bar. Avoid
    // the browser Fullscreen API there so Android's mandatory fullscreen safety
    // education banner is never triggered. In a normal browser tab, the safety
    // banner is browser-owned and cannot be suppressed by page JavaScript; the
    // Fullscreen API remains necessary if the URL bar itself must disappear.
    if (standaloneDisplayMode()) {
      state.mobileNativeFullscreenPending=false;
      state.mobileNativeFullscreenActive=false;
      return false;
    }
    if (state.mobileNativeFullscreenPending || mobileNativeFullscreenActive()) {
      state.mobileNativeFullscreenActive = mobileNativeFullscreenActive();
      return true;
    }
    var ctx = parentFrameContext();
    // IMPORTANT: request fullscreen from the MediaHub document that received
    // the user's tap. When an embedded child element enters Fullscreen, the
    // browser automatically promotes the ancestor iframe into the parent's
    // fullscreen chain. Requesting fullscreen on the parent iframe directly
    // from child code can be rejected on real mobile Chrome even when CSS
    // takeover makes the frame look full-screen, leaving browser chrome visible.
    // The child-element request is standards-compliant and removes the browser
    // address/navigation bar when Fullscreen is granted.
    // Prime the iframe permissions before the tap whenever possible, and again
    // synchronously here as a final guard. Do not overwrite unrelated allow tokens.
    primeParentFullscreenPermission();
  ...
  var PANEL_COPY = {
    ko:{targetLanguage:'번역 언어',liveVoiceTranslate:'실시간 음성 번역',captionSize:'자막 크기',captionSmall:'작게',captionNormal:'보통',captionLarge:'크게',colorTone:'색감',toneSoft:'부드럽게',toneNatural:'자연스럽게',toneVivid:'선명하게',brightness:'밝기',brightnessSoft:'부드럽게',brightnessNormal:'일반',brightnessBright:'밝게'},
    en:{targetLanguage:'Translation language',liveVoiceTranslate:'Live voice translation',captionSize:'Caption size',captionSmall:'Small',captionNormal:'Normal',captionLarge:'Large',colorTone:'Color tone',toneSoft:'Soft',toneNatural:'Natural',toneVivid:'Vivid',brightness:'Brightness',brightnessSoft:'Soft',brightnessNormal:'Normal',brightnessBright:'Bright'}
  };
  function panelCopy(){
    var lang=String((document.documentElement&&document.documentElement.lang)||'ko').toLowerCase();
    if(lang==='zh-hant'||lang==='zh-tw'||lang==='zh-hk')lang='zht';
    lang=lang.split('-')[0];
    return PANEL_COPY[lang]||PANEL_COPY.en;
  }
  var TRANSLATION_LANGUAGES = [
    ['ko','한국어'],['en','English'],['de','Deutsch'],['es','Español'],['fr','Français'],['it','Italiano'],['pt','Português'],['nl','Nederlands'],['pl','Polski'],['sv','Svenska'],['hu','Magyar'],['ru','Русский'],['uk','Українська'],['tr','Türkçe'],['ar','العربية'],['fa','فارسی'],['ur','اردو'],['hi','हिन्दी'],['bn','বাংলা'],['ta','தமிழ்'],['th','ไทย'],['vi','Tiếng Việt'],['id','Bahasa Indonesia'],['ms','Bahasa Melayu'],['tl','Filipino'],['sw','Kiswahili'],['uz','Oʻzbekcha'],['ja','日本語'],['zh','简体中文'],['zht','繁體中文']
  ];
  function defaultTranslationTarget(){
    var lang=String((document.documentElement&&document.documentElement.lang)||'ko').toLowerCase();
    if(lang==='zh-hant'||lang==='zh-tw'||lang==='zh-hk')lang='zht';
    lang=lang.split('-')[0];
    return TRANSLATION_LANGUAGES.some(function(item){return item[0]===lang;})?lang:'en';
  }
  function ensureTranslationTarget(){if(!state.translationTargetLang)state.translationTargetLang=defaultTranslationTarget();return state.translationTargetLang;}
  function applyDisplayPreferences(){
    if(!state.detail||isMobileUiMode())return;
    state.detail.dataset.captionSize=state.captionSize||'normal';
    state.detail.dataset.pictureTone=state.pictureTone||'natural';
    state.detail.dataset.pictureBrightness=state.pictureBrightness||'normal';
  }
  function setCaptionSize(value){state.captionSize=/^(small|large)$/.test(value)?value:'normal';applyDisplayPreferences();renderTrackPanel();}
  function setPictureTone(value){state.pictureTone=/^(soft|vivid)$/.test(value)?value:'natural';applyDisplayPreferences();renderPanel();}
  function setPictureBrightness(value){state.pictureBrightness=/^(soft|bright)$/.test(value)?value:'normal';applyDisplayPreferences();renderPanel();}
  function requestLiveVoiceTranslation(){requestTranslation();}
  ...
  var SVG = {
    ...
    gear:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.09a2 2 0 0 1 1 1.74v.5a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.38a2 2 0 0 0-.73-2.73l-.15-.09a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2Z"/><circle cx="12" cy="12" r="3"/></svg>',
    gearDesktop:'<svg viewBox="0 0 24 24" aria-hidden="true"><path class="fill" d="M19.43 12.98c.04-.32.07-.65.07-.98s-.03-.66-.08-.98l2.11-1.65a.5.5 0 0 0 .12-.64l-2-3.46a.5.5 0 0 0-.61-.22l-2.49 1a7.47 7.47 0 0 0-1.69-.98L14.5 2.42A.49.49 0 0 0 14 2h-4a.49.49 0 0 0-.49.42L9.13 5.07c-.61.25-1.17.59-1.69.98l-2.49-1a.49.49 0 0 0-.61.22l-2 3.46a.5.5 0 0 0 .12.64l2.11 1.65c-.05.32-.09.66-.09.98s.03.66.08.98l-2.11 1.65a.5.5 0 0 0-.12.64l2 3.46c.13.23.4.31.61.22l2.49-1c.52.4 1.08.73 1.69.98l.38 2.65c.04.24.24.42.49.42h4c.25 0 .46-.18.49-.42l.38-2.65c.61-.25 1.17-.58 1.69-.98l2.49 1c.22.09.49.01.61-.22l2-3.46a.5.5 0 0 0-.12-.64l-2.1-1.65ZM12 15.5A3.5 3.5 0 1 1 12 8a3.5 3.5 0 0 1 0 7.5Z"/></svg>',
  ...
  function injectStyle() {
    ...
    style.textContent = [
      ...
      '@media(min-width:821px) and (pointer:fine){.igdc-maru-panel{top:auto;right:18px;bottom:130px;width:min(360px,calc(100vw - 36px));height:auto;min-height:0;max-height:calc(100% - 212px);border-radius:13px;padding:13px 14px 14px;overflow-y:auto;scrollbar-gutter:stable;box-shadow:0 18px 52px rgba(0,0,0,.48)}.igdc-maru-panel-title{font-size:15px;margin-bottom:9px}.igdc-maru-panel-section{padding-top:8px;margin-top:8px}.igdc-maru-panel-label{margin-bottom:6px}.igdc-maru-panel-grid{gap:7px}.igdc-maru-panel-grid button{height:36px}.igdc-maru-panel-grid.igdc-grid-3{grid-template-columns:repeat(3,minmax(0,1fr))}.igdc-maru-panel-grid.igdc-grid-4{grid-template-columns:repeat(4,minmax(0,1fr))}.igdc-maru-panel-select{display:block;width:100%;height:36px;border:1px solid rgba(255,255,255,.12);border-radius:9px;background:#222933;color:#eef3f8;padding:0 10px;font:600 12.5px/1 system-ui;outline:none}.igdc-maru-panel-select:focus{border-color:rgba(97,169,255,.7);box-shadow:0 0 0 2px rgba(97,169,255,.12)}.igdc-maru-controls-row{right:16px;bottom:12px;gap:8px;max-width:60%;overflow-x:auto}.igdc-maru-controlbar{padding-left:14px;padding-right:14px;padding-bottom:12px}}',
      ...
      '#igdc-media-detail-view[data-caption-size="small"] video::cue{font-size:78%}#igdc-media-detail-view[data-caption-size="normal"] video::cue{font-size:100%}#igdc-media-detail-view[data-caption-size="large"] video::cue{font-size:132%}',
      '#igdc-media-detail-view[data-picture-tone="soft"]{--maru-saturation:.88;--maru-contrast:.96}#igdc-media-detail-view[data-picture-tone="natural"]{--maru-saturation:1;--maru-contrast:1}#igdc-media-detail-view[data-picture-tone="vivid"]{--maru-saturation:1.18;--maru-contrast:1.06}#igdc-media-detail-view[data-picture-brightness="soft"]{--maru-brightness:.90}#igdc-media-detail-view[data-picture-brightness="normal"]{--maru-brightness:1}#igdc-media-detail-view[data-picture-brightness="bright"]{--maru-brightness:1.10}@media(min-width:821px) and (pointer:fine){#igdc-media-detail-view .igdc-media-detail-stage video,#igdc-media-detail-view .igdc-media-detail-stage iframe{filter:brightness(var(--maru-brightness,1)) saturate(var(--maru-saturation,1)) contrast(var(--maru-contrast,1))}}',
      '@media(max-width:820px),(pointer:coarse){.igdc-maru-topbar{height:calc(50px + env(safe-area-inset-top));padding:env(safe-area-inset-top) max(6px,env(safe-area-inset-right)) 0 max(6px,env(safe-area-inset-left))}.igdc-maru-brand{font-size:13px}.igdc-maru-back-btn{max-width:150px;padding:0 7px!important;font-size:12px}.igdc-maru-menu{display:none!important}.igdc-maru-controlbar{min-height:84px!important;padding:6px 7px max(10px,env(safe-area-inset-bottom))!important}.igdc-maru-progress-row{height:32px;gap:7px;margin-bottom:4px}.igdc-maru-controls-row{position:absolute;right:max(7px,env(safe-area-inset-right));bottom:max(8px,env(safe-area-inset-bottom));left:auto;justify-content:flex-end;gap:7px;min-height:42px;max-width:100%}.igdc-maru-controls-row>[data-media-action]:not([data-mobile-inline="1"]){display:none!important}.igdc-maru-controls-row>.igdc-maru-spacer{display:none!important}.igdc-maru-icon-btn{width:44px;height:42px}.igdc-maru-text-btn{min-width:44px;height:42px;padding:0 7px;font-size:11.5px}.igdc-maru-volume{display:none}.igdc-maru-panel{top:calc(58px + env(safe-area-inset-top));bottom:calc(102px + env(safe-area-inset-bottom));right:max(7px,env(safe-area-inset-right));width:min(360px,calc(100vw - 14px));min-height:0;max-height:none;overflow-y:auto;padding-bottom:max(12px,env(safe-area-inset-bottom))}.igdc-media-detail-notice{bottom:100px}.igdc-media-detail-stage{width:100%!important;height:100%!important;min-width:0!important;min-height:0!important}.igdc-media-detail-stage> :not(.igdc-media-detail-pending):not(.igdc-transport-overlay){width:100%!important;height:100%!important}.igdc-transport-overlay{left:50%;top:50%;bottom:auto;transform:translate(-50%,-50%);gap:14px}.igdc-transport-overlay .igdc-maru-icon-btn,.igdc-transport-overlay .igdc-maru-text-btn{width:62px;height:62px;background:rgba(5,8,12,.52)}.igdc-transport-overlay [data-media-action="play"]{width:88px;height:88px}.igdc-transport-overlay [data-media-action="seek-back"],.igdc-transport-overlay [data-media-action="seek-forward"]{width:68px;height:68px}.igdc-transport-overlay [data-media-action="previous"],.igdc-transport-overlay [data-media-action="next"]{width:60px;height:60px}}',
      '@media(max-height:520px) and (orientation:landscape){.igdc-maru-topbar{height:calc(44px + env(safe-area-inset-top))}.igdc-maru-controlbar{min-height:72px!important;padding-top:3px!important;padding-bottom:max(5px,env(safe-area-inset-bottom))!important}.igdc-maru-progress-row{height:27px}.igdc-maru-controls-row{bottom:max(5px,env(safe-area-inset-bottom));min-height:36px}.igdc-maru-icon-btn,.igdc-maru-text-btn{height:36px}.igdc-maru-panel{top:calc(48px + env(safe-area-inset-top));bottom:calc(80px + env(safe-area-inset-bottom));min-height:0;max-height:none;overflow-y:auto}.igdc-transport-overlay{top:50%;gap:12px}.igdc-transport-overlay [data-media-action="play"]{width:84px;height:84px}}'
    ].join('');
  ...
  function setMobileViewMode(mode, options) {
    ...
    if(mode === 'immersive') {
      // Issue the native Fullscreen request as early as possible while the
      // original tap's transient user activation is still live. The parent
      // frame CSS takeover is applied immediately afterwards as the visual
      // fallback, not before the browser Fullscreen request.
      primeParentFullscreenPermission();
  ...
  function closePanelForIdle() {
    clearTimeout(state.panelTimer); state.panelTimer=0;
    if(!state.detail || !state.panel) return;
    var panel=state.detail.querySelector('.igdc-maru-panel');
    state.panel='';
    if(panel) panel.hidden=true;
    if(isMobileSession()) hideChromeNow();
    else { var video=currentVideo(); if(video && !video.paused) hideChromeNow(); }
  }
  function schedulePanelAutoHide(hold) {
    clearTimeout(state.panelTimer); state.panelTimer=0;
    if(!state.detail || !state.panel) return;
    state.panelTimer=setTimeout(closePanelForIdle, Math.max(2600, Number(hold)||5000));
  }
  function showChrome(hold) {
    ...
    if(isMobileSession()){
      state.chromeTimer=setTimeout(function(){
        if(!state.detail)return;
        if(state.panel){schedulePanelAutoHide(1600);return;}
        state.detail.classList.remove('igdc-chrome-visible');
      },hold||3600);
      return;
    }
  ...
  function syncUi() {
    ...
    updateTimeUi(); renderTrackPanel(); if(!isMobileUiMode())global.requestAnimationFrame(positionDesktopTransportSafely);
  }
  ...
  // V42R DESKTOP-ONLY: keep the center transport group clear of the right control lane.
  // Mobile/coarse-pointer sessions return before any geometry is changed.
  function positionDesktopTransportSafely() {
    if(!state.detail || isMobileUiMode()) return;
    var transport=state.detail.querySelector('.igdc-transport-overlay');
    var controls=state.detail.querySelector('.igdc-maru-controls-row');
    if(!transport || !controls) return;
    try {
      transport.style.transform='translateX(-50%) scale(1)';
      var db=state.detail.getBoundingClientRect();
      var tb=transport.getBoundingClientRect();
      var rb=controls.getBoundingClientRect();
      if(!db.width || !tb.width || !rb.width) return;

      var rightStart=rb.left;
      var nodes=controls.querySelectorAll('button,label,input');
      Array.prototype.forEach.call(nodes,function(node){
        if(!node) return;
        var cs=global.getComputedStyle ? global.getComputedStyle(node) : null;
        if(cs && (cs.display==='none' || cs.visibility==='hidden')) return;
        var r=node.getBoundingClientRect();
        if(r.width>2 && r.height>2) rightStart=Math.min(rightStart,r.left);
      });

      var gap=Math.max(30,Math.min(52,db.width*.026));
      var leftPad=Math.max(18,Math.min(34,db.width*.016));
      var usable=Math.max(1,rightStart-gap-(db.left+leftPad));
      var scale=Math.min(1,usable/tb.width);
      if(scale<.58) scale=.58;
      var half=(tb.width*scale)/2;
      var minCenter=db.left+leftPad+half;
      var maxCenter=rightStart-gap-half;
      if(maxCenter<minCenter){
        scale=Math.max(.46,Math.min(scale,usable/tb.width));
        half=(tb.width*scale)/2;
        minCenter=db.left+leftPad+half;
        maxCenter=rightStart-gap-half;
      }
      // Prefer slightly left of center so the transport group never collides
      // with captions/audio/volume/tools/fullscreen controls on narrower Windows layouts.
      var desired=db.left+(db.width*.455);
      var center=maxCenter>=minCenter?Math.max(minCenter,Math.min(desired,maxCenter)):minCenter;
      transport.style.left=Math.round(center-db.left)+'px';
      transport.style.transform='translateX(-50%) scale('+scale.toFixed(3)+')';
      transport.dataset.desktopSafeLane='1';
    } catch(_) {}
  }

  function bindDesktopLayoutObserver() {
    if(!state.detail || isMobileUiMode() || !global.ResizeObserver) return;
    try {
      if(state.desktopLayoutObserver) state.desktopLayoutObserver.disconnect();
      state.desktopLayoutObserver=new global.ResizeObserver(function(){
        positionDesktopTransportSafely();
        positionDesktopPanelSafely();
      });
      state.desktopLayoutObserver.observe(state.detail);
      var transport=state.detail.querySelector('.igdc-transport-overlay');
      var controls=state.detail.querySelector('.igdc-maru-controls-row');
      if(transport) state.desktopLayoutObserver.observe(transport);
      if(controls) state.desktopLayoutObserver.observe(controls);
    } catch(_) { state.desktopLayoutObserver=null; }
  }

  function positionDesktopPanelSafely() {
    if(!state.detail || isMobileUiMode()) return;
    var panel=state.detail.querySelector('.igdc-maru-panel');
    if(!panel || panel.hidden) return;
    var topbar=state.detail.querySelector('.igdc-maru-topbar');
    var controlbar=state.detail.querySelector('.igdc-maru-controlbar');
    var detailRect=state.detail.getBoundingClientRect();
    var topRect=topbar&&topbar.getBoundingClientRect();
    var controlRect=controlbar&&controlbar.getBoundingClientRect();
    var safeTop=Math.max(66,Math.round((topRect?topRect.bottom:detailRect.top+54)-detailRect.top+10));
    var safeBottom=Math.max(18,Math.round(detailRect.bottom-(controlRect?controlRect.top:detailRect.bottom-112)+18));
    var available=Math.max(160,Math.round(state.detail.clientHeight-safeTop-safeBottom));
  ...
  function openPanel(name) {
    ...
    state.panel=name||'tools';panel.hidden=false;panel.dataset.panel=state.panel;renderPanel();if(!isMobileUiMode())global.requestAnimationFrame(positionDesktopPanelSafely);showChrome(4400);schedulePanelAutoHide(4200);
  }
  ...
  function renderPanel() {
    ...
    function grid(sec,items,columns){var g=create('div','igdc-maru-panel-grid'+(columns?' igdc-grid-'+columns:''));items.forEach(function(it){var b=create('button','',it.label);b.type='button';if(it.action)b.dataset.mediaAction=it.action;if(it.value!=null)b.dataset.mediaValue=String(it.value);if(it.active)b.dataset.active='true';g.appendChild(b);});sec.appendChild(g);return g;}
    function languageSelect(sec){var pc=panelCopy(),sel=create('select','igdc-maru-panel-select');sel.dataset.mediaTranslationLanguage='1';sel.setAttribute('aria-label',pc.targetLanguage);var target=ensureTranslationTarget();TRANSLATION_LANGUAGES.forEach(function(item){var opt=create('option','',item[1]);opt.value=item[0];if(item[0]===target)opt.selected=true;sel.appendChild(opt);});sec.appendChild(sel);return sel;}
    if(name==='view'){
      ...
      var pc=panelCopy();
      var s3=section(e.subtitle);
      if(isMobileUiMode())grid(s3,[{label:captionsEnabled()?c.captionsOff:c.captionsOn,action:'captions'},{label:c.translate,action:'translate'}]);
      else{grid(s3,[{label:captionsEnabled()?c.captionsOff:c.captionsOn,action:'captions'},{label:c.translate,action:'translate'},{label:pc.liveVoiceTranslate,action:'live-voice-translate'}],3);var langSec=section(pc.targetLanguage);languageSelect(langSec);var sizeSec=section(pc.captionSize);grid(sizeSec,[{label:pc.captionSmall,action:'caption-size',value:'small',active:state.captionSize==='small'},{label:pc.captionNormal,action:'caption-size',value:'normal',active:state.captionSize==='normal'},{label:pc.captionLarge,action:'caption-size',value:'large',active:state.captionSize==='large'}],3);}
      ...
      if(isMobilePlaybackDevice()){
        ...
        var pc2=panelCopy();
        var shotSec=section(e.tools);grid(shotSec,[{label:e.shot,action:'shot'},{label:e.pip,action:'pip'}],2);
        var clipSec=section(e.clipTitle);grid(clipSec,[{label:e.clip20,action:'clip',value:20},{label:e.clip30,action:'clip',value:30},{label:e.clip60,action:'clip',value:60},{label:e.clip120,action:'clip',value:120}],2);clipSec.appendChild(create('div','igdc-maru-panel-hint',e.captureHint));
        var toneSec=section(pc2.colorTone);grid(toneSec,[{label:pc2.toneSoft,action:'picture-tone',value:'soft',active:state.pictureTone==='soft'},{label:pc2.toneNatural,action:'picture-tone',value:'natural',active:state.pictureTone==='natural'},{label:pc2.toneVivid,action:'picture-tone',value:'vivid',active:state.pictureTone==='vivid'}],3);
        var brightSec=section(pc2.brightness);grid(brightSec,[{label:pc2.brightnessSoft,action:'picture-brightness',value:'soft',active:state.pictureBrightness==='soft'},{label:pc2.brightnessNormal,action:'picture-brightness',value:'normal',active:state.pictureBrightness==='normal'},{label:pc2.brightnessBright,action:'picture-brightness',value:'bright',active:state.pictureBrightness==='bright'}],3);
      }
    }
    if(!isMobileUiMode())global.requestAnimationFrame(positionDesktopPanelSafely);
  }
  ...
  function close(options){options=options||{};if(!state.open)return;saveCurrentProgress(true);if(isMobileSession()&&state.mobileViewMode!=='immersive')setParentFrameMode('list');if(!options.fromHistory&&state.historyToken&&global.history&&global.history.state&&global.history.state.igdcMediaToken===state.historyToken){global.history.back();return;}cancelClipCapture();if(isPlayerFullscreen())leaveFullscreen();disposeStage();if(state.desktopLayoutObserver){try{state.desktopLayoutObserver.disconnect();}catch(_desktopObserver){}state.desktopLayoutObserver=null;}if(state.detail)state.detail.remove();restoreList();var y=state.scrollY;state.open=false;state.detail=null;state.stage=null;state.card=null;state.historyToken='';state.lastCaptionValue='';state.panel='';state.playRequested=false;state.mobileFullscreenIntent=false;state.mobileImmersiveActive=false;state.mobileSession=false;state.mobileViewMode='list';unbindParentViewportRepair();state.justExitedFullscreenAt=0;state.orientationChangingUntil=0;state.nativeVideoFullscreen=false;state.mobileNativeFullscreenPending=false;state.mobileNativeFullscreenActive=false;state.mobileNativeExitIntent=false;state.youtubeFrame=null;state.youtubeCurrentTime=0;state.youtubeDuration=0;state.youtubeEnded=false;unlockMobileOrientation();clearTimeout(state.chromeTimer);clearTimeout(state.panelTimer);state.panelTimer=0;clearTimeout(state.orientationTimer);if(state.fullscreenBodyOverflow!==null){document.body.style.overflow=state.fullscreenBodyOverflow;state.fullscreenBodyOverflow=null;}global.requestAnimationFrame(function(){global.scrollTo(0,y);frameHeight();});}
  ...
  function buildPlayerShell(card) {
    ...
    row.appendChild(iconButton('loop','loop',e.loopOn,''));row.appendChild(mobileInline(iconButton('shot','camera',e.shot,'')));row.appendChild(mobileInline(iconButton('tools-panel',isMobileUiMode()?'gear':'gearDesktop',e.settings,'')));row.appendChild(mobileInline(iconButton('fullscreen','fullscreen',c.fullscreen,'')));
  ...
  document.addEventListener('pointerdown',function(event){
    ...
    if(card&&card.getAttribute('data-placeholder')!=='true')primeParentFullscreenPermission();
