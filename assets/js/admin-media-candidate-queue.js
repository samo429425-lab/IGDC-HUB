/* IGDC Media Candidate Queue v4.0 - section selection + AI policy bridge + stable media front controls */
(function(){
  'use strict';
  ...
  function sectionReserveCapacity(key){return key==='media-trending'?0:20;}
  function manualSectionKeys(){return SECTION_ORDER.filter(function(key){return key!=='media-trending';});}
  function mediaAiPolicy(sectionKey,purpose){
    try{
      if(window.IGDCMediaAI&&typeof window.IGDCMediaAI.policyEnvelope==='function')return window.IGDCMediaAI.policyEnvelope(sectionKey||'',purpose||'global')||null;
    }catch(_e){}
    return null;
  }
  function mediaAiQuery(sectionKey,purpose){
    var p=mediaAiPolicy(sectionKey,purpose);
    if(!p)return'';
    var parts=[];
    if(Array.isArray(p.includeTopics))parts=parts.concat(p.includeTopics);
    if(Array.isArray(p.preferredContentTraits))parts=parts.concat(p.preferredContentTraits);
    return parts.map(text).filter(Boolean).slice(0,10).join(' ');
  }
  ...
  var poolCache=null;
  var poolSelected=new Set();
  var selectedMediaSections=new Set();
  ...
  function usableCandidateThumbnail(row){
    var url=safeWebUrl(row&&row.thumb);var value=lower(url);
    if(!url||value.indexOf('media-sample-card.png')>=0||value.indexOf('placeholder')>=0||value.indexOf('placehold.co')>=0||value.indexOf('placehold.it')>=0)return'';
    return url;
  }
  function thumbnailRepairRows(rows){return (rows||[]).filter(function(row){return !usableCandidateThumbnail(row);});}
  function activeRows(){
  ...
  function renderSummary(summary){
    ...
    $('summaryGrid').innerHTML=[
      ...
      card('썸네일 보완 대기',thumbnailRepairRows(activeRows()).length,'프론트 공급 전 생성·검증 필수'),
  ...
  function candidateCardHtml(row,sectionKey,readOnly){
    ...
    var thumb=usableCandidateThumbnail(row);
  ...
  function thumbnailRepairBodyHtml(list){
    if(!list.length)return '<div class="empty-section">썸네일 보완이 필요한 후보가 없습니다.</div>';
    return '<div class="read-only-note">이 구역의 콘텐츠는 원본 썸네일 확인 또는 프레임 생성이 성공하기 전에는 프론트 스냅샷에 들어가지 않습니다.</div><div class="candidate-card-grid">'+list.map(function(row){
      var id=text(row.contentId||row.id);
      return '<article class="candidate-card" data-candidate-id="'+esc(id)+'"><button type="button" class="candidate-thumb-button previewBtn" data-candidate-id="'+esc(id)+'"><span class="candidate-thumb-fallback">썸네일 생성 필요</span></button><div class="candidate-card-body"><div class="candidate-card-title">'+esc(row.title||'(제목 없음)')+'</div><div class="candidate-card-detail">'+esc(row.provider||'-')+' · '+esc(row.sectionKey||'-')+' · '+esc(row.reviewStatus||'-')+'</div><div class="candidate-card-footer"><button type="button" class="previewBtn" data-candidate-id="'+esc(id)+'">점검 재생</button><button type="button" class="secondary thumbnailGenerateBtn" data-candidate-id="'+esc(id)+'">썸네일 생성</button></div></div></article>';
    }).join('')+'</div>';
  }
  function renderRows(){
    var rows=visibleRows();
    var allActive=activeRows();
    var thumbVisible=thumbnailRepairRows(rows),thumbAll=thumbnailRepairRows(allActive);
    var thumbOpen=openSectionKey==='media-thumbnail-quarantine';
    var thumbSection='<section class="candidate-section'+(thumbOpen?' open':'')+'" data-section-key="media-thumbnail-quarantine"><button type="button" class="section-toggle" data-section-key="media-thumbnail-quarantine" aria-expanded="'+(thumbOpen?'true':'false')+'"><span class="section-toggle-main"><span class="section-toggle-title">썸네일 보완 대기</span><span class="section-count">'+thumbAll.length+'개'+(thumbVisible.length!==thumbAll.length?' · 표시 '+thumbVisible.length:'')+'</span></span><span class="section-chevron">⌄</span></button>'+(thumbOpen?'<div class="section-body">'+thumbnailRepairBodyHtml(thumbVisible)+'</div>':'')+'</section>';
    var html=thumbSection+SECTION_ORDER.map(function(key){
      ...
      var reserveCount=Math.min(sectionReserveCapacity(key),Math.max(0,fullCount-capacity));
      var overflowCount=Math.max(0,fullCount-capacity-sectionReserveCapacity(key));
      ...
      var batchAllowed=key!=='media-trending';
      var batchChecked=batchAllowed&&selectedMediaSections.has(key);
      return '<section class="candidate-section'+(open?' open':'')+'" data-section-key="'+esc(key)+'">'+
        '<div class="section-header">'+
          (batchAllowed?'<label class="section-batch-select"><input type="checkbox" class="section-batch-check" data-section-key="'+esc(key)+'" '+(batchChecked?'checked':'')+' aria-label="'+esc(SECTION_LABELS[key]||key)+' 섹션 선택"><span>섹션 선택</span></label>':'')+
          ...
          '<div class="section-header-actions"><button type="button" class="secondary sectionAiPolicyBtn" data-section-key="'+esc(key)+'">AI 정책 대화</button></div>'+
        '</div>'+
        (open?'<div class="section-body">'+sectionBodyHtml(key,list)+'</div>':'')+
      '</section>';
    }).join('');
    $('sectionAccordion').innerHTML=html;
    updateMediaSectionBatchUi();
      ...
      while(attempts<3&&!data){
        ...
        try{
          data=await post(COL,{
            ...
            qualityResearch:true,aiPolicy:mediaAiPolicy(section,'collector'),policyQuery:mediaAiQuery(section,'collector')
          });
        }catch(error){
  ...
  async function collect(admin,wholeRun,targetOverride){
    ...
    if(admin){
      var body={
        ...
        overrideReason:text($('adminOverrideReason').value),aiPolicy:mediaAiPolicy(section,'collector'),policyQuery:mediaAiQuery(section,'collector')
      };
    ...
    try{
      while((job.saved<job.target||visitBatches<minimumCoverageBatches)&&visitBatches<maxBatchesThisVisit&&!collectorStopRequested){
        ...
        while(attempts<3&&!data){
          attempts+=1;
          try{
            data=await post(COL,{source:'internet_archive',section:job.section,target:job.target,batchSize:requestBatchSize,page:job.page,batchMode:true,aiPolicy:mediaAiPolicy(job.section,'collector'),policyQuery:mediaAiQuery(job.section,'collector')});
          }catch(error){
            if((error.status===502||error.status===503||error.status===504)&&attempts<3){
  ...
  async function candidateAction(action,ids,sectionElement){
    ...
    var labels={approve:'승인',hold:'보류',reset:'재검토',reject:'반려',block:'영구 제외·차단'};
  ...
  async function removeCandidates(ids,note,sectionKey){
    ...
    var message='선택한 '+ids.length+'개 후보를 현재 후보 목록에서 삭제할까요? 원본 영상은 삭제되지 않으며, 이후 리서치에서 같은 콘텐츠가 다시 발견되면 후보로 다시 들어올 수 있습니다.';
    if(!window.confirm(message))return;
    try{
      var data=await post(ACT,{action:'forget',ids:ids,note:text(note),confirmPermanentDelete:true});
      show('후보 목록에서 '+Number(data.deleted||data.updated||0)+'건을 삭제했습니다. 이후 리서치 재발견은 허용됩니다.','ok');
      if(sectionKey)selectedBySection[sectionKey]=new Set();
      await refresh();
    }catch(error){show(error.message,'warn');}
  }
  ...
  function renderSuppliers(){
    ...
    body.innerHTML=supplierCache.map(function(row){
      ...
      var evidence=text(row.research&&row.research.snippet||row.notes);
      ...
      return '<tr><td><input type="checkbox" class="supplierSelectRow" data-id="'+esc(row.id)+'" aria-label="'+esc(row.name)+' 선택"></td><td><strong class="supplier-status-'+esc(status)+'">'+esc(supplierStatusLabel(status))+'</strong></td><td><strong>'+esc(row.name)+'</strong><div class="mono small">'+esc(row.id)+'</div></td><td>'+esc(supplierTypeLabel(row.supplierType))+'</td><td>'+esc(row.country||'-')+'</td><td>'+(website?'<a href="'+esc(website)+'" target="_blank" rel="noopener noreferrer" style="color:#9fdcff">'+esc(row.websiteHost||website)+'</a>':'-')+'</td><td>'+esc(terms||'-')+(evidence?'<div class="small" style="margin-top:5px">'+esc(evidence.slice(0,240))+'</div>':'')+'</td><td>'+esc(row.updatedAt||'-')+'</td><td><div class="supplier-row-actions">'+management+'</div></td></tr>';
    }).join('');
  ...
  async function researchSuppliers(mode){
    ...
    try{
      var baseQuery=text($('supplierResearchQuery').value),policyQuery=mediaAiQuery('', 'supplier');
      var data=await post(SUP,{action:'research',mode:mode,query:[baseQuery,policyQuery].filter(Boolean).join(' '),country:text($('supplierCountry').value),supplierType:allMode?'other':$('supplierType').value,limit:allMode?120:60,aiPolicy:mediaAiPolicy('', 'supplier')});
      ...
      if(Number(data.saved||0)>0)show('공급사 자동 리서치 완료 · 검색 '+Number(data.searched||0)+'건 · 정책 적격 '+Number(data.qualified||0)+'건 · 후보 '+Number(data.saved||0)+'건 저장','ok');
        ...
        var reason=diagnosis==='public_search_api_not_configured'
          ?'외부 공개 검색 API가 설정되지 않았습니다 · Google CSE/Naver/Bing 중 하나 이상을 설정하세요'
          :diagnosis==='all_research_lanes_returned_zero_results'
            ?('검색 결과 자체가 0건입니다'+(configured.length?' · 사용 가능 외부 검색원 '+configured.join(', '):''))
  ...
  async function collectSupplierContents(id){
    ...
    try{
      var data=await post(SUP,{action:'collect_contents',id:id,section:section,limit:30,aiPolicy:mediaAiPolicy(section,'supplier')});
      await refresh();
  ...
  async function autoCurateAll(){
    ...
    try{
      while(batches<100){
        ...
        var data=await post(AUTO,{cursor:cursor,batchSize:15,aiPolicy:mediaAiPolicy('', 'pool')});
  ...
  function setFrontButtonsDisabled(disabled){
    ['publishFrontBtn','publishAllSectionsFrontBtn','stopFrontBtn','selectedSectionsPublishBtn','selectedSectionsStopBtn','autoCurateBtn'].forEach(function(id){if($(id))$(id).disabled=disabled;});
  ...
  function updateMediaSectionBatchUi(){
    var keys=manualSectionKeys();
    var count=keys.filter(function(key){return selectedMediaSections.has(key);}).length;
    var all=$('selectAllMediaSections');
    if(all){all.checked=count===keys.length&&keys.length>0;all.indeterminate=count>0&&count<keys.length;}
    if($('selectedMediaSectionCount'))$('selectedMediaSectionCount').textContent='선택 섹션 '+count+'개';
    if($('selectedSectionsPublishBtn'))$('selectedSectionsPublishBtn').disabled=count===0;
    if($('selectedSectionsStopBtn'))$('selectedSectionsStopBtn').disabled=count===0;
    document.querySelectorAll('.section-batch-check').forEach(function(box){box.checked=selectedMediaSections.has(text(box.dataset.sectionKey));});
  }
  function setAllMediaSections(checked){
    selectedMediaSections.clear();
    if(checked)manualSectionKeys().forEach(function(key){selectedMediaSections.add(key);});
    updateMediaSectionBatchUi();renderRows();
  }
  async function publishSelectedSections(stopping){
    var keys=manualSectionKeys().filter(function(key){return selectedMediaSections.has(key);});
    if(!keys.length){show('먼저 반영할 섹션을 선택해 주세요.','warn');return;}
    var label=stopping?'선택 섹션 반영 해제':'선택 섹션 프론트 반영';
    if(!window.confirm(label+' · '+keys.map(function(k){return SECTION_LABELS[k]||k;}).join(', ')+' 을 실행할까요?'))return;
    setFrontButtonsDisabled(true);
    try{
      if(!stopping){for(var i=0;i<keys.length;i+=1)await frontMappingPreflight('publish_section',keys[i]);}
      var bulkAction=stopping?'stop_sections':'publish_sections';
      var data=null,bulkOk=false;
      try{
        data=await post(PUB,{storeRelease:true,publishFront:true,frontAction:bulkAction,sectionKeys:keys,includeSnapshot:'0',includeBlocked:'1'});
        bulkOk=!!data;
      }catch(error){
        if(!(error.status===400||error.status===404||error.status===409||error.status===422))throw error;
      }
      if(!bulkOk){
        for(var j=0;j<keys.length;j+=1){
          data=await post(PUB,{storeRelease:true,publishFront:true,frontAction:stopping?'stop_section':'publish_section',sectionKey:keys[j],includeSnapshot:'0',includeBlocked:'1'});
        }
      }
      if(data&&data.frontState)frontReleaseState=data.frontState;
      renderRows();
      show(label+' 완료 · '+keys.length+'개 섹션','ok');
    }catch(error){show(label+' 실패: '+error.message,'warn');}
    finally{setFrontButtonsDisabled(false);updateMediaSectionBatchUi();}
  }
  ...
  function poolSections(){
    if(!poolCache||!poolCache.sections)return [];
    var filter=text($('poolSectionFilter')&&$('poolSectionFilter').value);
    return Object.keys(poolCache.sections).filter(function(key){return !filter||key===filter;});
  }
  function renderPool(){
    if(!$('poolRows'))return;
    if(!poolCache||!poolCache.sections){$('poolRows').innerHTML='<tr><td colspan="10" class="small">후보 운영 현황을 불러오지 못했습니다.</td></tr>';return;}
    var keys=poolSections();
    $('poolSummary').innerHTML=keys.map(function(key){var sec=poolCache.sections[key]||{};return '<div class="card"><h2>'+esc(SECTION_LABELS[key]||key)+'</h2><div class="small">운영 '+Number(sec.primaryCount||0)+'/'+Number(sec.primaryCapacity||0)+' · 예비 '+Number(sec.reserveCount||0)+'/20 · 최신 '+Number(sec.latestCount||0)+' · 실제 프론트 '+Number(sec.publishedCount||0)+'</div></div>';}).join('');
    var rows=[];var bucket=text($('poolBucketFilter')&&$('poolBucketFilter').value)||'latest';
    keys.forEach(function(key){var sec=poolCache.sections[key]||{};var names=bucket==='all'?['primary','reserve','latest','held','overflow']:[bucket];names.forEach(function(name){(sec[name]||[]).forEach(function(row){rows.push(row);});});});
    rows.sort(function(a,b){return Number(b.score||0)-Number(a.score||0);});
    var bucketLabel={primary:'현재 운영',reserve:'예비 후보',latest:'최신 후보',held:'보류',overflow:'교체 대기'};
    $('poolRows').innerHTML=rows.length?rows.map(function(row){var id=text(row.id),checked=poolSelected.has(id),label=bucketLabel[row.bucket]||row.bucket||'-',klass=row.bucket==='primary'?'pool-status-primary':row.bucket==='reserve'?'pool-status-reserve':row.bucket==='latest'?'pool-status-latest':'pool-status-held';return '<tr><td><input class="poolRowCheck" type="checkbox" data-id="'+esc(id)+'" '+(checked?'checked':'')+'></td><td>'+pill(SECTION_LABELS[row.sectionKey]||row.sectionKey,'section')+'</td><td><span class="'+klass+'">'+esc(label)+'</span></td><td><strong>'+esc(row.title||'(제목 없음)')+'</strong><div class="small">'+esc(row.provider||'')+'</div></td><td>'+Number(row.score||0).toFixed(1)+'</td><td>'+esc(row.year||'-')+'</td><td>'+Number(row.downloads||0).toLocaleString()+'</td><td>'+esc(row.qualityHeight?row.qualityHeight+'p':'-')+'</td><td>'+esc(row.reviewStatus||'-')+' / '+esc(row.verificationStatus||'-')+'</td><td>'+(row.published?'배치 중':row.frontEnabled?'운영풀 후보':'비활성')+'</td></tr>';}).join(''):'<tr><td colspan="10" class="small">선택한 조건의 후보가 없습니다.</td></tr>';
    $('poolState').textContent='후보 운영 현황 · '+keys.length+'개 섹션 · 표시 '+rows.length+'개';
  }
  async function refreshPool(){
    if(!$('poolState'))return;
    $('poolState').textContent='후보 운영 현황 확인 중';
  ...
  function poolSelectedRowMap(){
    var map={};
    if(!poolCache||!poolCache.sections)return map;
    Object.keys(poolCache.sections).forEach(function(key){
      var sec=poolCache.sections[key]||{};
      ['primary','reserve','latest','held','overflow'].forEach(function(bucket){
        (sec[bucket]||[]).forEach(function(row){var id=text(row&&row.id);if(id)map[id]=row;});
      });
    });
    return map;
  }
  async function poolManualAction(action){
    var ids=selectedPoolIds();if(!ids.length){show('후보 운영 목록에서 대상을 선택해 주세요.','warn');return;}
    var labels={promote:'운영풀 승격',hold:'보류',delete:'삭제(재검색 허용)',permanent_block:'영구 제외·차단',auto:'자동관리/보류 복구'};
    var label=labels[action]||action;
    var detail=action==='hold'?'기존 보류 기능을 사용합니다. 후보는 DB에 남고 복구할 수 있습니다.':action==='delete'?'현재 후보 행만 삭제합니다. 원본 영상은 삭제하지 않으며 이후 리서치에서 다시 발견될 수 있습니다.':action==='permanent_block'?'기존 영구 차단 상태를 사용합니다. 후보 행을 남겨 자동 리서치 재유입을 막습니다.':action==='auto'?'보류 항목은 보류 직전 상태로 복구하고 후보풀 수동 제어를 해제합니다.':'입력된 후보를 운영풀 우선 후보로 지정합니다.';
    if(!window.confirm('선택 '+ids.length+'개를 '+label+' 처리할까요?\n\n'+detail))return;
    try{
      var data={updated:0,deleted:0,rejected:[]};
      if(action==='promote'){
        data=await post(POOL,{action:'promote',ids:ids});
      }else if(action==='hold'){
        data=await post(ACT,{action:'hold',ids:ids,note:'candidate_pool_hold'});
      }else if(action==='delete'){
        data=await post(ACT,{action:'forget',ids:ids,note:'candidate_pool_delete_recollect_allowed',confirmPermanentDelete:true});
      }else if(action==='permanent_block'){
        data=await post(ACT,{action:'block',ids:ids,note:'candidate_pool_permanent_block'});
      }else if(action==='auto'){
        var rowMap=poolSelectedRowMap();
        var heldIds=ids.filter(function(id){return lower(rowMap[id]&&rowMap[id].reviewStatus)==='hold';});
        var resumed=0;
        if(heldIds.length){var restoreData=await post(ACT,{action:'resume',ids:heldIds,note:'candidate_pool_resume'});resumed=Number(restoreData.updated||0);}
        var autoData=await post(POOL,{action:'auto',ids:ids});
        data=autoData||{};data.updated=Math.max(Number(data.updated||0),resumed);data.resumed=resumed;
      }
      poolSelected.clear();await refreshPool();await refresh();
      show(label+' '+Number(data.deleted||data.updated||0)+'건 완료'+(data.rejected&&data.rejected.length?' · 승인 미완료 '+data.rejected.length+'건 제외':''),'ok');
    }catch(error){show(label+' 실패: '+error.message,'warn');}
  }

  async function researchLatestAll(){
    if(collectAllRunning)return;
    var target=Math.max(1,Math.min(20,Number($('collectorLimit').value)||5));
    if(!window.confirm('운영 후보 수량과 관계없이 9개 섹션의 최신 콘텐츠를 각 '+target+'개 목표로 추가 리서치할까요?\n\n새 콘텐츠는 기존 운영/예비 후보와 섞어 즉시 공개하지 않고 최신 리서치 후보 목록에 들어갑니다.'))return;
    var sections=SECTION_ORDER.filter(function(key){return key!=='media-trending';});var total=0,errors=0;
    collectAllRunning=true;collectorStopRequested=false;$('poolResearchLatestBtn').disabled=true;$('collectorStopBtn').disabled=false;
    try{
      for(var i=0;i<sections.length&&!collectorStopRequested;i+=1){
        $('collectorSection').value=sections[i];$('poolState').textContent='최신 후보 리서치 '+(i+1)+'/9 · '+(SECTION_LABELS[sections[i]]||sections[i]);
        var result=await collect(false,true,target);total+=Number(result&&result.newSavedAll||0);if(result&&result.error)errors+=1;
      }
      await refresh();await refreshPool();
      show('최신 후보 전체 리서치 완료 · 신규 '+total+'건'+(errors?' · 일시 오류 '+errors+'회':'')+' · 새 후보는 별도 최신 목록에서 검토합니다.',errors?'warn':'ok');
  ...
  async function poolAutoRebalance(){
    ...
    $('poolAutoRebalanceBtn').disabled=true;$('poolState').textContent='AI 자동 비교·치환 중';
    try{
      var data=await post(POOL,{action:'auto_rebalance',sectionKey:section,aiPolicy:mediaAiPolicy(section,'pool')});
      var pub=await post(PUB,{storeRelease:true,publishFront:true,frontAction:section?'publish_section':'publish_all',sectionKey:section||undefined,includeSnapshot:'0',includeBlocked:'1'});
      var dispatch=pub.frontPublication||{};if(dispatch.queued!==true)throw new Error(frontPublishReason(dispatch.reason));
      await refreshPool();await refresh();show('AI 자동 비교·치환 완료 · 후보 상태 '+Number(data.updated||0)+'건 조정 · 프론트 재배포 요청 완료','ok');
    }catch(error){show('AI 자동 비교·치환 실패: '+error.message,'warn');}
    finally{$('poolAutoRebalanceBtn').disabled=false;}
  }
  function downloadPool(){
    if(!poolCache){show('후보 운영 현황을 먼저 새로고침해 주세요.','warn');return;}
    download(JSON.stringify(poolCache,null,2)+'\n','igdc-media-candidate-pool-status.json');
  }

  function bind(){
    $('refreshBtn').onclick=refresh;
    $('poolRefreshBtn').onclick=refreshPool;
    ...
    $('poolPromoteBtn').onclick=function(){poolManualAction('promote');};
    $('poolHoldBtn').onclick=function(){poolManualAction('hold');};
    $('poolDeleteBtn').onclick=function(){poolManualAction('delete');};
    $('poolPermanentBlockBtn').onclick=function(){poolManualAction('permanent_block');};
    $('poolAutoBtn').onclick=function(){poolManualAction('auto');};
    $('poolRows').addEventListener('change',function(event){if(!event.target.classList.contains('poolRowCheck'))return;var id=text(event.target.dataset.id);if(event.target.checked)poolSelected.add(id);else poolSelected.delete(id);});
    ...
    if($('selectAllMediaSections'))$('selectAllMediaSections').onchange=function(){setAllMediaSections(this.checked);};
    if($('selectedSectionsPublishBtn'))$('selectedSectionsPublishBtn').onclick=function(){publishSelectedSections(false);};
    if($('selectedSectionsStopBtn'))$('selectedSectionsStopBtn').onclick=function(){publishSelectedSections(true);};
    ...
    $('sectionAccordion').addEventListener('click',function(event){
      var aiButton=event.target.closest('.sectionAiPolicyBtn');
      if(aiButton){var aiKey=text(aiButton.dataset.sectionKey);if(window.IGDCMediaAI&&typeof window.IGDCMediaAI.openDialog==='function')window.IGDCMediaAI.openDialog('section',aiKey,(SECTION_LABELS[aiKey]||aiKey)+' 섹션');return;}
    ...
    $('sectionAccordion').addEventListener('change',function(event){
      var sectionBatch=event.target.closest('.section-batch-check');
      if(sectionBatch){var key=text(sectionBatch.dataset.sectionKey);if(sectionBatch.checked)selectedMediaSections.add(key);else selectedMediaSections.delete(key);updateMediaSectionBatchUi();return;}
