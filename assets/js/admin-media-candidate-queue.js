...
(function(){
  ...
  async function ensureFrontPublishThumbnails(sectionKey){
    var targets=activeRows().filter(function(row){
      ...
      if(sectionKey&&text(row.sectionKey)!==sectionKey)return false;
    ...
    async function one(row){
      var id=text(row.contentId||row.id);
      try{
        // Validate any stored/provider thumbnail first. If it is absent/broken,
        // capture only one of the first few seconds; never scan the whole video.
        var resolved=await post(THUMB,{action:'resolve',id:id});
        result.checked+=1;
        ...
        // Do not destroy an approved media slot because one provider blocks frame capture.
        // The front renderer keeps the official sample image as the visual fallback.
        result.checked+=1;result.fallback+=1;
      }
    }
    async function worker(){
      while(true){
        var index=cursor++;if(index>=targets.length)return;
        $('collectorState').textContent='프론트 반영 전 썸네일 준비 '+(index+1)+'/'+targets.length;
        await one(targets[index]);
      }
    }
    // Small admin-only parallelism: fast enough without hammering video/provider origins.
  ...
  async function publishFrontAction(action,sectionKey,sectionKeys){
    ...
    try{
      ...
      if(!stopping){
        ...
        thumbPrep=await ensureFrontPublishThumbnails(sectionKey);
      }
