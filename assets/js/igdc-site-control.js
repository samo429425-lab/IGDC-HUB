...
\n\n
/* =========================================================
 * IGDC SITE CONTROL — SAFE ROLE FIX (FINAL)
 * - Keep layout intact
 * - Remove AI automatic diagnostic coupling
 * - Summary card opens SiteDiagnosticModal
 * - '자동 진단 실행' acts as local reset/refresh
 * ========================================================= */
(function(){
  try{
    // 1) Summary card -> open SiteDiagnosticModal only
    const summaryCard = document.querySelector('[data-site-diagnostic], .site-ai-diagnostic');
    if(summaryCard){
      summaryCard.addEventListener('click', function(e){
        e.stopPropagation();
        if (window.SiteDiagnosticModal && typeof SiteDiagnosticModal.open === 'function') {
          SiteDiagnosticModal.open();
        }
      }, true);
    }

    // 2) Rebind '자동 진단 실행' as local refresh/reset (no AI / no MARU)
    const resetBtn = Array.from(document.querySelectorAll('button'))
      .find(b => b.textContent && b.textContent.trim() === '자동 진단 실행');
    if(resetBtn){
      resetBtn.onclick = function(e){
        e.stopPropagation();
        // If local diagnostics functions exist, re-run them safely
        try{
          window.runLocalDiagnostics && runLocalDiagnostics();
          window.updateSiteSummary && updateSiteSummary();
        }catch(_){}
      };
    }

    // 3) Guard: prevent any global AI updater from touching site diagnostic area
    document.addEventListener('igdc:ai:update', function(ev){
      if(ev && ev.detail && ev.detail.role === 'maru-global'){
        // ignore for site diagnostic area
