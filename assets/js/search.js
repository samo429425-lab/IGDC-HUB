    ...
    function renderSearchOwnedResultView(url, it, opts){
      ...
      if(!opts.skipHistory) try{
        ...
        history.pushState({
          ...
          type: activeType,
          target: target,
          title: String((it && it.title) || target).slice(0, 180),
          view: 'result'
        }, '', u.toString());
      }catch(e){}
      ...
      kicker.textContent = uiText('searchResultDetail', 'Search result detail');
      ...
      const factsData = [
        sourceLabel ? `${uiText('source', 'Source')} ${sourceLabel}` : '',
        domainOf(target) ? `${uiText('domain', 'Domain')} ${domainOf(target)}` : '',
        it && it.searchCategory ? `${uiText('category', 'Category')} ${it.searchCategory}` : '',
        it && it.lang ? `${uiText('language', 'Language')} ${it.lang}` : ''
      ].filter(Boolean);
      factsData.forEach(v => {
      ...
      if(related.length){
        ...
        rt.textContent = uiText('relatedSearch', 'Related searches');
      ...
      if(proxySrc){
        ...
        proxyHead.className = 'maru-search-owned-proxy-head';
        const proxyTitle = document.createElement('div');
        proxyTitle.className = 'maru-search-owned-proxy-title';
        proxyTitle.textContent = uiText('internalSource', 'Internal original view');
        const proxyHelp = document.createElement('div');
        proxyHelp.className = 'maru-search-owned-proxy-help';
        proxyHelp.textContent = uiText('internalSourceHelp', 'The original page is displayed inside IGDC Search.');
        proxyHead.appendChild(proxyTitle);
        proxyHead.appendChild(proxyHelp);
