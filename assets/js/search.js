...
function adaptiveSearchTarget(q, type){
  ...
  if (words.length >= 3 || narrowHints.test(text)) target = 1500;
  if (words.length <= 1 || broadHints.test(text)) target = 3000;
  if (/^(news|image|video|sns|blog|tour)$/.test(safeType)) target = Math.max(target, 2400);
  if (/^(map|knowledge|book|shopping)$/.test(safeType)) target = Math.max(1200, Math.min(target, 2200));
...
async function fetchInstantSearchPack(q, type = activeType){
  ...
  try {
    ...
    function collectNaturalImages(it){
      ...
      const mediaPreview = it && it.media && it.media.preview && typeof it.media.preview === 'object' ? it.media.preview : {};
      const raw = []
        ...
        .concat(mediaPreview.poster ? [mediaPreview.poster] : [])
        .concat(mediaPreview.thumbnail ? [mediaPreview.thumbnail] : [])
        .concat(Array.isArray(it && it.imageSet) ? it.imageSet : [])
    ...
    function inferDisplayGroupClient(it){
      ...
      if (source.includes('public-data') || source.includes('opendata') || source.includes('open-data') || type === 'dataset' || type === 'public_data' || text.includes('공공데이터') || text.includes('공공 데이터') || text.includes('데이터셋') || text.includes('open data') || text.includes('dataset')) return 'public_data';
      ...
      if (source.includes('academic') || source.includes('scholar') || source.includes('research') || source.includes('paper') || type === 'academic' || type === 'paper' || type === 'research' || text.includes('학술') || text.includes('논문') || text.includes('연구') || text.includes('학회') || text.includes('저널') || text.includes('thesis') || text.includes('journal') || text.includes('doi') || host.includes('scholar.google') || host.includes('arxiv.org') || host.includes('doi.org')) return 'academic';
      ...
      if (source.includes('shopping') || source.includes('shop') || source.includes('commerce') || type === 'shopping' || type === 'product' || text.includes('쇼핑') || text.includes('상품') || text.includes('구매') || text.includes('가격') || /(?:coupang|gmarket|auction|11st|amazon|ebay|aliexpress|shop|store|mall)/i.test(host)) return 'shopping';
      if (source.includes('sports') || type === 'sports' || text.includes('스포츠') || text.includes('축구') || text.includes('야구') || text.includes('농구') || /(?:sports|espn|fifa|kbo|mlb|nba|kfa)/i.test(host)) return 'sports';
      if (source.includes('finance') || type === 'finance' || text.includes('금융') || text.includes('증권') || text.includes('주식') || text.includes('환율') || /(?:finance|stock|market|krx|investing|bloomberg|marketwatch|tradingview)/i.test(host)) return 'finance';
      if (source.includes('webtoon') || type === 'webtoon' || text.includes('웹툰') || text.includes('만화') || text.includes('comic') || text.includes('manga') || /(?:webtoon|comic|manga)/i.test(host)) return 'webtoon';
    ...
    function renderGroupedSlice(slice, page){
      ...
      groups.forEach(groupInfo => {
        ...
        previewItems.forEach((it, idx) => renderItem(markMapPreviewPolicyClient(it, groupInfo.group, idx, false), body));
        ...
        if (hiddenItems.length) {
          ...
          more.addEventListener('click', () => {
            ...
            if(!hiddenMounted){
              ...
              hiddenItems.forEach((it, idx) => renderItem(markMapPreviewPolicyClient(it, groupInfo.group, previewItems.length + idx, true), hiddenWrap));
    ...
    function resultDescriptionClient(it){
      const payload = it && it.payload && typeof it.payload === 'object' ? it.payload : {};
      const data = it && it.data && typeof it.data === 'object' ? it.data : {};
      ...
      for(const v of candidates){
        ...
        if(text && text.length >= 8) return text.slice(0, 420);
      }
      return '';
    }

    function markMapPreviewPolicyClient(it, group, indexInGroup, hidden){
      const copy = Object.assign({}, it || {});
      const g = group || displayGroupOfItem(copy);
      if(g === 'local_tour' || normalizeSearchType(activeType) === 'map'){
        copy.__maruMapPreviewAllowed = !hidden && (parseInt(indexInGroup, 10) || 0) < 3;
      } else {
    ...
    function shouldRenderMapPreviewClient(it){
      ...
      if(it && it.__maruMapPreviewAllowed === true) return true;
      if(it && it.__maruMapPreviewAllowed === false) return false;
      return false;
    }
    ...
    function renderItem(it, mountTarget){
      ...
      d.className = 'desc';
      d.textContent = resultDescriptionClient(it);
...
// 그 외는 아예 표시 안 함 (safe 제거)
      ...
      const mapPreviewNode = (!playableMediaNode && !isRealThumb && shouldRenderMapPreviewClient(it)) ? renderMapPreviewClient(it) : null;
      if (mapPreviewNode) {
    ...
    function renderPage(page, skipEnrich = false){
      ...
      if (slice.some(isDisplayGroupModule)) {
        slice.forEach(entry => {
          ...
          else renderItem(markMapPreviewPolicyClient(entry, displayGroupOfItem(entry), 999, true));
        });
      } else if (shouldUseDisplayGroups(slice)) {
        renderGroupedSlice(slice, page);
      } else {
        slice.forEach((it, idx) => renderItem(markMapPreviewPolicyClient(it, displayGroupOfItem(it), idx, false)));
      }
