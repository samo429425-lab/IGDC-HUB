...
async function fetchInstantSearchPack(q, type = activeType){
  ...
  try {
    ...
    function openResultInsideSearchFrame(url, it){
      ...
      // Search result click policy:
      // - never replace search.html on the search page
      // - no iframe viewer / no confirmation panel
      // - open the original page immediately from the user click
      // - keep the current search list, tabs and pager untouched in this tab
      if(isSearchPage){
        try{
