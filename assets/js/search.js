...
function adaptiveSearchTarget(q, type){
  ...
  // 300 candidates ready immediately and let the faucet keep filling up to roughly 4,500.
  let target = 3600;
  if (safeType !== 'all') target = 3000;
  if (words.length >= 3 || narrowHints.test(text)) target = 3000;
  if (words.length <= 1 || broadHints.test(text)) target = 4500;
  if (/^(news|image|video|sns|blog|tour)$/.test(safeType)) target = Math.max(target, 3600);
  if (/^(map|knowledge|book|shopping)$/.test(safeType)) target = Math.max(2500, Math.min(target, 3600));
...
async function runSearch(q, type = activeType){
  ...
  try{
    ...
    // Search.js is an always-open receiver. First paint must never wait for
    // the rest of the stream, and the backfill faucet must start immediately
    // after the first usable supply pack. Page 1~12 are only the initial
    // preload window; pages after that keep flowing into loadedServerPages.
    startIntakeOnce('first-supply-open-receiver');
