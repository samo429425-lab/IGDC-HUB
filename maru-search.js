...
const VERSION = 'A1.5.36-570-pinset-nonbreaking-collapse-google-sns-youtube';
...
function youtubePosterCandidates(videoId, current){
  ...
  // YouTube 카드는 대표 썸네일 1개만 내려준다.
  // maxres/hq/mq를 동시에 imageSet에 넣으면 프론트에서 작은 카드/큰 카드가 중복 노출될 수 있다.
...
function mediaProfileForItem(it){
  ...
  const isYouTubeLike = !!((sourceText.includes('youtube') || typeText === 'video') && videoId);
  if(isYouTubeLike){
    images = youtubePosterCandidates(videoId, images[0]).slice(0, 1);
  }

  const explicitOriginals = isYouTubeLike ? images : qualitySortImagesForItem([
    it.originalImage, it.fullImage, it.imageOriginal, it.viewerImage, it.openImageUrl, it.contentUrl,
  ...
  const imageSet = isYouTubeLike ? compactImages([bestDisplay || original || images[0]]).slice(0, 1) : qualitySortImagesForItem([bestDisplay, original].concat(images), it).slice(0, 6);
...
function buildSearchSections(items, q, opts){
  ...
  const sections = SEARCH_SECTION_ORDER.map(sectionId => {
    ...
    return {
      ...
      // 전체 검색 후보를 자르지 않고, 접힌 항목을 별도 필드로 보존한다.
      // 프론트가 collapsedItems를 지원하면 접힘을 펼칠 수 있고,
      // 지원하지 않아도 기존 items/results 파이프라인은 그대로 유지된다.
      collapsedItems: all.slice(previewLimit),
      hiddenItems: all.slice(previewLimit),
      allItems: all,
      more: {
        q: safeString(q || ''),
        type: sectionId,
...
function parseExpandedSectionSet(raw){
  raw = raw || {};
  const out = new Set();
  const add = v => safeString(v).split(',').map(x => x.trim()).filter(Boolean).forEach(x => out.add(x));
  add(raw.expandedSections || raw.expanded || raw.openSections || raw.openSection);
  const action = safeString(raw.action || raw.mode || raw.fn).trim().toLowerCase();
  if(action === 'expand-section') add(raw.section || raw.sourceSectionId || raw.displaySectionId || raw.type);
  return out;
}
...
function buildCollapseAwareVisiblePagePack(items, q, raw, fullSectionPack){
  ...
  for(let i=0; i<flat.length; i += 3){
    ...
    const sourceId = sectionIdForItem(chunk[0]);
    const displayId = sourceId + '_viewport_' + (++displaySeq);
    const full = fullById[sourceId] || null;
    const fullItems = full && Array.isArray(full.allItems) ? full.allItems : (full && Array.isArray(full.items) ? full.items : []);
    const chunkKeys = new Set(chunk.map(x => safeString(firstNonEmpty(x && x.url, x && x.link, x && x.openUrl, x && x.id, x && x.title)).toLowerCase()).filter(Boolean));
    const collapsed = fullItems.filter(x => !chunkKeys.has(safeString(firstNonEmpty(x && x.url, x && x.link, x && x.openUrl, x && x.id, x && x.title)).toLowerCase()));
    const isExpanded = expanded.has(sourceId) || expanded.has(displayId);
    const visible = isExpanded ? chunk.concat(collapsed) : chunk;
    for(const item of visible){
      stream.push(Object.assign({}, item, {
        sectionId: displayId,
        displaySectionId: displayId,
        sourceSectionId: sourceId,
        _viewportSourceId: sourceId,
        _viewportDisplayId: displayId,
        _viewportExpanded: isExpanded
      }));
    }
  }

  const totalItems = stream.length;
  const totalPages = Math.max(totalItems ? 1 : 0, Math.ceil(totalItems / perPage));
  const safePage = totalPages ? Math.min(page, totalPages) : 1;
  const offset = totalPages ? (safePage - 1) * perPage : 0;
  ...
  return {
    enabled: true,
    mode: 'collapse-aware-visible-stream-do-not-limit-search-body',
    q: safeString(q || ''),
    page: safePage,
    requestedPage: page,
    perPage,
    ...
    principle: 'collapsed cards are excluded from the visible stream; expanding a section inserts them into the stream and pushes later cards to later pages.'
  };
}
...
function buildViewportDisplaySections(pageItems, q, fullSectionPack, raw){
  ...
  function flush(){
    ...
    const sourceId = current.sourceSectionId;
    const displayId = current.displaySectionId;
    const meta = SEARCH_SECTION_META[sourceId] || SEARCH_SECTION_META.general_web;
    const full = fullById[sourceId] || null;
    const fullItems = full && Array.isArray(full.allItems) ? full.allItems : (full && Array.isArray(full.items) ? full.items : []);
    const visibleKeys = new Set(current.items.map(x => safeString(firstNonEmpty(x && x.url, x && x.link, x && x.openUrl, x && x.id, x && x.title)).toLowerCase()).filter(Boolean));
    const collapsedItems = fullItems
      .filter(x => !visibleKeys.has(safeString(firstNonEmpty(x && x.url, x && x.link, x && x.openUrl, x && x.id, x && x.title)).toLowerCase()))
      .slice(0, 60)
    ...
    out.push({
      ...
      sourceTotal: full && typeof full.total === 'number' ? full.total : current.items.length + collapsedItems.length,
      previewLimit: current.items.length,
      hasMore: !isExpanded && collapsedItems.length > 0,
      collapsedCount: !isExpanded ? collapsedItems.length : 0,
      hiddenCount: !isExpanded ? collapsedItems.length : 0,
      items: current.items,
      collapsedItems: isExpanded ? [] : collapsedItems,
      hiddenItems: isExpanded ? [] : collapsedItems,
      expanded: isExpanded,
      more: {
        q: safeString(q || ''),
        type: sourceId,
        ...
        hiddenCount: !isExpanded ? collapsedItems.length : 0,
        sourceTotal: full && typeof full.total === 'number' ? full.total : current.items.length + collapsedItems.length,
        principle: 'collapsedItems are outside items, so they do not consume the visible 15-card viewport.'
      }
    });
    current = null;
  }
  ...
  for(let i=0; i<list.length; i++){
    ...
    const sourceId = safeString(rawItem && (rawItem.sourceSectionId || rawItem._viewportSourceId)).trim() || sectionIdForItem(rawItem);
    const displayId = safeString(rawItem && (rawItem.displaySectionId || rawItem._viewportDisplayId || rawItem.sectionId)).trim() || (sourceId + '_viewport_' + (out.length + 1));
    const meta = SEARCH_SECTION_META[sourceId] || SEARCH_SECTION_META.general_web;
    if(!current || current.displaySectionId !== displayId){
      flush();
      current = { sourceSectionId: sourceId, displaySectionId: displayId, items: [] };
    }
    current.items.push(Object.assign({}, rawItem, {
      sectionId: displayId,
      displaySectionId: displayId,
      sourceSectionId: sourceId,
      sectionTitle: meta.title,
  ...
  return {
    enabled: true,
    mode: 'viewport-visible-card-sections-backfill-15-collapse-aware-nonbreaking',
    ...
    principle: '접힘은 유지하지만 접힌 항목은 items 밖 collapsedItems에 두고, 현재 페이지 items 합계가 15장이 되도록 뒤 항목을 앞으로 당긴다.'
  };
}
...
function buildImmediateResidentResponse(q, raw, residentPack, baseMeta){
  ...
  const viewportSections = buildViewportDisplaySections(visiblePagePack.pageItems, q, fullSectionPack, raw || {});
...
const Containers = {
  ...
  web_google_sns: { async fetch(q, limit, start){ return googleSocialSearch(q, limit, start); } },
...
async function orchestrateSearch({ event, q, limit, start, lang, deep, externalOff, externalMode, noMedia, searchType }){
  ...
  const work = (async () => {
    ...
    async function pullFromGoogleSns(){
      let count = 0;
      if(timeLeft() <= 1700) { record('google_sns', 'skipped-time', 0); return 0; }
      const s = await Containers.web_google_sns.fetch(q, 10, start || 1).catch(() => null);
      const n = addBundle(s, 'google_sns', collected, sourceState);
      count += n;
      record('google_sns', count ? 'ok' : 'empty', count);
      return count;
    }
...
async function googleSocialSearch(q, limit, start){
  ...
  const cx = envFirst('GOOGLE_CSE_ID','GOOGLE_CX','GOOGLE_SEARCH_ENGINE_ID','GOOGLE_CUSTOM_SEARCH_ENGINE_ID','GOOGLE_PROGRAMMABLE_SEARCH_ENGINE_ID');
  if(!key || !cx) return null;
  const socialQuery = safeString(q).trim() + ' (site:instagram.com OR site:facebook.com OR site:tiktok.com OR site:x.com OR site:twitter.com OR site:threads.net)';
  const url = 'https://www.googleapis.com/customsearch/v1' +
    '?key=' + encodeURIComponent(key) +
    '&cx=' + encodeURIComponent(cx) +
    '&q=' + encodeURIComponent(socialQuery) +
    '&num=' + Math.min(limit || 10, 10) +
    '&start=' + (start || 1) +
    '&hl=ko' +
    '&gl=kr' +
    '&safe=off';
  const data = await fetchWithTimeout(url, null, 3000).then(r => r.ok ? r.json() : null).catch(() => null);
  const items = Array.isArray(data && data.items) ? data.items : [];
  return {
    source: 'google_sns',
    results: items.map(it => {
      const pagemap = it.pagemap || {};
      const cseThumb = Array.isArray(pagemap.cse_thumbnail) && pagemap.cse_thumbnail[0] ? pagemap.cse_thumbnail[0].src : '';
      const og = Array.isArray(pagemap.metatags) && pagemap.metatags[0] ? (pagemap.metatags[0]['og:image'] || pagemap.metatags[0]['twitter:image'] || '') : '';
      return {
        title: it.title || '', link: it.link || '', url: it.link || '', snippet: it.snippet || '',
        type: 'sns', mediaType: 'article', source: 'google_sns', provider: 'google_sns',
        thumbnail: cseThumb || og, thumb: cseThumb || og, image: cseThumb || og, imageSet: compactImages([cseThumb, og]),
        payload: { source: 'google_sns', displayLink: it.displayLink, formattedUrl: it.formattedUrl }
      };
    })
  };
}
...
async function youtubeSearch(q, limit){
  ...
  const results = (data.items || []).map(it => {
    ...
    const oneThumb = videoId ? 'https://i.ytimg.com/vi/' + videoId + '/hqdefault.jpg' : thumb;
    return { title: (it.snippet && it.snippet.title) || '', link: watchUrl, url: watchUrl, snippet: (it.snippet && it.snippet.description) || '', type: 'video', mediaType: 'video', source: 'youtube', thumbnail: oneThumb, thumb: oneThumb, image: oneThumb, imageUrl: oneThumb, imageSet: compactImages([oneThumb]).slice(0,1), originalImage: oneThumb, fullImage: oneThumb, viewerImage: oneThumb, openImageUrl: oneThumb, contentUrl: oneThumb, cardImage: oneThumb, videoId, videoUrl: watchUrl, watchUrl, embedUrl: videoId ? 'https://www.youtube.com/embed/' + videoId : '', openUrl: watchUrl, media: { type: 'video', preview: { poster: oneThumb, original: oneThumb, videoUrl: watchUrl, embedUrl: videoId ? 'https://www.youtube.com/embed/' + videoId : '' } }, payload: { source: 'youtube', thumb: oneThumb, originalImage: oneThumb, videoId, videoUrl: watchUrl } };
  });
