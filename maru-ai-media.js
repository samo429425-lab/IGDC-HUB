...
function isMaruInstructionLeak(value) {
  ...
  return /(?:system\s*(?:prompt|message|policy)|developer\s*message|internal\s*(?:policy|instruction)|return\s+(?:json|only)|create\s+accurate\s+timed\s+subtitle|preserve\s+proper\s+nouns|target\s+(?:subtitle\s+)?language|source\s+language\s+hint|subtitle\s+translation\s+engine|never\s+reproduce\s+or\s+mention\s+these\s+instructions|audible\s+(?:spoken|dialogue|lyric)|clearly\s+sung|musical\s+(?:note|marker|image)|instrumental[-\s]*only|vocal\s+onset|internal\s+(?:rule|guideline)|(?:자막|가사|음성)\s*(?:생성|번역|출력)\s*(?:규칙|지침|정책|안내)?|(?:명확하게|실제)\s*(?:노래|가사|음성)|(?:짧은|하나의)\s*(?:음표|음악)|(?:가사가|음성이)\s*명확하게\s*(?:불려|들려)|음악적(?:이거나|인)|(?:내부|시스템)\s*(?:지침|정책|규정|안내))/iu.test(text);
}

function dropMaruInstructionLeakSegments(segments) {
  return (Array.isArray(segments) ? segments : []).filter((seg) => {
    const text = safeString(seg?.text || '').trim();
    if (!text || isMaruInstructionLeak(text)) return false;
    // Preserve every real spoken/sung cue. Only explicit non-speech labels are removed.
    return !/^\s*[\[(（]?(?:music|instrumental|background music|silence|무음|연주|음악만|효과음)[\])）]?\s*$/iu.test(text);
  });
}
...
function buildTranscriptionFields(body, options = {}) {
  ...
  // Never send server-policy prose as a transcription prompt. Whisper treats prompts
  // as previous transcript context, so an internal instruction can surface as a cue.
  // A caller may provide only a short literal proper-name context; operational words
  // are rejected and no default prompt is ever supplied.
  const requestedPrompt = safeString(body.transcriptionPrompt || '').replace(/\s+/g, ' ').trim();
  const safeLiteralContext = requestedPrompt
    && requestedPrompt.length <= 240
    && !/(?:return|output|translate|preserve|instruction|policy|prompt|subtitle|caption|rule|guideline|번역|지침|정책|규칙|자막|출력|음성인식)/iu.test(requestedPrompt)
    ? requestedPrompt : '';
  if (safeLiteralContext) fields.prompt = safeLiteralContext;
  return fields;
}
