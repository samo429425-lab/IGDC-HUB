(function(){
  ...
  async function saveAssignment(){if(!requireCandidate())return;var stateV=$('assignmentState').value,ready=(stateV==='approved'||stateV==='pinned')?'ready':'not_ready';await api('assignment.save',{candidateId:selectedId,hubKey:$('assignmentHub').value,countryCode:$('assignmentCountry').value,regionCode:$('assignmentRegion').value,slotKey:$('assignmentSlot').value,priority:$('assignmentPriority').value,state:stateV,publicationStatus:ready,manualPinned:$('assignmentPinned').checked,note:$('assignmentNote').value},'POST');note('슬롯 배치를 저장했습니다. 승인/고정 상태면 발행 준비로 표시됩니다.','success');openCandidate(selectedId)}
