...
const VERSION = 'A1.5.14-balanced-tabs-natural-multi-image';
...
function balanceMixedResults(ranked){
  ...
  const order = ['official','news','tour','map','knowledge','image','video','blog','cafe','sns','web','shopping','sports','finance','webtoon'];
  ...
  function take(cat, n){
    ...
    let c = 0;
    for(const it of arr){
      const key = safeString(firstNonEmpty(it.id, it.url, it.link, it.title));
  ...
  // First screen: balanced, not official-only.
  take('official', 3);
  take('news', 3);
  take('tour', 2);
  take('map', 2);
